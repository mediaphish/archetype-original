...
import { pinAutoThread } from '../../../../lib/ao/autoHub.js';
...
export default async function handler(req, res) {
  ...
  try {
    const thread = await pinAutoThread(auth.email, threadId);
    return res.status(200).json({ ok: true, thread });
  } catch (e) {
    const msg = e.message || 'Could not pin chat';
    const code = /up to 15/i.test(msg) ? 400 : 500;
