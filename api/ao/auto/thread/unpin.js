...
import { unpinAutoThread } from '../../../../lib/ao/autoHub.js';
...
export default async function handler(req, res) {
  ...
  try {
    const thread = await unpinAutoThread(auth.email, threadId);
    return res.status(200).json({ ok: true, thread });
  } catch (e) {
    return res.status(500).json({ ok: false, error: e.message || 'Could not unpin chat' });
  }
}
