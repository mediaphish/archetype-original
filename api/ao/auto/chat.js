...
import { detectUserApprovalIntent } from '../../../lib/ao/detectUserApprovalIntent.js';
...
async function trySaveDraftFromExchange(userMessage, assistantReply, email, recentHistory = [], options = {}) {
  ...
  // #132: bare "ready"/"yes"/etc. must not approve from long planning docs (e.g. paste-ready).
  const approvalIntent = detectUserApprovalIntent(userMessage);
  const isApproval = !!approvalIntent.isApproval;
  const isRemoval =
...
`.trim();
    ...
    //
    // 2026-09-15: Auto pitched a rerun of "Twenty Points Apart" and only admitted
    // the overlap when Bart asked. Bart: "I want it to go away if it's already been
    // done or has overlap." The save-time check never sees a pitch written straight
    // into chat, so the final reply is checked here, after synthesis and every
    // other pass. A match is discarded and the model is asked for a different
    // angle, checked again. If nothing clear comes back, Bart gets a short plain
    // message naming what is already published, never the repeat. Skipped only
    // when Bart himself cleared the overlap this turn.
    try {
      const { looksLikeNewPiecePitch, findPriorCoverage } = await import('../../../lib/ao/priorCoverage.js');
      ...
      if (!clearedThisTurn && looksLikeNewPiecePitch(fullReply)) {
        ...
        if (matches.length) {
          const { MAX_PITCH_RETRIES, mergeMatches, buildPitchRetryInstruction, overlapExhaustedReply } = await import(
            '../../../lib/ao/pitchRetry.js'
          );
          console.warn('[chat.js] Pitch repeats published work; replacing it:', matches.map((m) => m.slug).join(', '));
          sendEvent('reset', { reset: true });

          let seen = matches;
          let replacement = null;
          const retryClient = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY });
          const retryModel = process.env.AUTO_ANTHROPIC_MODEL || 'claude-sonnet-5';
          const baseMessages =
            Array.isArray(streamResult?.messages) && streamResult.messages.length
              ? streamResult.messages
              : [{ role: 'user', content: userMessage }];

          for (let attempt = 0; attempt < MAX_PITCH_RETRIES; attempt++) {
            if (SOFT_TIMEOUT_MS - (Date.now() - streamStartedAt) < 30_000) break;
            try {
              const res = await retryClient.messages.create({
                model: retryModel,
                max_tokens: 2000,
                system: `${streamResult?.systemPrompt || ''}\n\n${buildPitchRetryInstruction(seen)}`.trim(),
                messages: baseMessages,
              });
              let candidate = (res?.content || [])
                .filter((b) => b?.type === 'text')
                .map((b) => b.text)
                .join('')
                .trim();
              if (!candidate) continue;
              candidate = enforceResponseRules(candidate, recentHistory);
              const voice = await enforceVoiceGuardrails(candidate, {
                anthropicClient: retryClient,
                contextLabel: 'pitch-retry',
              });
              candidate = voice.text;
              const again = await findPriorCoverage({ text: candidate.slice(0, 6000) });
              if (!again.length) {
                replacement = candidate;
                break;
              }
              seen = mergeMatches(seen, again);
            } catch (retryErr) {
              console.warn('[chat.js] pitch retry failed:', retryErr?.message || retryErr);
              break;
            }
          }

          fullReply = replacement || overlapExhaustedReply(seen);
