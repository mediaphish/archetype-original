...
function tokens(s) {
  ...
  return parts.filter((p) => p.length >= 5).slice(0, 350);
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const recentPostedSimilar = await findRecentPostedSimilar({ email: auth.email, queryText: anchorQuery });
...
- If this looks like something posted recently, treat that as a soft warning: suggest a fresh angle instead of repeating yourself.
...
- recent_posted_similar: ${JSON.stringify(recentPostedSimilar || [])}
