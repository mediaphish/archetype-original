 ...
 * ALI Magic Link - Send
 * 
 * Sends a magic link email to the user (works for both contacts and Super Admins)
 * 
 * POST /api/ali/auth/send-magic-link
 * Body: { email: string }
 */
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Check if user exists (either as contact or Super Admin)
    // This check is optional - we'll send the link either way (security best practice: don't reveal if user exists)
    ...
    const magicLink = `${siteUrl}/api/ali/auth/verify-magic-link?token=${token}&email=${encodeURIComponent(emailLower)}`;
    ...
    if (!resendApiKey) {
      ...
      console.log('[MAGIC LINK] Development mode - link:', magicLink);
      ...
      <html>
        ...
        <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
          <div style="text-align: center; margin-bottom: 30px;">
            <h1 style="color: #1a1a1a; font-size: 24px; margin-bottom: 10px;">Sign in to ALI</h1>
    ...
    if (!emailResponse.ok) {
      ...
      console.error('[MAGIC LINK] Email send error:', errorData);
