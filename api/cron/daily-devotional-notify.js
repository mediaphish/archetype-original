...
export default async function handler(req, res) {
  // Vercel Cron Jobs automatically send a specific authorization header
  // For security, we can verify it matches a secret if set
  // Vercel sends: Authorization: Bearer <token>
  // The token is automatically generated by Vercel for cron jobs
  const authHeader = req.headers['authorization'] || req.headers['x-vercel-cron'];
  
  // If CRON_SECRET is set, verify it (for additional security)
  // Otherwise, trust Vercel's automatic authentication
  if (process.env.CRON_SECRET) {
    if (authHeader !== `Bearer ${process.env.CRON_SECRET}`) {
      return res.status(401).json({ error: 'Unauthorized' });
    }
  } else if (!authHeader) {
    // If no auth header at all and no secret set, allow (for testing)
    // In production, Vercel will always send the header
    console.warn('⚠️  No authorization header found - this should only happen in testing');
  }
