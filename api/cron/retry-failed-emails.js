...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (let i = 0; i < failedEmails.length; i++) {
      ...
      // Skip if already at max retries (shouldn't happen due to query, but safety check)
      if (failure.retry_count >= MAX_RETRIES) {
        console.log(`⏭️  Skipping ${failure.email} - already at max retries (${failure.retry_count})`);
        maxedOutCount++;
      ...
      if (!post) {
        console.warn(`⚠️  Post not found for ${failure.post_slug}, marking as failed...`);
        stillFailedCount++;
        errors.push({ 
          email: failure.email, 
        ...
        // Mark as failed since post doesn't exist
        await supabaseAdmin
          .from("journal_email_failures")
          .update({ 
            status: 'failed',
      ...
      // Retry with 3 attempts per cron run
      let retries = 3;
      let sent = false;

      while (retries > 0 && !sent) {
        try {
          console.log(`📨 Retrying email to ${failure.email} for ${slug}... (attempt ${failure.retry_count + 1}/${MAX_RETRIES})`);
            ...
            if (result.error.name === 'rate_limit_exceeded' || result.error.statusCode === 429) {
              ...
              if (retries > 0) {
                ...
                if (shouldMarkAsFailed) {
                  maxedOutCount++;
                  console.error(`❌ Max retries reached for ${failure.email} - marking as permanently failed`);
                } else {
                  console.error(`❌ Still failed after retry: ${failure.email} for ${slug} (${newRetryCount}/${MAX_RETRIES} attempts)`);
                }
              }
            } else {
              // Non-rate-limit error - mark as failed if at max retries, otherwise keep pending
              const newRetryCount = failure.retry_count + 1;
              const shouldMarkAsFailed = newRetryCount >= MAX_RETRIES;
