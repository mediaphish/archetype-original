-- AO Automation — allow `cleared` items in Analyst
-- Run in Supabase SQL editor.
--
-- The app uses `status='cleared'` to flush the Pending inbox without rejecting items.
-- This extends the ao_quote_review_queue status CHECK constraint.
...
BEGIN
  ALTER TABLE public.ao_quote_review_queue
    ADD CONSTRAINT ao_quote_review_queue_status_check
      CHECK (status IN ('pending', 'held', 'approved', 'rejected', 'cleared'));
