...
export default function EditEvent() {
    ...
    const fetchEvent = async () => {
      ...
      try {
        ...
        if (json.ok && json.event) {
          ...
          if (event.state !== 'LIVE') {
            setError('Can only edit LIVE events. This event is already OPEN or CLOSED.');
            setLoading(false);
            return;
          }
          ...
          if (eventDate < today) {
            setError('Can only edit future events.');
            setLoading(false);
            return;
          }
          ...
          setFormData({
            ...
            host_location: event.host_location || '',
            host_location_lat: event.host_location_lat?.toString() || '',
            host_location_lng: event.host_location_lng?.toString() || '',
            host_description: event.host_description || '',
            sponsor_name: event.sponsor_name || '',
            sponsor_logo_url: event.sponsor_logo_url || '',
            sponsor_website: event.sponsor_website || '',
            sponsor_phone: event.sponsor_phone || '',
            sponsor_pot_value: event.sponsor_pot_value ? formatUSD((event.sponsor_pot_value * 100).toString()) : '',
            sponsor_description: event.sponsor_description || ''
          });

          // Set word counts
          if (event.host_description) {
            setHostDescriptionWordCount(event.host_description.trim().split(/\s+/).filter(w => w.length > 0).length);
          }
          if (event.sponsor_description) {
            setSponsorDescriptionWordCount(event.sponsor_description.trim().split(/\s+/).filter(w => w.length > 0).length);
          }
        } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const resp = await fetch(`/api/operators/events/${id}/update`, {
        method: 'PUT',
    ...
    <div className="min-h-screen bg-[#fafafa]">
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        <div className="mb-6">
          ...
          <h1 className="text-3xl font-semibold text-gray-900">Edit Event</h1>
        </div>
