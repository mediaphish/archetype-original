...
export function calculateLeadershipMirror(leaderScores, teamScores) {
  // Accept any keys present in the provided score objects.
  // (Back-compat: callers that pass only ali/alignment/stability/clarity still work.)
  const keys = Array.from(
    new Set([
      ...Object.keys(leaderScores || {}),
  ...
  const severity = {};
  keys.forEach((k) => {
    gaps[k] = calculatePerceptionGap(leaderScores?.[k], teamScores?.[k]);
    severity[k] = classifyGapSeverity(gaps[k]);
  });

  return { gaps, severity, leaderScores, teamScores };
}
