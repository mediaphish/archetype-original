...
export async function postToInstagram(options, accountId) {
  ...
  if (process.env.META_ACCESS_TOKEN && process.env.INSTAGRAM_BUSINESS_ID && (accountId === 'meta' || accountId === 'default' || accountId === 'ig_meta')) {
    return postToInstagramMeta({ text, imageUrl: image });
  }
