...
import { postToInstagramLogin } from './instagramLoginAdapters.js';
...
export async function postToInstagram(options, accountId) {
  ...
  // Instagram API with Instagram Login (no linked Facebook Page required) — check this
  // FIRST. Any account_id with a row in ao_instagram_login_tokens is never reachable via
  // the classic graph.facebook.com Page-linked pipeline below, so it must never fall
  // through to it.
  const loginConn = await getInstagramLoginConnection(accountId);
  if (loginConn?.token && loginConn?.igUserId) {
    return postToInstagramLogin(
      { text, imageUrl: image },
      { token: loginConn.token, igUserId: loginConn.igUserId }
    );
  }
