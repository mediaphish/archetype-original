...
import { getInstagramLoginConnection } from './instagramLoginConnection.js';
import { INSTAGRAM_LOGIN_GRAPH_BASE } from './instagramLoginAdapters.js';
 ...
 * Login-API accounts (no linked Page) use graph.instagram.com first.
 * Page-linked accounts keep the classic graph.facebook.com path.
 * Requires instagram_business_manage_comments (Login) or instagram_manage_comments (Page).
 */
async function firstCommentInstagram(accountId, externalId, text) {
  const loginConn = await getInstagramLoginConnection(accountId);
  if (loginConn?.token) {
    const url = `${INSTAGRAM_LOGIN_GRAPH_BASE}/${externalId}/comments?access_token=${encodeURIComponent(loginConn.token)}`;
    const res = await fetch(url, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ message: text })
    ...
    const data = await res.json().catch(() => ({}));
    if (res.ok && data.id) return { success: true };
    return { success: false, error: data.error?.message || `Instagram Login comments API ${res.status}` };
  }
