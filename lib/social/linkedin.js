...
import { getLinkedinVersionHeaders, getLinkedinVersion } from './linkedinVersion.js';
...
export async function postToLinkedIn(options, accountId = 'personal') {
  ...
  if (imageUrl && imageUrl.trim()) {
    try {
      ...
      if (!initRes.ok) {
        ...
        // Same trace capture as the post call. A card-carrying post fails here
        // before /posts is ever reached, so this is the request support would
        // need to look up, and its uuid is a different one.
        const initUuid = initRes.headers.get('x-li-uuid') || 'none';
        console.error(
          `[LinkedIn] POST /images?action=initializeUpload failed ${initRes.status} ` +
            `x-li-uuid=${initUuid} linkedin-version=${getLinkedinVersion()} owner=${author}: ` +
            errText.slice(0, 200)
        );
        throw new Error(
          `LinkedIn image init failed (${initRes.status}, x-li-uuid=${initUuid}): ${errText.slice(0, 200)}`
        );
      }
      ...
      console.warn('[LinkedIn] Image upload failed; not posting text-only:', msg);
      return { success: false, error: `LinkedIn image attach failed: ${msg}` };
    }
  }
  ...
  // LinkedIn stamps every response with tracing headers. x-li-uuid is the one
  // their support asks for by name — it is how they locate a specific request in
  // their own logs, and without it a support ticket is a description of a
  // problem rather than a thing they can look up. We were reading x-restli-id
  // and discarding the rest, so a failed post left Bart with nothing to hand
  // them. Captured on success too, since a post that succeeds but behaves oddly
  // is exactly as hard to investigate without it.
  const trace = {
    uuid: res.headers.get('x-li-uuid') || null,
    fabric: res.headers.get('x-li-fabric') || null,
    pop: res.headers.get('x-li-pop') || null,
    request_url: `${LINKEDIN_API_BASE}/posts`,
  ...
  if (res.ok && postId) {
    return { success: true, postId, trace };
  }

  let errMsg = `LinkedIn API ${res.status}`;
  let serviceErrorCode = null;
  try {
    const data = await res.json();
    // serviceErrorCode is LinkedIn's specific reason code and is more useful to
    // support than the human-readable message, which is often generic.
    serviceErrorCode = data.serviceErrorCode ?? data.error?.serviceErrorCode ?? null;
  ...
  // Logged as well as returned. The trace headers are only available at the
  // moment of the call; if this is not written down here it is gone, and the
  // next person to ask for it has to reproduce the failure to get it back.
  console.error(
    `[LinkedIn] POST /posts failed ${res.status}` +
      (serviceErrorCode ? ` serviceErrorCode=${serviceErrorCode}` : '') +
      ` x-li-uuid=${trace.uuid || 'none'} x-li-fabric=${trace.fabric || 'none'}` +
      ` linkedin-version=${trace.linkedin_version} author=${author}: ${errMsg}`
  );

  return { success: false, error: errMsg, serviceErrorCode, trace };
}
