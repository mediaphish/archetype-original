...
import { postToFacebookPage } from './metaAdapters.js';
...
export async function postToFacebook(options, accountId) {
  ...
  if (process.env.META_ACCESS_TOKEN && process.env.FACEBOOK_PAGE_ID && (accountId === 'meta' || accountId === 'default' || accountId === 'page_meta')) {
    const url = (imageUrl && imageUrl.trim()) || null;
    if (url) {
      const validation = await validateImageForPublish(url);
      if (!validation.valid) return { success: false, error: validation.error };
    }
    const result = await postToFacebookPage({ url: url || null, caption: text });
