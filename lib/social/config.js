...
export function getSocialCredentials(platform, accountId) {
  ...
  if (platform === 'facebook') {
    // Single Meta app: META_ACCESS_TOKEN + FACEBOOK_PAGE_ID
    const metaToken = process.env.META_ACCESS_TOKEN;
    const metaPageId = process.env.FACEBOOK_PAGE_ID;
    if (metaToken && metaPageId && (accountId === 'meta' || accountId === 'default' || accountId === 'page_meta')) {
      return { accessToken: metaToken, pageId: metaPageId };
    }
    const matchPage = accountId.match(/^page_(.+)$/);
    const matchGroup = accountId.match(/^group_(.+)$/);
    if (matchPage) {
      const id = matchPage[1];
      const token = process.env[`FACEBOOK_PAGE_${id.toUpperCase()}_ACCESS_TOKEN`] ?? process.env[`FACEBOOK_PAGE_${id}_ACCESS_TOKEN`] ?? metaToken;
      const pageId = process.env[`FACEBOOK_PAGE_${id.toUpperCase()}_PAGE_ID`] ?? process.env[`FACEBOOK_PAGE_${id}_PAGE_ID`] ?? (id === 'meta' ? metaPageId : null);
  ...
  if (platform === 'instagram') {
    // Single Meta app: META_ACCESS_TOKEN + INSTAGRAM_BUSINESS_ID
    const metaToken = process.env.META_ACCESS_TOKEN;
    const igBusinessId = process.env.INSTAGRAM_BUSINESS_ID;
    if (metaToken && igBusinessId && (accountId === 'meta' || accountId === 'default' || accountId === 'ig_meta')) {
      return { accessToken: metaToken, igUserId: igBusinessId };
    }
    // ig_1, ig_2 or account id as number
    const match = accountId.match(/^ig_(.+)$/) || (accountId.match(/^\d+$/) ? { 1: accountId } : null);
    if (match) {
      const n = match[1];
      const token = process.env[`INSTAGRAM_ACCOUNT_${n}_ACCESS_TOKEN`] ?? metaToken;
      const igUserId = process.env[`INSTAGRAM_ACCOUNT_${n}_IG_USER_ID`] ?? (n === 'meta' ? igBusinessId : null);
