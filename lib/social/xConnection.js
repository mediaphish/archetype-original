...
export async function getXAccessToken() {
  ...
  if (!needsRefresh) {
    return {
      ...
      // Callers gate image posting on media.write. The column was already being
      // selected and then dropped here, so every scope check saw undefined and
      // reported "missing media.write" while uploads were in fact succeeding.
  ...
  return {
    ...
    username: row.username || null,
    userId: row.user_id || null,
    // Prefer the refreshed grant; fall back to the stored row, since X omits
    // scope from some refresh responses.
