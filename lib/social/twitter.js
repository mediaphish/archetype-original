 ...
 *
 * Exported so api/ao/auto/x-media-check.js can verify the image path against
 * the real API without publishing a tweet — uploading yields a private media
 * id that expires unused.
 */
export async function uploadTwitterMediaOAuth1(creds, base64) {
...
export async function postToTwitter(options, accountId = 'personal') {
  ...
  // If an image was provided, do not fall through to a text-only tweet.
  if (trimmedUrl) {
    if (!creds) {
      return { success: false, error: 'X image attach failed: media upload credentials are missing' };
    }
