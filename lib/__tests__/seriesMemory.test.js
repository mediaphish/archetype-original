...
describe('formatSeriesMemoryForPrompt', () => {
  ...
  it('lists open ideas separately from decided ones', () => {
    const out = formatSeriesMemoryForPrompt(series, ideas);
    expect(out).toContain('Ideas in play');
    expect(out).toContain('Barnabas');
    expect(out).toContain('Ideas already decided');
  ...
  it('renders ideas even when no series is recorded yet', () => {
    const out = formatSeriesMemoryForPrompt([], ideas);
    expect(out).toContain('Barnabas');
...
describe('IDEA_STATUSES', () => {
  it('is the closed set the store validates against', () => {
    expect(IDEA_STATUSES).toEqual(['open', 'chosen', 'rejected']);
  });
});
