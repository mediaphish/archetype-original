...
describe('formatSeriesMemoryForPrompt', () => {
  ...
  it('lists open candidates separately from decided ones', () => {
    const out = formatSeriesMemoryForPrompt(series, candidates);
    expect(out).toContain('Candidate ideas still open');
    expect(out).toContain('Barnabas');
    expect(out).toContain('Candidates already decided');
  ...
  it('instructs against the exact failure this memory exists to stop', () => {
    ...
    // four were already published and ~10 candidates had been discussed.
    const out = formatSeriesMemoryForPrompt(series, candidates);
    expect(out).toContain('brainstorm from a blank page');
    expect(out).toContain('do not silently invent a replacement');
  });

  it('renders candidates even when no series is recorded yet', () => {
    const out = formatSeriesMemoryForPrompt([], candidates);
    expect(out).toContain('Barnabas');
...
describe('CANDIDATE_STATUSES', () => {
  it('is the closed set the store validates against', () => {
    expect(CANDIDATE_STATUSES).toEqual(['open', 'chosen', 'rejected']);
  });
});
