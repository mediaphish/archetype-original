...
import { refreshImageSeriesMemoryTitle } from '../ao/editorialMemory.js';
...
export async function saveDraft({
  ...
  // Title correction must refresh image_series memory immediately (not wait for next image gen).
  const titleChanged =
    kindNorm === 'journal' &&
    priorTitle != null &&
    String(priorTitle).trim() !== titleNorm &&
    series_slug &&
    String(series_slug).trim() &&
    String(series_slug).trim() !== 'standalone';
  if (titleChanged) {
    try {
      await refreshImageSeriesMemoryTitle({
        email: emailNorm,
        partSlug: slugNorm,
        postTitle: titleNorm,
      });
    } catch (err) {
      console.warn('[contentDrafts] refreshImageSeriesMemoryTitle:', err?.message || err);
    }
  }
