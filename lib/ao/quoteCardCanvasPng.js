...
import { stripMarkdownBoldForCardDisplay } from './quoteCardMarkdown.js';
import { buildQuoteCardLineBlocks } from './quoteCardTextLayout.js';
...
function wrapQuoteToLines(ctx, quote, maxWidth, maxLines, fontSize) {
  ...
  const blocks = buildQuoteCardLineBlocks(quote);
  const lines = [];
...
/** Draw image scaled to fit inside maxW×maxH without stretching (centered horizontally, bottom-aligned to bottomY). */
function drawImageContain(ctx, img, centerX, bottomY, maxW, maxH) {
...
export async function renderMinimalQuoteCardPngBuffer({ quote, sourceName, logoUrl } = {}) {
  const q = stripMarkdownBoldForCardDisplay(String(quote || '').trim());
  ...
  if (logoUrl) {
    try {
      ...
      const bottomEdge = H - PAD;
      drawImageContain(ctx, img, W / 2, bottomEdge, LOGO_MAX_W, LOGO_MAX_H);
    } catch (_) {
      ctx.font = `700 44px ${FONT_FAMILY}, sans-serif`;
      ctx.fillStyle = '#f5f5f5';
      ctx.textAlign = 'center';
      ctx.fillText('AO', W / 2, H - PAD - 36);
    }
  } else {
    ctx.font = `700 44px ${FONT_FAMILY}, sans-serif`;
    ctx.fillStyle = '#f5f5f5';
