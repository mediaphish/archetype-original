 ...
 */
/**
 * Numbered rows like "21. Can we write 20 more…?" are instructions to Auto, not card copy.
 * Also filters router-extracted junk that slipped into paste_quote_batch.
 */
export function shouldSkipAsQuoteCardBody(text) {
  const t = String(text || '').trim();
  if (!t) return true;
  const lower = t.toLowerCase();
  // Assistant-directed planning / research questions (often pasted as an extra numbered line).
  if (/^(can we|are you able|could you|would you)\b/i.test(t)) return true;
  if (/^(please|i need you to|help me)\b.*\b(research|find|write \d+|seed)\b/i.test(t)) return true;
  if (/\bseed the system\b/i.test(t)) return true;
  if (/write \d+ more\b/i.test(lower) && /\boverlap\b/i.test(lower)) return true;
  if (/\bresearch\b/i.test(lower) && /\b(find|more)\b/i.test(lower) && /\d+\s+more\b/i.test(lower)) return true;
  // Standalone rejection / meta lines accidentally numbered.
  if (/^i\s+reject\b/i.test(t)) return true;
  return false;
}
...
export function parseUserSuppliedQuoteCards(text) {
  ...
  const flushPair = () => {
    ...
    if (quote && !shouldSkipAsQuoteCardBody(quote)) {
  ...
  for (let line of raw) {
    ...
    if (numbered) {
      ...
      if (!shouldSkipAsQuoteCardBody(body)) {
        cards.push({ quote: body, source_title: '', url: '' });
      }
      continue;
    }
