...
async function handleApproveStage(input, ctx) {
  ...
  return {
    ...
    message:
      stage === 'captions'
        ? `Captions approved and saved on the draft for ${Object.keys(result.approved_captions || {}).length} channel(s): ${Object.keys(result.approved_captions || {}).join(', ')}. They now show as approved in the panel. Nothing was scheduled; the schedule is the next approval.`
        : 'Image approved and recorded on the draft. The panel moves to Captions.',
  };
}
