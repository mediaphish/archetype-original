...
/**
 * Prior published coverage. Currently log-only: it never blocks a save.
 *
 * Blocking was switched off 2026-09-16. Bart's corpus lives in one territory
 * (trust, accountability, standards), so a real resurface scores as close to
 * his older posts as a rerun does, and the block stopped a brief he had
 * approved. Bart: "The point of a resurface is to look at things happening in
 * leadership and find parallels in my existing writing." Overlap with older
 * writing is the job. The narrower rule (what exactly counts as a rerun) is
 * waiting on Bart's decision; until then this records matches in the log only.
 */
async function priorCoverageGate({ text, slug, seriesSlug, excludeSlugs, cleared }) {
  if (cleared) return null;
  const { findPriorCoverage } = await import('./priorCoverage.js');
  const matches = await findPriorCoverage({ text, slug, seriesSlug, excludeSlugs });
  if (matches.length) {
    console.log(
      '[autoToolHandlers] prior coverage (log only, not blocking):',
      slug,
      matches.map((m) => `${m.slug}:${m.similarity}`).join(', ')
    );
  }
  return null;
}
