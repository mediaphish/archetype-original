...
async function handleApproveStage(input, ctx) {
  ...
  if (!bartApprovedInMessage(ctx?.userMessage)) {
    return {
      ok: false,
      gate: 'approval_not_given',
      error:
        `Not recorded. Bart's message this turn does not approve the ${stage || 'step'}. ` +
        'Ask him whether he approves it; record the approval only after he says so.',
    };
  }
