...
async function getExistingImageUrl(email, series_slug, part_number, kind) {
  try {
...
async function handleSaveDraft(input, ctx) {
  ...
  const series_slug =
    canonicalizeSlug(input?.series_slug) || deriveSeriesSlug(slug) || 'standalone';
  const part_number =
    input?.part_number != null && input.part_number !== ''
      ? parseInt(input.part_number, 10) || 1
      : extractPartNumber(slug);

  const preservedImageUrl = await getExistingImageUrl(email, series_slug, part_number, kind);
  const row = {
  ...
  return { ok: true, slug, kind, status, series_slug, part_number, title };
}
...
async function attachImageUrlToDraft({ email, slug, imageUrl, series_slug, part_number }) {
  if (!email || !imageUrl) return { attached: false };
  const emailNorm = email.toLowerCase().trim();

  if (series_slug && part_number != null) {
    const partNum = parseInt(part_number, 10) || 1;
    const { data, error } = await supabaseAdmin
      .from('ao_content_drafts')
      .update({ image_url: imageUrl, updated_at: new Date().toISOString() })
      .eq('created_by_email', emailNorm)
      .eq('series_slug', canonicalizeSlug(series_slug))
      .eq('part_number', partNum)
      .neq('status', 'published')
      .neq('status', 'abandoned')
      .select('slug')
      .limit(1);
    if (!error && data?.length) return { attached: true, slug: data[0].slug };
  }
