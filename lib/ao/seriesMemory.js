/**
 * Cross-thread series and candidate memory for Auto (Phase 2).
 *
 ...
 *
 *   2. Nothing captured exploratory, pre-decision work. The 8–10 candidate
...
// intended name rather than adding a parallel one. 'candidate' had to be added
// to the constraint; nothing existed for pre-decision ideas.
export const SERIES_KIND = 'series_plan';
export const CANDIDATE_KIND = 'candidate';
export const CANDIDATE_STATUSES = ['open', 'chosen', 'rejected'];
export const SERIES_PROMPT_CAP = 12;
export const CANDIDATE_PROMPT_CAP = 40;
...
function candidateSourceKey(seriesKey, name) {
...
function rowToCandidate(row) {
  ...
  return {
    ...
    status: CANDIDATE_STATUSES.includes(extra.status) ? extra.status : 'open',
...
/**
 * Capture an idea that has been discussed but not decided on.
 *
 ...
 */
export async function upsertCandidate({ email, seriesKey = null, name, note = '', status = 'open' }) {
  const emailNorm = normEmail(email);
  ...
  const statusNorm = CANDIDATE_STATUSES.includes(status) ? status : 'open';
    ...
    .upsert(
      {
        created_by_email: emailNorm,
        kind: CANDIDATE_KIND,
        source_url_or_slug: candidateSourceKey(key, nameText),
        series_key: key,
        title: nameText.slice(0, 200),
        body_text: String(note || '').trim(),
        published_at: new Date().toISOString(),
        topic_tags: ['candidate', statusNorm],
        extra: {
  ...
  if (error) return { ok: false, error: error.message };
  return { ok: true, candidate: rowToCandidate(data) };
}

export async function listCandidates(email, { seriesKey = null, status = null, limit = CANDIDATE_PROMPT_CAP } = {}) {
  const emailNorm = normEmail(email);
  if (!emailNorm) return { ok: false, error: 'email is required', candidates: [] };
  ...
  let query = (await db())
    ...
    .limit(Math.max(Number(limit) || CANDIDATE_PROMPT_CAP, 1) * 2);

  if (seriesKey) query = query.eq('series_key', seriesKeyFor(seriesKey));

  const { data, error } = await query;
  if (error) return { ok: false, error: error.message, candidates: [] };

  let rows = (data || []).filter(isActive).map(rowToCandidate).filter(Boolean);
  if (status) rows = rows.filter((c) => c.status === status);
  return { ok: true, candidates: rows.slice(0, limit) };
}

/** Mark a candidate chosen or rejected, so it stops looking undecided. */
export async function resolveCandidate({ email, name, seriesKey = null, status = 'chosen' }) {
  const emailNorm = normEmail(email);
  const nameText = String(name || '').trim();
  if (!emailNorm || !nameText) return { ok: false, error: 'email and name are required' };
  if (!CANDIDATE_STATUSES.includes(status)) return { ok: false, error: `status must be one of ${CANDIDATE_STATUSES.join(', ')}` };

  const listed = await listCandidates(emailNorm, { seriesKey, limit: 200 });
  if (!listed.ok) return listed;

  const needle = nameText.toLowerCase();
  const target =
    listed.candidates.find((c) => c.name.toLowerCase() === needle) ||
    listed.candidates.find((c) => c.name.toLowerCase().includes(needle)) ||
    null;
  if (!target) return { ok: false, error: 'No matching candidate found' };
...
/** Render series + candidates as an always-on system prompt block. */
export function formatSeriesMemoryForPrompt(series = [], candidates = []) {
  ...
  if (open.length) {
    sections.push(
      `### Candidate ideas still open (discussed, not yet decided)\n` +
        open.map((c) => `- ${c.name}${c.series_key ? ` [${c.series_key}]` : ''}${c.note ? ` — ${c.note}` : ''}`).join('\n')
    );
  }
  if (decided.length) {
    sections.push(
      `### Candidates already decided\n` +
  ...
  return (
    ...
    `brainstorm from a blank page when open candidates exist. If something here looks ` +
...
export async function loadSeriesMemoryContext(email) {
  try {
    const [seriesResult, candidateResult] = await Promise.all([
      ...
      listCandidates(email, { limit: CANDIDATE_PROMPT_CAP }),
    ]);
    if (!seriesResult.ok && !candidateResult.ok) return '';
    return formatSeriesMemoryForPrompt(seriesResult.series || [], candidateResult.candidates || []);
  } catch (err) {
