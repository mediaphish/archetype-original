...
// db() above serves ao_editorial_memory_items, which is ungated. ao_content_drafts
// is gated by scripts/verify-no-direct-table-access.mjs, so reads of it go through
// the sanctioned accessor instead.
...
/**
 * Header image of the previous part of whatever series this post belongs to.
 *
 * Exists because the drafts table cannot answer this. Every Archetype entry has
 * series_slug set to its OWN slug with part_number 1 — the-jezebel-archetype,
 * the-ruth-archetype, the-barnabas-archetype and the rest are each recorded as
 * their own one-part series. So the series-continuity branch in
 * resolveGenerateImageReferenceUrls never fires for them, and a new entry would
 * fall through to the approved-card fallback and inherit a LIKENESS card. The
 * Archetype headers are painterly biblical scenes; that would be the wrong
 * reference entirely.
 *
 * Series memory does know the truth — seven published Archetype parts in order
 * — so this resolves the series from there and reads the prior part's image off
 * its draft row.
 *
 * @returns {Promise<{ imageUrl: string, fromSlug: string, seriesKey: string }|null>}
 */
export async function loadPriorPartImageUrl({ email, slug, title = '' } = {}) {
  const emailNorm = normEmail(email);
  const slugText = String(slug || '').trim();
  if (!emailNorm || !slugText) return null;

  try {
    const { matchSeries } = await import('./seriesMatch.js');
    const { series } = await listSeries(emailNorm, { limit: 100 });
    const hit = matchSeries(`${title} ${slugText.replace(/-/g, ' ')}`, series || []);
    if (!hit?.key) return null;

    const found = (series || []).find((s) => s.series_key === hit.key);
    const parts = (found?.parts || []).filter((p) => p.slug && p.slug !== slugText);
    if (!parts.length) return null;

    // Prefer the most recently published part; that is the current look.
    const ordered = [...parts].sort((a, b) => (b.part_number || 0) - (a.part_number || 0));

    const { data, error } = await (await drafts())
      .select('slug, image_url')
      .eq('created_by_email', emailNorm)
      .in('slug', ordered.map((p) => p.slug))
      .not('image_url', 'is', null);

    if (error || !data?.length) return null;

    const bySlug = new Map(data.filter((r) => r.image_url).map((r) => [r.slug, r.image_url]));
    for (const part of ordered) {
      const url = bySlug.get(part.slug);
      if (url) return { imageUrl: String(url).trim(), fromSlug: part.slug, seriesKey: hit.key };
    }
    return null;
  } catch (err) {
    console.warn('[seriesMemory] prior part image lookup failed:', err?.message || err);
    return null;
  }
}
...
/**
 * Record a published post against its series, if it belongs to one.
 *
 * Called from the publish path so the arc maintains itself. Without this the
 * seeded back-catalog decays the moment the next part ships — the same decay
 * that made every previous memory attempt on this project stop being true.
 *
 * Resolution order matters:
 *
 *   1. Match against series already in memory, by the post's title and slug.
 *      This is the only step that handles a series whose parts have different
 *      names — the-judas-archetype and the-ruth-archetype share no prefix, so
 *      slug derivation alone reads each one as its own separate series.
 *   2. Fall back to explicit "-part-N" slugs, which do identify a series
 *      reliably, and create it.
 *   3. Otherwise do nothing. A standalone post is not a series, and inventing
 *      one for every publish would fill memory with noise.
 *
 * Never throws and must never block a publish.
 *
 * @returns {Promise<{ recorded: boolean, seriesKey: string|null, reason: string }>}
 */
export async function recordPublishedPost({
  ...
  try {
    const emailNorm = normEmail(email);
    const slugText = String(slug || '').trim();
    if (!emailNorm || !slugText) return { recorded: false, seriesKey: null, reason: 'missing_email_or_slug' };

    const { matchSeries } = await import('./seriesMatch.js');
    const { series } = await listSeries(emailNorm, { limit: 100 });

    // 1. Known series, matched on the post's own title and slug.
    const hit = matchSeries(`${title} ${slugText.replace(/-/g, ' ')}`, series || []);
    if (hit?.key) {
      const existing = (series || []).find((s) => s.series_key === hit.key);
      const already = existing?.parts?.find((p) => p.slug === slugText);
      const result = await recordSeriesPart({
        email: emailNorm,
        seriesKey: hit.key,
        seriesTitle: hit.title,
        partNumber: already?.part_number ?? partNumberHint ?? null,
        slug: slugText,
      ...
      return {
        recorded: result.ok,
        seriesKey: hit.key,
        reason: result.ok ? 'matched_known_series' : result.error,
      };
    }

    // 2. Explicit part numbering identifies a series not yet in memory.
    const partMatch = slugText.match(/-part-(\d+)/i);
    const hintKey =
      seriesSlugHint && seriesKeyFor(seriesSlugHint) !== seriesKeyFor(slugText)
        ? seriesKeyFor(seriesSlugHint)
        : null;

    if (partMatch || hintKey) {
      const key = hintKey || seriesKeyFor(slugText.replace(/-part-\d+.*$/i, ''));
      if (key && key !== seriesKeyFor(slugText)) {
        const result = await recordSeriesPart({
          email: emailNorm,
          seriesKey: key,
          seriesTitle:
            String(title || key)
              ...
              .trim() || key,
          partNumber: partNumberHint ?? (partMatch ? Number.parseInt(partMatch[1], 10) : null),
          slug: slugText,
          title: title || slugText,
          status: 'published',
          publishedAt,
        ...
        return {
          recorded: result.ok,
          seriesKey: key,
          reason: result.ok ? 'new_series_from_part_slug' : result.error,
        };
      }
    }

    return { recorded: false, seriesKey: null, reason: 'not_part_of_a_series' };
  } catch (err) {
    console.error('[seriesMemory] recordPublishedPost failed:', err?.message || err);
    return { recorded: false, seriesKey: null, reason: err?.message || 'error' };
  }
}
