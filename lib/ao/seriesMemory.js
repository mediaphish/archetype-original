/**
 * Cross-thread series and idea memory for Auto (Phase 2).
 *
 ...
 *
 *   2. Nothing captured exploratory, pre-decision work. The 8–10 idea
...
function ideaSourceKey(seriesKey, name) {
...
function rowToIdea(row) {
  ...
  return {
    ...
    status: IDEA_STATUSES.includes(extra.status) ? extra.status : 'open',
 ...
 */
export async function upsertIdea({ email, seriesKey = null, name, note = '', status = 'open' }) {
  const emailNorm = normEmail(email);
  ...
  const statusNorm = IDEA_STATUSES.includes(status) ? status : 'open';
    ...
    .upsert(
      {
        created_by_email: emailNorm,
        kind: IDEA_KIND,
        source_url_or_slug: ideaSourceKey(key, nameText),
        series_key: key,
        title: nameText.slice(0, 200),
        body_text: String(note || '').trim(),
        published_at: new Date().toISOString(),
        topic_tags: ['idea', statusNorm],
        extra: {
  ...
  if (error) return { ok: false, error: error.message };
  return { ok: true, idea: rowToIdea(data) };
}

export async function listIdeas(email, { seriesKey = null, status = null, limit = IDEA_PROMPT_CAP } = {}) {
  const emailNorm = normEmail(email);
  if (!emailNorm) return { ok: false, error: 'email is required', ideas: [] };
  ...
  let query = (await db())
    ...
    .limit(Math.max(Number(limit) || IDEA_PROMPT_CAP, 1) * 2);

  if (seriesKey) query = query.eq('series_key', seriesKeyFor(seriesKey));

  const { data, error } = await query;
  if (error) return { ok: false, error: error.message, ideas: [] };

  let rows = (data || []).filter(isActive).map(rowToIdea).filter(Boolean);
  if (status) rows = rows.filter((c) => c.status === status);
  return { ok: true, ideas: rows.slice(0, limit) };
}

/** Mark a idea chosen or rejected, so it stops looking undecided. */
export async function resolveIdea({ email, name, seriesKey = null, status = 'chosen' }) {
  const emailNorm = normEmail(email);
  const nameText = String(name || '').trim();
  if (!emailNorm || !nameText) return { ok: false, error: 'email and name are required' };
  if (!IDEA_STATUSES.includes(status)) return { ok: false, error: `status must be one of ${IDEA_STATUSES.join(', ')}` };

  const listed = await listIdeas(emailNorm, { seriesKey, limit: 200 });
  if (!listed.ok) return listed;

  const needle = nameText.toLowerCase();
  const target =
    listed.ideas.find((c) => c.name.toLowerCase() === needle) ||
    listed.ideas.find((c) => c.name.toLowerCase().includes(needle)) ||
    null;
  if (!target) return { ok: false, error: 'No matching idea found' };
...
/** Render series + ideas as an always-on system prompt block. */
export function formatSeriesMemoryForPrompt(series = [], ideas = []) {
  ...
  return (
    ...
    `brainstorm from a blank page when open ideas exist. If something here looks ` +
...
export async function loadSeriesMemoryContext(email) {
  try {
    const [seriesResult, candidateResult] = await Promise.all([
      ...
      listIdeas(email, { limit: IDEA_PROMPT_CAP }),
    ]);
    if (!seriesResult.ok && !candidateResult.ok) return '';
    return formatSeriesMemoryForPrompt(seriesResult.series || [], candidateResult.ideas || []);
  } catch (err) {
