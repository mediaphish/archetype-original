/**
 * gateActionClaims.js
 *
 * Blocks Auto from telling Bart an action already happened unless evidence backs
 * the claim. Destructive regenerate-and-replace is intentionally NOT used here —
 * chat.js annotates the streamed reply instead (keeps real content visible).
 *
 * Evidence scopes:
 * - turn: successful tool result (or legacy signal) from this turn only
 *   (approve / publish stay here — higher stakes)
 * - thread: this turn OR earlier successful tool results persisted on the thread
 *   (factual save-status reports like "is saved in drafts")
 */

/**
 * @typedef {{
 *   id: string,
 *   label: string,
 *   tools: string[],
 *   legacySignalTests: RegExp[],
 *   claimPatterns: RegExp[],
 *   evidenceScope?: 'turn' | 'thread',
 *   evidenceFromResults?: (toolResults: Array<{ name?: string, result?: object }>) => boolean,
 * }} ActionClaimRule
 */

/**
 * Tools that read real state.
 *
 * A status report ("header image is attached", "the draft is approved",
 * "publish is locked for Wednesday") is not a claim that Auto just did
 * something. When one of these tools succeeded this turn, the report is what the
 * tool returned, and flagging it teaches Bart to ignore the warning. 2026-09-15:
 * Auto opened with "Confirming current schedule state", read it, reported it,
 * and got "claimed completed action(s)... (publish, generate/attach image)".
 *
 * A read never backs a first-person claim ("I've published", "I attached"):
 * reading state cannot be the evidence that Auto performed an action.
 */
const STATE_READ_TOOLS = ['get_publish_state', 'query_state', 'get_schedule_status'];

/** @type {ActionClaimRule[]} */
export const ACTION_CLAIM_RULES = [
  {
    id: 'save_status',
    label: 'save status (already confirmed)',
    // present_outline saves the draft row too. 2026-09-15: "status: draft" was
    // flagged on a draft present_outline had saved one turn earlier.
    tools: ['save_draft', 'present_outline'],
    readTools: STATE_READ_TOOLS,
    evidenceScope: 'thread',
    legacySignalTests: [/\[JOURNAL_CONTENT\]/i, /\[DEVOTIONAL_CONTENT\]/i],
    claimPatterns: [
      /\bis saved(?:\s+in\s+drafts?)?\b/i,
      /\bsaved(?:\s+as\s+(?:a\s+)?draft)\b/i,
      /\bdraft\s+(?:has\s+been\s+|is\s+|was\s+)?saved\b/i,
      /\bstatus:\s*draft\b/i,
    ],
  },
  {
    id: 'save_or_rebuild',
    label: 'save/rebuild draft (this turn)',
    tools: ['save_draft', 'present_outline'],
    evidenceScope: 'turn',
    legacySignalTests: [/\[JOURNAL_CONTENT\]/i, /\[DEVOTIONAL_CONTENT\]/i],
    claimPatterns: [
      /\bi(?:['’]ve| have)\s+(?:now\s+)?saved\b/i,
      /\brebuilt\b/i,
      /\brebuilding\s+the\s+draft\s+now\b/i,
      /\bbuilding\s+the\s+draft\s+now\b/i,
      /\bstructure\s+now\s+matches\b/i,
      /\b(?:the\s+)?draft\s+(?:has\s+been\s+|was\s+)?rebuilt\b/i,
      /\bsaved\s+for\s+real\b/i,
      /\bconfirmed\s+and\s+saved\b/i,
      /\blocked\s+in\s+as\s+approved\b/i,
      /\ball\s+\d+\s+(?:caption\s+)?channels?\s+(?:are\s+)?(?:locked\s+in|approved|saved)\b/i,
    ],
  },
  {
    id: 'present_outline',
    label: 'present outline / brief',
    tools: ['present_outline'],
    evidenceScope: 'turn',
    legacySignalTests: [],
    claimPatterns: [
      /\bi(?:['’]ve| have)\s+(?:presented|saved|locked)\s+(?:the\s+)?(?:outline|brief)\b/i,
      /\boutline\s+(?:has\s+been\s+|is\s+|was\s+)?(?:presented|saved|locked)\b/i,
    ],
  },
  {
    id: 'approve',
    label: 'approve draft',
    tools: ['approve_draft'],
    readTools: STATE_READ_TOOLS,
    evidenceScope: 'turn',
    legacySignalTests: [],
    claimPatterns: [
      /\bi(?:['’]ve| have)\s+approved\b/i,
      /\bdraft\s+(?:has\s+been\s+|is\s+|was\s+)?approved\b/i,
      /\bmarked\s+(?:it\s+|the\s+draft\s+)?as\s+approved\b/i,
    ],
    // Also backed when another successful tool this turn returns draft.status approved
    // (e.g. schedule_journal_publish / publish_journal payload).
    evidenceFromResults: (toolResults) =>
      (toolResults || []).some(
        (r) =>
          r?.result?.ok &&
          (r.result?.draft?.status === 'approved' || r.result?.status === 'approved')
      ),
  },
  {
    id: 'publish',
    label: 'publish',
    tools: ['publish_journal'],
    readTools: STATE_READ_TOOLS,
    evidenceScope: 'turn',
    legacySignalTests: [/\[PUBLISH_JOURNAL/i, /\[PUBLISH_DEVOTIONAL/i],
    // The bare word "published" used to match on its own, so every mention of
    // an existing post ("Twenty Points Apart", published 8/28/26) was flagged
    // as a claim that Auto had just published something (2026-09-16, three
    // false warnings in one session). A claim needs a subject doing it now.
    claimPatterns: [
      /\bi(?:['’]ve| have| just)\s+(?:just\s+)?published\b/i,
      /\b(?:it|the\s+(?:post|draft|piece|journal(?:\s+entry)?))\s+(?:has\s+(?:now\s+)?been|is\s+now|was\s+just)\s+published\b/i,
      /\bis\s+now\s+live\s+on\s+(?:the\s+)?site\b/i,
    ],
  },
  {
    id: 'schedule',
    label: 'schedule captions/posts',
    tools: ['schedule_captions', 'get_schedule_status'],
    evidenceScope: 'turn',
    legacySignalTests: [/\[SOCIAL_CAPTIONS\]/i, /\[PUBLISH_JOURNAL/i],
    claimPatterns: [
      /\b(?:has\s+been\s+|were\s+|was\s+)?scheduled\b/i,
      /\bi(?:['’]ve| have)\s+scheduled\b/i,
      /\bcaptions?\s+(?:are|were)\s+scheduled\b/i,
      /\bschedule\s+times?\b(?!\s*[?:])/i,
      /\btimes?,?\s+grounded\s+in\s+your\s+real\s+engagement\b/i,
    ],
  },
  {
    id: 'schedule_journal_publish',
    label: 'schedule journal auto-publish date',
    tools: ['schedule_journal_publish', 'get_schedule_status'],
    readTools: STATE_READ_TOOLS,
    evidenceScope: 'turn',
    legacySignalTests: [],
    claimPatterns: [
      /\bscheduled\s+(?:to\s+)?publish\b/i,
      /\bpublish\s+(?:is\s+)?(?:locked\s+in|scheduled)\s+for\b/i,
      /\bauto-?publish(?:es|ing)?\s+on\b/i,
      /\bscheduled_publish_at\b/i,
    ],
  },
  {
    id: 'recommended_schedule',
    label: 'recommended peak / best posting times',
    tools: ['get_recommended_schedule'],
    evidenceScope: 'turn',
    legacySignalTests: [],
    claimPatterns: [
      /\bbest\s+(?:time|times|schedule)\b/i,
      /\bpeak\s+(?:time|times|engagement)\b/i,
      /\boptimal\s+(?:time|times|schedule)\b/i,
      /\bbased\s+on\s+your\s+last\s+\d+\s+posts?\b/i,
    ],
  },
  {
    id: 'caption_length_performance',
    label: 'caption length vs engagement',
    tools: ['get_caption_length_performance'],
    evidenceScope: 'turn',
    legacySignalTests: [],
    claimPatterns: [
      /\b(?:longer|shorter)\s+captions?\s+(?:perform|work|do better|get more|win)\b/i,
      /\b(?:do|does|whether)\s+longer\s+(?:instagram\s+)?captions?\b/i,
      /\bengagement\s+by\s+caption\s+length\b/i,
    ],
  },
  {
    id: 'performance_effectiveness',
    label: 'performance / effectiveness claim',
    tools: ['get_caption_length_performance', 'get_recommended_schedule'],
    evidenceScope: 'turn',
    legacySignalTests: [],
    claimPatterns: [
      /\bwill\s+(?:this|that|it)\s+(?:caption|post|format|tactic)?.{0,40}\bperform\b/i,
      /\bdoes\s+(?:posting|this|that).{0,40}\bhelp\s+engagement\b/i,
      /\bshould\s+we\s+.{0,50}\b(?:for reach|for growth|for engagement)\b/i,
    ],
  },
  {
    id: 'list_stated_preferences',
    label: 'list standing preferences',
    tools: ['list_stated_preferences'],
    evidenceScope: 'turn',
    legacySignalTests: [],
    claimPatterns: [
      /\b(?:here\s+(?:are|is)|these\s+are)\s+(?:your\s+)?(?:standing\s+)?preferences?\b/i,
      /\bi\s+(?:currently\s+)?remember\s+(?:these|the\s+following)\b/i,
      /\bstored\s+(?:standing\s+)?preferences?\b/i,
    ],
  },
  {
    id: 'forget_stated_preference',
    label: 'forget / retract preference',
    tools: ['forget_stated_preference'],
    evidenceScope: 'turn',
    legacySignalTests: [],
    claimPatterns: [
      /\bi(?:['’]ve| have)\s+(?:forgotten|removed|retracted)\s+(?:that\s+)?preference\b/i,
      /\bpreference\s+(?:has\s+been\s+|was\s+)?(?:forgotten|removed|retracted|superseded)\b/i,
    ],
  },
  {
    id: 'generate_image',
    label: 'generate/attach image',
    tools: ['generate_image'],
    readTools: STATE_READ_TOOLS,
    evidenceScope: 'turn',
    legacySignalTests: [/\[DALLE_GENERATE/i, /\[IMAGE_GENERATED/i],
    claimPatterns: [
      /\bi(?:['’]ve| have)\s+(?:generated|attached|uploaded)\s+(?:the\s+)?(?:image|header|graphic)\b/i,
      /\b(?:image|header|graphic)\s+(?:has\s+been\s+|was\s+|is\s+)?(?:generated|attached|uploaded)\b/i,
      /\battached\s+(?:as\s+)?(?:the\s+)?header\b/i,
    ],
    // Also backed when another successful tool this turn returns a real image_url
    // (e.g. schedule_journal_publish draft.image_url, publish_journal image_url).
    evidenceFromResults: (toolResults) =>
      (toolResults || []).some(
        (r) => r?.result?.ok && !!(r.result?.draft?.image_url || r.result?.image_url)
      ),
  },
  {
    // Auto saying Bart's approval of the image or captions was recorded. Before
    // approve_stage existed there was no way to record it at all.
    id: 'stage_approval',
    label: 'record image/captions approval',
    tools: ['approve_stage'],
    readTools: STATE_READ_TOOLS,
    evidenceScope: 'turn',
    legacySignalTests: [],
    claimPatterns: [
      /\bi(?:['’]ve| have)\s+(?:recorded|saved|logged)\s+(?:the\s+|your\s+)?approval\b/i,
      /\b(?:image|captions?)\s+(?:is|are|has\s+been|have\s+been)\s+(?:now\s+)?(?:approved|recorded\s+as\s+approved)\b/i,
      /\bapproval\s+(?:is|has\s+been|was)\s+(?:recorded|saved)\b/i,
      // Inventing the tool's answer is the same fabrication as inventing its
      // success. 2026-09-20: "the system that records image approval checked
      // your message just now and did not register it as an approval — twice",
      // said in turns where the tool was never called. Auto admitted it later.
      /\bthe\s+(?:system|tool|server|check)\b[^.!?\n]{0,60}\b(?:did not|didn['’]?t|does not|doesn['’]?t)\s+(?:register|record|accept|take)\b/i,
      /\b(?:came|come)\s+back\s+not\s+recorded\b/i,
      /\bthe\s+(?:call|tool)\s+(?:failed|declined|refused)\b/i,
    ],
  },
  {
    id: 'quote_card',
    label: 'generate quote card',
    tools: ['generate_quote_card'],
    evidenceScope: 'turn',
    legacySignalTests: [/\[CARD[\s\S]*?\[\/CARD\]/i],
    claimPatterns: [
      /\bi(?:['’]ve| have)\s+(?:generated|built|created)\s+(?:the\s+)?(?:quote\s+)?cards?\b/i,
      /\b(?:quote\s+)?cards?\s+(?:have\s+been\s+|were\s+|was\s+)?(?:generated|built|created)\b/i,
    ],
  },
  {
    id: 'reshare',
    label: 'trigger reshare',
    tools: ['trigger_reshare'],
    evidenceScope: 'turn',
    legacySignalTests: [/\[TRIGGER_RESHARE\]/i, /\[RESHARE_RESULT\]/i],
    claimPatterns: [
      /\bi(?:['’]ve| have)\s+(?:run|triggered|started)\s+(?:the\s+)?reshare\b/i,
      /\breshare\s+(?:has\s+been\s+|was\s+|is\s+)?(?:run|triggered|complete|done)\b/i,
    ],
  },
  {
    id: 'fetch',
    label: 'fetch full text',
    tools: ['fetch_full_text'],
    evidenceScope: 'turn',
    legacySignalTests: [/\[CORPUS_FETCH_FULL_TEXT/i, /\[DRAFT_FETCH_FULL_TEXT/i],
    claimPatterns: [
      /\bi(?:['’]ve| have)\s+(?:fetched|loaded|pulled)\s+(?:the\s+)?(?:full\s+)?(?:text|post|part)\b/i,
      /\b(?:full\s+)?(?:text|post)\s+(?:has\s+been\s+|was\s+)?(?:fetched|loaded|pulled)\b/i,
    ],
  },
  {
    // Catches Auto asserting an external fact/citation as researched when no
    // web_search call actually happened this turn. Mirrors the 2026-08-18
    // Jezebel/Kelo v. New London incident: "I found a strong, well-documented
    // one" was stated as fact with zero web_search call behind it, and no
    // existing rule caught it because all prior rules only cover save/publish/
    // schedule-type claims, never research/citation claims. Per the system
    // prompt's own rule (autoV2.js): "A citation with no corresponding
    // web_search call behind it is a fabricated citation, even if it sounds
    // plausible."
    id: 'research_citation',
    label: 'external fact/citation researched (this turn)',
    tools: ['web_search'],
    evidenceScope: 'turn',
    legacySignalTests: [],
    claimPatterns: [
      /\bi\s+found\s+a\s+(?:strong|real|well-documented|solid|great|good)\b/i,
      /\bi(?:['’]ve| have)\s+(?:verified|confirmed|researched|checked)\s+(?:the|this|that)\b/i,
      /\b(?:verified|confirmed)\s+(?:kelo\s+)?facts?\b/i,
      /\bwell[- ]documented\s+(?:one|case|example|precedent|parallel)\b/i,
      /\baccording\s+to\s+(?:current\s+)?research\b/i,
      /\breal,?\s+named\s+sources?\b/i,
      /\bgrounded\s+in\s+current\s+research\b/i,
      /\bthis\s+is\s+(?:correct|accurate),?\s+the\s+widely\s+corroborated\s+figure\b/i,

      // Added 2026-08-21, after the Kelo claim recurred. The rules above were
      // written from one transcript and match that transcript's exact wording;
      // they miss the plainest phrasing of the same claim, which is how Auto
      // opened its reply the second time: "I did the research now".
      //
      // Asserting research is the claim, whatever verb carries it.
      /\bi\s+(?:just\s+)?did\s+(?:the|some|my)\s+research\b/i,
      /\bi\s+(?:just\s+)?(?:researched|fact-?checked)\b/i,
      /\bi\s+(?:just\s+)?(?:looked|searched)\s+(?:it|this|that|them)\s+up\b/i,
      /\bi\s+(?:just\s+)?(?:ran|did)\s+(?:a|the)\s+(?:search|web\s+search|lookup)\b/i,
      // Requires a research-shaped noun so "I found a way to shorten it" does
      // not trip the gate.
      /\bi\s+found\s+(?:a|an|one|some|two|three)\b[^.!?]{0,60}\b(?:cases?|examples?|parallels?|precedents?|sources?|study|studies|articles?|reports?|interviews?|quotes?|rulings?|statistics?)\b/i,
      /\b(?:verified|confirmed|researched)\s+(?:facts?|details?|sources?|figures?)\b/i,
    ],
  },
];

const FUTURE_INTENT =
  /\b(i(?:['’]ll| will)|let me|going to|about to|i(?:['’]m| am) going to|next i(?:['’]ll| will)|i plan to|i(?:['’]ll| will) now|want me to|should i|ready to|shall i|do you want me to|would you like me to|say the word)\b/i;

const NEGATED_CLAIM =
  /\b(not|never|nothing|none|isn['’]t|hasn['’]t|haven['’]t|didn['’]t|wasn['’]t|won['’]t|no)\b[\s\S]{0,40}$/i;

/**
 * Successful tool names from this turn’s toolResults.
 * @param {Array<{ name?: string, result?: { ok?: boolean } }>} toolResults
 * @returns {Set<string>}
 */
export function successfulToolNames(toolResults = []) {
  const set = new Set();
  for (const row of toolResults || []) {
    if (row?.name && row?.result?.ok) set.add(row.name);
  }
  return set;
}

/**
 * Collect successful tool results persisted on earlier assistant messages in this thread.
 * @param {Array<{ role?: string, meta?: object }>} messages
 * @returns {Array<{ name?: string, result?: { ok?: boolean } }>}
 */
export function collectPriorSuccessfulToolResults(messages = []) {
  const out = [];
  for (const m of messages || []) {
    if (String(m?.role || '') !== 'assistant') continue;
    const meta = m?.meta && typeof m.meta === 'object' ? m.meta : null;
    if (!meta) continue;
    const rows = meta.tool_results || meta.toolResults;
    if (!Array.isArray(rows)) continue;
    for (const row of rows) {
      if (row?.name && row?.result?.ok) out.push(row);
    }
    // Older / compact meta: tools_used list without full results
    if ((!rows || rows.length === 0) && Array.isArray(meta.tools_used)) {
      for (const name of meta.tools_used) {
        if (name) out.push({ name, result: { ok: true } });
      }
    }
  }
  return out;
}

function isFutureIntentAround(reply, index) {
  const start = Math.max(0, index - 80);
  const window = reply.slice(start, index + 20);
  return FUTURE_INTENT.test(window);
}

function isNegatedAround(reply, index) {
  const start = Math.max(0, index - 28);
  const before = reply.slice(start, index);
  return NEGATED_CLAIM.test(before);
}

/**
 * Skip matches that fall inside backticks or a long hyphen-joined slug/token,
 * not natural prose (e.g. do-we-have-...-published-...-hazing).
 * Conservative: normal sentences with spaces around the word are unaffected.
 */
export function isInsideCodeOrSlugSpan(reply, index, matchLength) {
  const text = String(reply || '');
  const idx = Math.max(0, Number(index) || 0);
  const len = Math.max(0, Number(matchLength) || 0);

  const before = text.slice(0, idx);
  const backtickCount = (before.match(/`/g) || []).length;
  if (backtickCount % 2 === 1) return true;

  const charBefore = idx > 0 ? text[idx - 1] : '';
  const charAfter = text[idx + len] || '';
  const flankedByHyphen = charBefore === '-' || charAfter === '-';
  if (!flankedByHyphen) return false;

  const windowStart = Math.max(0, idx - 40);
  const windowEnd = Math.min(text.length, idx + len + 40);
  const window = text.slice(windowStart, windowEnd);
  const hyphenRunMatch = window.match(/[a-z0-9]+(?:-[a-z0-9]+){4,}/i);
  return !!hyphenRunMatch;
}

/**
 * @param {string} reply
 * @param {{
 *   toolResults?: Array<{ name?: string, result?: { ok?: boolean } }>,
 *   priorToolResults?: Array<{ name?: string, result?: { ok?: boolean } }>,
 *   toolsUsed?: string[],
 * }} evidence
 * @returns {{
 *   unbacked: Array<{ ruleId: string, label: string, matchedText: string, requiredTools: string[], evidenceScope: string }>,
 *   backedRuleIds: string[],
 * }}
 */
export function findUnbackedActionClaims(reply, evidence = {}) {
  const text = String(reply || '');
  if (!text.trim()) return { unbacked: [], backedRuleIds: [] };

  const turnOk = successfulToolNames(evidence.toolResults);
  // Always union toolsUsed (Anthropic server tools like web_search never appear
  // in client toolResults). Do not wait for toolResults to be empty.
  if (Array.isArray(evidence.toolsUsed)) {
    for (const name of evidence.toolsUsed) {
      if (name) turnOk.add(name);
    }
  }

  const threadOk = new Set(turnOk);
  for (const name of successfulToolNames(evidence.priorToolResults)) {
    threadOk.add(name);
  }

  const unbacked = [];
  const backedRuleIds = [];

  for (const rule of ACTION_CLAIM_RULES) {
    let matchedText = null;
    let matchIndex = 0;
    for (const pattern of rule.claimPatterns) {
      pattern.lastIndex = 0;
      const m = pattern.exec(text);
      if (!m) continue;
      if (isFutureIntentAround(text, m.index)) continue;
      if (isNegatedAround(text, m.index)) continue;
      if (isInsideCodeOrSlugSpan(text, m.index, m[0].length)) continue;
      matchedText = m[0];
      matchIndex = m.index;
      break;
    }
    if (!matchedText) continue;

    const scope = rule.evidenceScope === 'thread' ? 'thread' : 'turn';
    const okSet = scope === 'thread' ? threadOk : turnOk;
    const toolBacked = rule.tools.some((t) => okSet.has(t));
    const legacyBacked = rule.legacySignalTests.some((re) => re.test(text));
    // Field evidence is turn-scoped only (same-turn tool payloads), even for thread rules.
    const fieldBacked =
      typeof rule.evidenceFromResults === 'function' &&
      !!rule.evidenceFromResults(evidence.toolResults || []);

    // A successful state read this turn backs a status report, never a
    // first-person claim of having done the action.
    // Some patterns match only the verb ("published"), so "I have" sits just
    // before the match rather than inside it.
    const firstPerson =
      /^\s*i\b|^\s*i['’]/i.test(matchedText) ||
      /\bi(?:['’]ve|\s+have|\s+had)?\s+(?:just\s+|already\s+|now\s+)?$/i.test(
        text.slice(Math.max(0, matchIndex - 30), matchIndex)
      );
    const readBacked =
      !firstPerson && Array.isArray(rule.readTools) && rule.readTools.some((t) => turnOk.has(t));

    if (toolBacked || legacyBacked || fieldBacked || readBacked) {
      backedRuleIds.push(rule.id);
      continue;
    }

    unbacked.push({
      ruleId: rule.id,
      label: rule.label,
      matchedText,
      requiredTools: [...rule.tools],
      evidenceScope: scope,
    });
    try {
      const idx = text.toLowerCase().indexOf(String(matchedText).toLowerCase());
      const start = Math.max(0, (idx >= 0 ? idx : 0) - 60);
      const end = Math.min(
        text.length,
        (idx >= 0 ? idx : 0) + String(matchedText).length + 60
      );
      console.warn('[gateActionClaims] fired', {
        ruleId: rule.id,
        matchedText,
        context: text.slice(start, end),
      });
    } catch (_) {
      /* diagnostic only */
    }
  }

  return { unbacked, backedRuleIds };
}

/**
 * Internal correction note (kept for logging / tests). Not used to regenerate replies.
 */
export function buildUnbackedClaimCorrectionNote(unbacked) {
  const lines = (unbacked || []).map((u) => {
    const tools = (u.requiredTools || []).join(' or ');
    const scope =
      u.evidenceScope === 'thread'
        ? 'this thread'
        : 'this turn';
    return `- Claimed "${u.matchedText}" (${u.label}) but no successful ${tools} tool call (and no legacy action signal) was found for ${scope}.`;
  });

  return (
    `SYSTEM CORRECTION — ACTION CLAIM GATE:\n` +
    `You claimed to have completed backend action(s) without a real successful tool result:\n` +
    `${lines.join('\n')}\n\n` +
    `Do one of the following now:\n` +
    `1) Make the real tool call(s) with correct arguments and only then describe the outcome, OR\n` +
    `2) Rephrase as a plan / next step in future tense — do not state the action as already done.\n` +
    `Do not repeat false completion language.`
  );
}

/**
 * User-visible warning appended under the original streamed reply (never replaces it).
 */
export function buildVisibleUnbackedClaimWarning(unbacked) {
  const labels = (unbacked || []).map((u) => u.label).filter(Boolean);
  const unique = [...new Set(labels)];
  return (
    `---\n**⚠️ SYSTEM WARNING:** This reply claimed completed action(s) without a confirmed successful tool result (${unique.join(
      ', '
    ) || 'unknown'}). Treat those claims as unverified — the content above was kept as streamed; nothing above this line was rewritten by the server.`
  );
}

/**
 * Annotate a reply in place. Never replaces substantive content.
 * @returns {{ reply: string, appendedNote: string|null, unbacked: array }}
 */
export function annotateUnbackedActionClaims(reply, evidence = {}) {
  const original = String(reply || '');
  const { unbacked, backedRuleIds } = findUnbackedActionClaims(original, evidence);
  if (!unbacked.length) {
    return { reply: original, appendedNote: null, unbacked: [], backedRuleIds };
  }
  const appendedNote = buildVisibleUnbackedClaimWarning(unbacked);
  return {
    reply: `${original.trim()}\n\n${appendedNote}`.trim(),
    appendedNote,
    unbacked,
    backedRuleIds,
  };
}

export function hasUnbackedActionClaims(reply, evidence = {}) {
  return findUnbackedActionClaims(reply, evidence).unbacked.length > 0;
}
