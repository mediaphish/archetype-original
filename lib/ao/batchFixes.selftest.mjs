...
// --- #122 caption quality heuristics ---
{
  ...
  assert.ok(thin.failures[0].reasons.some((r) => /stub/i.test(r)));

  // A missing CTA is a warning now, not a block. Measured 2026-08-21 against
  // Bart's 303 published captions: 1% of Instagram and at most 19% anywhere
  // carry a question, and he closes on a declarative line. Blocking on it
  // rejected three of the four captions in a real published set.
  const noCtaText =
  ...
  const noCta = evaluateCaptionQualityGate({ linkedin_personal: noCtaText });
  assert.strictEqual(noCta.ok, true, noCta.error);
  assert.ok(noCta.warnings.some((w) => /question or CTA/i.test(w.reason)));

  // The previous "solid" fixture was one sentence repeated eight times — copy
  // written to clear a length floor. That is the padding the self-repetition
  // check now rejects, so it is no longer a valid example of good copy.
  const padded = `${'This post argues that standards hold when leaders stay present. '.repeat(8)}`;
  const paddedResult = evaluateCaptionQualityGate({ linkedin_personal: padded });
  assert.strictEqual(paddedResult.ok, false);
  assert.ok(paddedResult.failures[0].reasons.some((r) => /repeats itself/i.test(r)));

  const solid =
    'Burns drew a line most organizations still cross without noticing. Leadership is not ' +
    'getting people to do things; it is transforming the people doing them. What outlasts you ' +
    'is who they became while the work got done.';
