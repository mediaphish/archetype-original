...
const MODEL = process.env.AUTO_ANTHROPIC_MODEL || 'claude-opus-5';
 ...
 *   post_recording?: { post_recording_surprise?: string, post_recording_follow_up?: string, post_recording_landed?: string } | null,
 * }} input
 */
export async function mapEpisodeCorpusConnections(input) {
  ...
  const postNotes = input?.post_recording || null;
  const corpusBlock = await buildCorpusIndexBlock(transcript, researchBrief);

  let postRecordingBlock = '';
  if (
    postNotes &&
    (postNotes.post_recording_surprise ||
      postNotes.post_recording_follow_up ||
      postNotes.post_recording_landed)
  ) {
    postRecordingBlock = `

Bart's post-recording notes:
- What surprised him: ${postNotes.post_recording_surprise || '(none)'}
- What he wants to explore further: ${postNotes.post_recording_follow_up || '(none)'}
- What landed unexpectedly: ${postNotes.post_recording_landed || '(none)'}

Use these notes to weight your corpus connections and thematic threads. What Bart found surprising or significant is more likely to be worth surfacing than moments he didn't flag.`;
  }
