/**
 * voiceGuardrails.js
 *
 * Code-level enforcement of Bart's voice / anti-AI-signature rules.
 * Soft instructions in system prompts were not holding under generation pressure —
 * same failure mode that led to enforceResponseRules.js. Moving the list here makes
 * it guaranteed on finalized text (saved thread messages, captions, drafts).
 *
 * Flow:
 * 1. detectVoiceViolations(text) — pure scan
 * 2. enforceVoiceGuardrails(text) — self-correct via a cheap rewrite pass (up to 2x),
 *    then forced literal strip as last resort. Never throws; on API failure returns
 *    the original text unmodified.
 *
 * Streaming note: main chat still streams tokens live before this runs (same pattern
 * as enforceResponseRules). Violations may flicker in the stream; they must not survive
 * into what is persisted or published.
 *
 * VOICE_VIOLATIONS is meant to be edited directly as new tics are noticed — same spirit
 * as KNOWN_REAL_SIGNALS in enforceResponseRules.js.
 */

/** @typedef {{ id: string, label: string, pattern: RegExp, kind: 'phrase' | 'structural', soft?: boolean }} VoiceViolationDef */
/** @typedef {{ id: string, label: string, matchedText: string, index: number, soft?: boolean }} VoiceViolationHit */

/**
 * Edit this list when Bart catches a new AI signature tic.
 * `soft: true` = still rewrite, but do NOT hard-strip on forced-fallback
 * (used for words that are sometimes literal, e.g. travel "journey").
 *
 * @type {VoiceViolationDef[]}
 */
export const VOICE_VIOLATIONS = [
  // --- Sitting / pausing family (the tic that triggered this fix) ---
  { id: 'worth-sitting-with', label: 'worth sitting with', pattern: /\bworth\s+sitting\s+with\b/gi, kind: 'phrase' },
  { id: 'worth-sitting-in', label: 'worth sitting in', pattern: /\bworth\s+sitting\s+in\b/gi, kind: 'phrase' },
  { id: 'worth-sitting-on', label: 'worth sitting on', pattern: /\bworth\s+sitting\s+on\b/gi, kind: 'phrase' },
  { id: 'sit-with-that', label: 'sit with that', pattern: /\bsit\s+with\s+that(?:\s+for\s+a\s+moment)?\b/gi, kind: 'phrase' },
  { id: 'something-to-sit-with', label: 'something to sit with', pattern: /\bsomething\s+to\s+sit\s+with\b/gi, kind: 'phrase' },
  // The bare construction, added 2026-09-08 after "the hardest leadership
  // failures to sit with" shipped in a post TWICE. It matched none of the
  // variants above, because those enumerate three ways of saying it and this
  // was the fourth. Nobody can list a voice.
  //
  // Bart: "If I never saw 'sit with' again in a post that I ever write, that's
  // a success." Safe to make this broad now that guardrails run on prose only:
  // a literal "I sat with him for an hour" in conversation is never touched,
  // because conversation is no longer policed at all.
  { id: 'sit-with-bare', label: 'sit with / sitting with', pattern: /\bsit(?:ting)?\s+with\b/gi, kind: 'phrase' },
  { id: 'hold-space-for', label: 'hold(s) space for', pattern: /\bholds?\s+space\s+for\b/gi, kind: 'phrase' },

  // --- Hedge / announce phrases ---
  { id: 'worth-noting', label: "it's worth noting", pattern: /\bit'?s\s+worth\s+noting\b/gi, kind: 'phrase' },
  { id: 'important-to-note', label: "it's important to note", pattern: /\bit'?s\s+important\s+to\s+note\b/gi, kind: 'phrase' },
  { id: 'important-to-remember', label: "it's important to remember", pattern: /\bit'?s\s+important\s+to\s+remember\b/gi, kind: 'phrase' },
  { id: 'at-its-core', label: 'at its core', pattern: /\bat\s+its\s+core\b/gi, kind: 'phrase' },
  { id: 'in-many-ways', label: 'in many ways', pattern: /\bin\s+many\s+ways\b/gi, kind: 'phrase' },
  { id: 'in-essence', label: 'in essence', pattern: /\bin\s+essence\b/gi, kind: 'phrase' },
  { id: 'cannot-be-overstated', label: 'cannot be overstated', pattern: /\bcannot\s+be\s+overstated\b/gi, kind: 'phrase' },
  // The tic is "this speaks to the importance of", an abstract standing in for
  // an argument. A leader who "speaks to the people they lead" is literally
  // talking to them, which is the subject of half the devotionals.
  { id: 'speaks-to', label: 'this/which speaks to the', pattern: /\b(?:this|that|which|it)\s+speaks\s+to\s+(?:the|a)\b/gi, kind: 'phrase' },
  { id: 'shed-light-on', label: 'shed light on', pattern: /\bshed\s+light\s+on\b/gi, kind: 'phrase' },

  // --- Words the system prompt banned but nothing enforced ---
  //
  // Added 2026-09-09. The prompt's BANNED WORDS list named 33 words; the module
  // caught 16 of them. Auto was told not to use these and nothing verified it,
  // which is the two-sources-of-truth drift Bart predicted. The prompt's lists
  // are now generated from this array, so the gap cannot reopen.
  //
  // Several are scoped to their figurative use or marked soft, because Bart
  // writes about scripture and leadership where the literal sense is ordinary.
  // "Testament" is the clearest case: he writes "New Testament" constantly, so
  // only "a testament to" is the tic.
  { id: 'bolster', label: 'bolster', pattern: /\bbolster(s|ing|ed)?\b/gi, kind: 'phrase' },
  { id: 'foster-figurative', label: 'foster (figurative)', pattern: /\bfoster(s|ing|ed)?\s+(a|an|the)\b/gi, kind: 'phrase', soft: true },
  { id: 'harness-figurative', label: 'harness (figurative)', pattern: /\bharness(es|ing|ed)?\s+the\b/gi, kind: 'phrase', soft: true },
  { id: 'pave-the-way', label: 'pave the way', pattern: /\bpave[sd]?\s+the\s+way\b/gi, kind: 'phrase' },
  { id: 'pivotal', label: 'pivotal', pattern: /\bpivotal\b/gi, kind: 'phrase' },
  { id: 'groundbreaking', label: 'groundbreaking', pattern: /\bground.?breaking\b/gi, kind: 'phrase' },
  { id: 'cutting-edge', label: 'cutting-edge', pattern: /\bcutting.edge\b/gi, kind: 'phrase' },
  // Deliberately does NOT match "transformational", which is the name of one of
  // his own series.
  { id: 'transformative', label: 'transformative', pattern: /\btransformative\b/gi, kind: 'phrase' },
  { id: 'game-changing', label: 'game-changing', pattern: /\bgame.chang(ing|er)\b/gi, kind: 'phrase' },
  { id: 'comprehensive', label: 'comprehensive', pattern: /\bcomprehensive\b/gi, kind: 'phrase', soft: true },
  { id: 'seamless', label: 'seamless', pattern: /\bseamless(ly)?\b/gi, kind: 'phrase' },
  { id: 'intricate', label: 'intricate', pattern: /\bintricate(ly)?\b/gi, kind: 'phrase' },
  { id: 'vibrant', label: 'vibrant', pattern: /\bvibrant\b/gi, kind: 'phrase' },
  { id: 'holistic', label: 'holistic', pattern: /\bholistic(ally)?\b/gi, kind: 'phrase' },
  { id: 'testament-to', label: 'a testament to', pattern: /\ba\s+testament\s+to\b/gi, kind: 'phrase' },
  { id: 'realm-of', label: 'the realm of', pattern: /\bthe\s+realm\s+of\b/gi, kind: 'phrase', soft: true },
  { id: 'impactful', label: 'impactful', pattern: /\bimpactful\b/gi, kind: 'phrase' },

  // --- Transition clichés ---
  { id: 'furthermore', label: 'furthermore', pattern: /\bfurthermore\b/gi, kind: 'phrase' },
  { id: 'moreover', label: 'moreover', pattern: /\bmoreover\b/gi, kind: 'phrase' },
  // The banned phrase is the filler idiom, where "when it comes to hiring" means
  // "regarding hiring". It is not the literal sense of something arriving at a
  // person. 2026-09-13: this pattern stopped every production build over a
  // devotional line "how does a leader receive correction when it comes to
  // them", which is correction arriving, not filler. A pronoun or "mind" after
  // "to" is the literal sense, so it is let through. When a rule and the writing
  // disagree, the writing is the thing being protected.
  { id: 'when-it-comes-to', label: 'when it comes to', pattern: /\bwhen\s+it\s+comes\s+to\b(?!\s+(?:them|him|her|me|us|you|it|mind)\b)/gi, kind: 'phrase' },
  { id: 'at-the-end-of-the-day', label: 'at the end of the day', pattern: /\bat\s+the\s+end\s+of\s+the\s+day\b/gi, kind: 'phrase' },
  { id: 'in-conclusion', label: 'in conclusion', pattern: /\bin\s+conclusion\b/gi, kind: 'phrase' },
  { id: 'in-a-world-where', label: 'in a world where', pattern: /\bin\s+a\s+world\s+where\b/gi, kind: 'phrase' },

  // --- Vague-depth / filler ---
  { id: 'delve', label: 'delve (into)', pattern: /\bdelve(?:\s+into)?\b/gi, kind: 'phrase' },
  { id: 'tapestry', label: '(rich) tapestry', pattern: /\b(?:rich\s+)?tapestry\b/gi, kind: 'phrase' },
  { id: 'navigate', label: 'navigate', pattern: /\bnavigate\b/gi, kind: 'phrase' },
  { id: 'resonate', label: 'resonate/resonates/resonating', pattern: /\bresonat(?:e|es|ing)\b/gi, kind: 'phrase' },
  { id: 'underscores', label: 'underscores (the importance of)', pattern: /\bunderscores?(?:\s+the\s+importance\s+of)?\b/gi, kind: 'phrase' },
  { id: 'testament-to', label: 'testament to', pattern: /\btestament\s+to\b/gi, kind: 'phrase' },
  { id: 'nuanced', label: 'nuanced', pattern: /\bnuanced\b/gi, kind: 'phrase' },
  { id: 'multifaceted', label: 'multifaceted', pattern: /\bmultifaceted\b/gi, kind: 'phrase' },
  { id: 'boasts', label: 'boast(s)', pattern: /\bboasts?\b/gi, kind: 'phrase' },
  { id: 'seamlessly', label: 'seamlessly', pattern: /\bseamlessly\b/gi, kind: 'phrase' },
  { id: 'robust', label: 'robust', pattern: /\brobust\b/gi, kind: 'phrase' },
  { id: 'unpack', label: 'unpack(ing)', pattern: /\bunpack(?:ing)?\b/gi, kind: 'phrase' },
  // Soft: often literal travel / literal terrain — rewrite when metaphorical, never forced-strip
  { id: 'journey', label: 'journey (metaphor)', pattern: /\bjourney\b/gi, kind: 'phrase', soft: true },
  { id: 'landscape', label: 'landscape (metaphor)', pattern: /\blandscape\b/gi, kind: 'phrase', soft: true },

  // --- Corporate / marketing-speak ---
  { id: 'unlock', label: 'unlock(s|ing)', pattern: /\bunlock(?:s|ing)?\b/gi, kind: 'phrase' },
  // Marketing usage. "What they will unleash" is the ordinary transitive verb,
  // and the corpus hit was a proverb about anger, which is exactly right.
  { id: 'unleash', label: 'unleash the potential/power', pattern: /\bunleash(?:es|ing)?\s+(?:the|your|their|its)\s+(?:potential|power|creativity|value|growth|capability)\b/gi, kind: 'phrase' },
  // Marketing usage only. "Elevate your brand / the experience / your business"
  // is the tic. Raising a person up is the literal sense and it is the centre of
  // Bart's subject: "use your authority today to protect or elevate someone
  // else", "power is not meant to elevate the leader above others". All nine
  // corpus hits were the literal sense, and elevate was never in his stated
  // banned-words list, so the broad pattern was inventing a rule he never set.
  {
    id: 'elevate',
    label: 'elevate (marketing usage)',
    pattern: /\belevat(?:e|es|ing)\s+(?:your|the|our|their)\s+(?:brand|business|experience|game|content|strategy|results|performance|potential|impact)\b/gi,
    kind: 'phrase',
  },
  // Verb only. The noun is ordinary English and load-bearing in Bart's
  // accountability writing: "information is treated as leverage", "surrendering
  // leverage they intend to keep", "relational leverage". The pattern was named
  // leverage-verb and matched both, so 7 of its 15 corpus hits were nouns he
  // meant. Matches an infinitive, a modal, or a subject pronoun before it,
  // plus the inflected forms which are always verbal.
  {
    id: 'leverage-verb',
    label: 'leverage (as a verb)',
    pattern: /\b(?:to|will|can|could|should|would|must|they|we|you|he|she|it|and)\s+leverage\b|\bleverag(?:es|ed|ing)\b/gi,
    kind: 'phrase',
  },
  { id: 'game-changer', label: 'game-changer', pattern: /\bgame[- ]changer\b/gi, kind: 'phrase' },
  { id: 'paradigm-shift', label: 'paradigm shift', pattern: /\bparadigm\s+shift\b/gi, kind: 'phrase' },
  { id: 'in-todays-world', label: "in today's (fast-paced) world", pattern: /\bin\s+today'?s\s+(?:fast-paced\s+)?world\b/gi, kind: 'phrase' },

  // --- Structural patterns ---
  { id: 'not-only-but-also', label: 'not only X but also Y', pattern: /\bnot\s+only\b[\s\S]{0,60}\bbut\s+also\b/gi, kind: 'structural' },
  // The false-contrast tic is "not just X, it's Y", where "it's" opens the
  // replacement clause. "Not just its collapse but its scale" is the possessive
  // and ordinary English, so the apostrophe is now required.
  // Must stay inside one sentence. With [\s\S] the window crossed a full stop and
  // matched "not just employees." followed by an unrelated "it's" in the next
  // sentence, which is not the construction at all.
  //
  // 2026-09-19: it also matched "cost you something real, not just where it's
  // convenient", where "it's" belongs to the same clause, and stopped every
  // deploy. Bart: "in context, that phrase is fine." The construction pivots to
  // a second clause, so "it's" must follow a comma, semicolon, colon or dash.
  { id: 'not-just-its', label: "not just X, it's Y", pattern: /\bnot\s+just\b[^.!?\n]{0,40}[,;:\u2013\u2014-]\s*it['\u2019]s\b/gi, kind: 'structural' },
  { id: 'its-not-its', label: "it's not X, it's Y", pattern: /\bit'?s\s+not\b[\s\S]{0,40}\b(?:it'?s|about)\b/gi, kind: 'structural' },
  // Em dash — already a prompt rule ("No em dashes. Ever."); enforce in code too.
  { id: 'em-dash', label: 'em dash (—)', pattern: /—/g, kind: 'structural' },
];

/**
 * @param {string} text
 * @returns {VoiceViolationHit[]}
 */
export function detectVoiceViolations(text) {
  const input = String(text || '');
  if (!input) return [];

  /** @type {VoiceViolationHit[]} */
  const hits = [];

  for (const def of VOICE_VIOLATIONS) {
    // Fresh lastIndex each scan — patterns are global.
    def.pattern.lastIndex = 0;
    let match;
    while ((match = def.pattern.exec(input)) !== null) {
      hits.push({
        id: def.id,
        label: def.label,
        matchedText: match[0],
        index: match.index,
        soft: !!def.soft,
      });
      // Avoid zero-length infinite loops
      if (match[0].length === 0) def.pattern.lastIndex += 1;
    }
  }

  hits.sort((a, b) => a.index - b.index);
  return hits;
}

function formatViolationList(violations) {
  return violations
    .map((v) => `- ${v.label}: "${v.matchedText.replace(/\s+/g, ' ').trim()}"`)
    .join('\n');
}

/**
 * Last-resort: remove hard (non-soft) matched substrings. Soft hits are left alone.
 * @param {string} text
 * @param {VoiceViolationHit[]} violations
 */
function forceStripViolations(text, violations) {
  let out = String(text || '');
  const hard = violations.filter((v) => !v.soft);
  // Strip longer matches first so nested overlaps behave better
  const unique = [...hard].sort((a, b) => b.matchedText.length - a.matchedText.length);
  const seen = new Set();
  for (const v of unique) {
    const key = v.matchedText.toLowerCase();
    if (seen.has(key)) continue;
    seen.add(key);
    // Escape for literal replace-all
    const escaped = v.matchedText.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    out = out.replace(new RegExp(escaped, 'gi'), '');
  }
  // Collapse leftover double spaces from phrase strips (not newlines)
  out = out.replace(/[^\S\n]{2,}/g, ' ').replace(/ +\n/g, '\n').trim();
  return out;
}

async function rewriteOnce(anthropicClient, text, violations, attemptLabel) {
  const model =
    process.env.AUTO_VOICE_CORRECTION_MODEL ||
    process.env.AUTO_ANTHROPIC_MODEL ||
    'claude-haiku-4-5-20251001';

  const system = `You wrote a response that used flagged phrases/patterns:
${formatViolationList(violations)}

Rewrite the text below to remove ONLY those, preserving all other meaning, structure, formatting, and voice exactly. Do not shorten it, do not add commentary, do not explain what you changed. Return ONLY the corrected text.`;

  const response = await anthropicClient.messages.create({
    model,
    max_tokens: Math.min(8000, Math.max(1000, Math.ceil(String(text).length / 2) + 500)),
    system,
    messages: [{ role: 'user', content: String(text) }],
  });

  const corrected = (response.content?.[0]?.text || '').trim();
  if (!corrected) {
    throw new Error(`Voice correction returned empty text (${attemptLabel})`);
  }
  // The correction prompt explicitly instructs "do not shorten it," but nothing enforced
  // that -- confirmed live: a correction pass can come back substantially shorter than the
  // original, and with no check here, that shortened text silently became the final reply,
  // replacing what Bart was already reading on screen with something visibly different.
  // A real edit that only removes a few flagged words should barely change length; a drop
  // this large means the model rewrote instead of edited, which this function must treat
  // as a failed attempt, not a success.
  const lengthRatio = corrected.length / Math.max(1, String(text).length);
  if (lengthRatio < 0.75) {
    throw new Error(
      `Voice correction dropped too much content (${attemptLabel}): ${String(text).length} chars -> ${corrected.length} chars (${Math.round(lengthRatio * 100)}%)`
    );
  }
  return corrected;
}

/**
 * Self-correct Auto-generated text against VOICE_VIOLATIONS.
 * Never throws. On API failure, returns the original text unmodified.
 *
 * @param {string} text
 * @param {{ anthropicClient?: import('@anthropic-ai/sdk').default, contextLabel?: string }} [options]
 */
export async function enforceVoiceGuardrails(
  text,
  { anthropicClient = null, contextLabel = 'response' } = {}
) {
  const original = String(text ?? '');
  try {
    let violations = detectVoiceViolations(original);
    if (violations.length === 0) {
      return { text: original, corrected: false, forced: false, violations: [] };
    }

    if (!anthropicClient) {
      console.warn(
        `[voiceGuardrails] Violations found in ${contextLabel} but no anthropicClient — returning original (${violations.length} hit(s))`
      );
      return { text: original, corrected: false, forced: false, violations };
    }

    let current = original;
    const originalViolations = violations;

    // Pass 1
    try {
      current = await rewriteOnce(anthropicClient, current, violations, 'pass-1');
    } catch (err) {
      console.warn(
        `[voiceGuardrails] Correction API failed (${contextLabel}); returning original:`,
        err?.message || err
      );
      return { text: original, corrected: false, forced: false, violations: originalViolations };
    }

    violations = detectVoiceViolations(current);
    if (violations.length === 0) {
      return { text: current, corrected: true, forced: false, violations: originalViolations };
    }

    // Pass 2 — name remaining violations explicitly
    try {
      current = await rewriteOnce(anthropicClient, current, violations, 'pass-2');
    } catch (err) {
      console.warn(
        `[voiceGuardrails] Second correction API failed (${contextLabel}); forced strip:`,
        err?.message || err
      );
      const stripped = forceStripViolations(current, violations);
      console.warn(
        `[voiceGuardrails] FORCED STRIP after correction API failure (${contextLabel}):`,
        violations.map((v) => v.id).join(', ')
      );
      return { text: stripped, corrected: true, forced: true, violations: originalViolations };
    }

    violations = detectVoiceViolations(current);
    if (violations.length === 0) {
      return { text: current, corrected: true, forced: false, violations: originalViolations };
    }

    // Last resort
    const stripped = forceStripViolations(current, violations);
    console.warn(
      `[voiceGuardrails] FORCED STRIP after 2 failed self-correction attempts (${contextLabel}):`,
      violations.map((v) => `${v.id}:"${v.matchedText}"`).join('; ')
    );
    return { text: stripped, corrected: true, forced: true, violations: originalViolations };
  } catch (err) {
    console.warn(
      `[voiceGuardrails] Unexpected error (${contextLabel}); returning original:`,
      err?.message || err
    );
    return { text: original, corrected: false, forced: false, violations: [] };
  }
}

/**
 * Walk a JSON-ish value and run enforceVoiceGuardrails on every string leaf.
 * Used for caption maps / derivation plans so JSON structure stays intact.
 *
 * @param {unknown} value
 * @param {{ anthropicClient?: import('@anthropic-ai/sdk').default, contextLabel?: string }} [options]
 */
export async function enforceVoiceGuardrailsDeep(value, options = {}) {
  if (typeof value === 'string') {
    const result = await enforceVoiceGuardrails(value, options);
    return { value: result.text, anyCorrected: result.corrected, violations: result.violations };
  }
  if (Array.isArray(value)) {
    let anyCorrected = false;
    const allViolations = [];
    const out = [];
    for (let i = 0; i < value.length; i++) {
      const r = await enforceVoiceGuardrailsDeep(value[i], {
        ...options,
        contextLabel: `${options.contextLabel || 'deep'}[${i}]`,
      });
      out.push(r.value);
      if (r.anyCorrected) anyCorrected = true;
      allViolations.push(...(r.violations || []));
    }
    return { value: out, anyCorrected, violations: allViolations };
  }
  if (value && typeof value === 'object') {
    let anyCorrected = false;
    const allViolations = [];
    const out = {};
    for (const [k, v] of Object.entries(value)) {
      const r = await enforceVoiceGuardrailsDeep(v, {
        ...options,
        contextLabel: `${options.contextLabel || 'deep'}.${k}`,
      });
      out[k] = r.value;
      if (r.anyCorrected) anyCorrected = true;
      allViolations.push(...(r.violations || []));
    }
    return { value: out, anyCorrected, violations: allViolations };
  }
  return { value, anyCorrected: false, violations: [] };
}

/**
 * Render the banned list as prompt text.
 *
 * The single source of truth Bart asked for on 2026-09-09. The system prompt
 * used to carry its own hand-typed lists of banned words, phrases and
 * constructions. They drifted, measurably: the prompt named 33 banned words and
 * the module enforced 16 of them, so Auto was instructed to avoid 17 words that
 * nothing verified. In the other direction, neither list contained "sit with",
 * which is why it shipped twice.
 *
 * Generating the prompt text from VOICE_VIOLATIONS means adding a pattern here
 * both enforces it and tells Auto about it, in one edit. Two lists cannot
 * disagree when there is one list.
 */
export function formatBannedListForPrompt() {
  const hard = VOICE_VIOLATIONS.filter((v) => !v.soft).map((v) => v.label);
  const soft = VOICE_VIOLATIONS.filter((v) => v.soft).map((v) => v.label);

  const lines = [
    '### BANNED — ENFORCED IN CODE, NOT ADVISORY',
    '',
    'This list is generated from lib/ao/voiceGuardrails.js, the same module that blocks a save.',
    'If a phrase below reaches a draft, save_draft refuses it and the text is handed back to you.',
    'You cannot talk your way past this and there is no version of "I fixed it" that skips the check.',
    '',
    `NEVER USE: ${hard.join(', ')}.`,
  ];

  if (soft.length) {
    lines.push(
      '',
      `AVOID unless the literal sense is genuinely meant: ${soft.join(', ')}.`,
      'These have ordinary uses (foster care, a harness, the New Testament), so they are rewritten rather than stripped.'
    );
  }

  lines.push(
    '',
    'Em dashes and en dashes are absolute. Not in prose, not in titles, not in captions.',
    '',
    'This applies to PROSE ONLY. In conversation you may quote any of these freely, including',
    'when Bart names one in a note or when you are explaining why one got through. Discussing a',
    'banned phrase is not using it.'
  );

  return lines.join('\n');
}

/**
 * Scan prose only, ignoring markdown headings.
 *
 * Bart, 2026-09-09: "Headings don't bother me. It's the prose that does.
 * Headings with em dashes are not AI specific."
 *
 * This is not a loophole, it is the rule. Two of his own conventions live in
 * headings and neither is a tic:
 *
 *   "## The Question Worth Sitting With" closes seven of the nine ALI series
 *   entries. It is a deliberate structural device, repeated on purpose.
 *
 *   "# Power vs. Authority: Part 1 — The Hunger" uses a dash as a title
 *   separator, which he ruled exempt: "Title separators don't bother me. It's
 *   logical those don't waive the I am written by AI flag."
 *
 * Without this, the save gate would refuse the next ALI post for using the
 * heading the series is built on, and the build check would fail forever on
 * nine published files that are exactly right.
 */
export function detectVoiceViolationsInProse(text) {
  const lines = String(text ?? '').split('\n');

  // Blank rather than remove, so reported character offsets still line up with
  // the original text.
  //
  // Three kinds of line are not Bart's prose and must never be rewritten to fix
  // his voice:
  //
  //   HEADINGS. "## The Question Worth Sitting With" closes seven ALI entries
  //   as a deliberate device, and dashes in titles are separators he ruled fine.
  //
  //   BLOCKQUOTES and QUOTED LINES. The Saul entry quotes 1 Samuel 28, which
  //   contains "Moreover". The detector flagged it. Editing scripture, or any
  //   sourced quotation, to remove a banned word would falsify the quote, which
  //   is a far worse failure than the tic.
  //
  //   ITALIC SUBTITLE LINES like "*The Archetype Series — Entry Three*", which
  //   are title separators on a line markdown does not mark as a heading.
  const prose = lines
    .map((line) => {
      const t = line.trim();
      const isHeading = /^\s{0,3}#{1,6}\s/.test(line);
      const isBlockquote = /^\s{0,3}>/.test(line);
      // A line that is entirely a quotation, straight or curly quotes.
      const isQuotedLine = /^["“][\s\S]*["”][.,]?$/.test(t);
      // *Series — Entry Three* style subtitle.
      const isItalicSubtitle = /^\*[^*]+\*$/.test(t);
      const exempt = isHeading || isBlockquote || isQuotedLine || isItalicSubtitle;
      return exempt ? ' '.repeat(line.length) : line;
    })
    .join('\n');

  return detectVoiceViolations(prose);
}
