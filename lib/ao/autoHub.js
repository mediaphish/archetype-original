...
/**
 * MODE SYSTEM — CURRENTLY UNUSED
 *
 * The following functions (wantsExitTrainingMode, shouldLoadAccountQuoteCardContext,
 * finalizeAutoModeForQuoteWork, detectAutoMode) were built to shape Auto's behavior
 * based on what stage of work Bart is in. chat.js currently hardcodes mode: 'plan'
 * on every message and does not call any of these functions.
 *
 * These are not dead in the sense of being wrong — they are dormant pending a
 * decision to wire them in. Do not delete them. Do not wire them in without a
 * separate explicit prompt to do so.
 *
 * Status: dormant as of July 2, 2026.
 */
...
/**
 * Persists the moment Bart clicked Clear on the Generated Images panel, onto
 * the thread itself, so it survives a page reload. The panel previously only
 * held this timestamp in an in-memory React ref -- the instant the page
 * reloaded (refreshing the browser, reopening the conversation later, or
 * anything else that remounts the page), the ref reset to nothing and every
 * image ever generated in that thread reappeared, no matter how many times
 * Clear had been clicked before. Stored the same way user_archived already
 * is, in the thread's own state column.
 */
export async function setDesignImagesClearedAt(email, threadId, clearedAtIso) {
  const owner = safeText(email, 200).toLowerCase();
  const id = safeText(threadId, 80);
  if (!owner || !id || !clearedAtIso) throw new Error('email, thread id, and clearedAtIso required');
  ...
  const updated = await supabaseAdmin
    ...
    .update({
      updated_at: new Date().toISOString(),
      state: { ...prevState, design_images_cleared_at: clearedAtIso },
    })
    .eq('id', id)
    .eq('created_by_email', owner)
    .select('*')
...
/**
 * Episode-level Riverside recording link + schedule, stored on the shared Auto thread
 * (ao_auto_threads.state.podcast_episode). Guests on the same episode share episode_thread_id.
 */
export async function getPodcastEpisodeLink(email, threadId) {
  const owner = safeText(email, 200).toLowerCase();
  const id = safeText(threadId, 80);
  if (!owner || !id) throw new Error('email and thread id required');
  ...
  const pe =
    existing.data.state &&
    typeof existing.data.state === 'object' &&
    existing.data.state.podcast_episode &&
    typeof existing.data.state.podcast_episode === 'object'
      ? existing.data.state.podcast_episode
      : null;
  ...
  if (!pe) {
    return {
      riverside_link: null,
      riverside_episode_id: null,
      scheduled_at: null,
      timezone: null,
      recording_link_sent_at: null,
    };
  }

  return {
    riverside_link: pe.riverside_link || null,
    riverside_episode_id: pe.riverside_episode_id || null,
    scheduled_at: pe.scheduled_at || null,
    timezone: pe.timezone || null,
    recording_link_sent_at: pe.recording_link_sent_at || null,
  };
}

export async function setPodcastEpisodeLink(
  email,
  threadId,
  { riverside_link, riverside_episode_id, scheduled_at, timezone }
) {
  const owner = safeText(email, 200).toLowerCase();
  const id = safeText(threadId, 80);
  if (!owner || !id) throw new Error('email and thread id required');
  ...
  const podcast_episode = {
    riverside_link: riverside_link || null,
    riverside_episode_id: riverside_episode_id || null,
  ...
  const updated = await supabaseAdmin
    ...
    .update({
      updated_at: new Date().toISOString(),
      state: { ...prevState, podcast_episode },
    })
    .eq('id', id)
    .eq('created_by_email', owner)
    .select('*')
...
export async function markPodcastEpisodeLinkSent(email, threadId, sentAtIso) {
  const owner = safeText(email, 200).toLowerCase();
  const id = safeText(threadId, 80);
  if (!owner || !id || !sentAtIso) throw new Error('email, thread id, and sentAtIso required');
  ...
  const prevState =
    existing.data.state && typeof existing.data.state === 'object' ? existing.data.state : {};
  const prevEpisode =
    prevState.podcast_episode && typeof prevState.podcast_episode === 'object'
      ? prevState.podcast_episode
      : {};
  ...
  const updated = await supabaseAdmin
    ...
    .update({
      ...
      state: {
        ...
        podcast_episode: {
          ...prevEpisode,
          recording_link_sent_at: sentAtIso,
        },
      },
    })
    .eq('id', id)
...
export async function getPodcastCombinedShowNotes(email, threadId) {
  const owner = safeText(email, 200).toLowerCase();
  const id = safeText(threadId, 80);
  if (!owner || !id) throw new Error('email and thread id required');
  ...
  return {
    combined_show_notes_md:
      pe && typeof pe.combined_show_notes_md === 'string' ? pe.combined_show_notes_md : '',
  };
}

export async function setPodcastCombinedShowNotes(email, threadId, combinedShowNotesMd) {
  const owner = safeText(email, 200).toLowerCase();
  const id = safeText(threadId, 80);
  if (!owner || !id) throw new Error('email and thread id required');
  ...
  const updated = await supabaseAdmin
    ...
    .update({
      ...
      state: {
        ...
        podcast_episode: {
          ...prevEpisode,
          combined_show_notes_md: String(combinedShowNotesMd ?? ''),
        },
      },
    })
    .eq('id', id)
