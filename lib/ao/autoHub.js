...
export function detectAutoMode(text, fallback = 'general') {
  ...
  if (/\bpackage\b|\bready to go out\b|\bi have a post\b|\bdon't refine\b|\bdo not refine\b/.test(s)) return 'package';
  ...
  if (/^plan\b|\blet'?s plan\b|\bplan this\b/.test(s)) return 'plan';
  return fallback;
}
