/**
 * Quote card → public PNG URL (Supabase storage) for social APIs.
 * Minimal cards use canvas + bundled font; SVG→Sharp kept as fallback when only SVG exists.
 */

import crypto from 'crypto';
import { supabaseAdmin } from '../supabase-admin.js';
import { rasterizeQuoteCardSvgToPngBuffer } from './quoteCardRasterize.js';
import { renderMinimalQuoteCardPngBuffer } from './quoteCardCanvasPng.js';
 ...
 */
export async function uploadQuoteCardPngBuffer(png, opts = {}) {
  const subfolder = String(opts.subfolder || 'quote-cards').replace(/^\/+|\/+$/g, '') || 'quote-cards';
  if (!png || !Buffer.isBuffer(png) || png.length < 100) {
    return { ok: false, error: 'Invalid or empty PNG' };
  }
...
/**
 * Minimal black card: canvas PNG (reliable text), not librsvg.
 * @param {{ quote: string, sourceName?: string, logoUrl?: string | null }} parts
 * @param {{ subfolder?: string }} [opts]
 */
export async function uploadMinimalQuoteCardToPublicUrl(parts, opts = {}) {
  const raster = await renderMinimalQuoteCardPngBuffer({
    quote: parts.quote,
    sourceName: parts.sourceName,
    logoUrl: parts.logoUrl,
  });
  if (!raster.ok) {
    return { ok: false, error: raster.error || 'Could not render quote card' };
  }
  return uploadQuoteCardPngBuffer(raster.buffer, opts);
}
...
export async function uploadQuoteCardSvgToPublicUrl(svgString, opts = {}) {
  ...
  return uploadQuoteCardPngBuffer(raster.buffer, { subfolder });
}
