...
/** Pull-quote retrieval: journal posts, devotionals, and the public RemAIning Human ebook only (no internal ALI canon chunks). */
...
export async function getCorpusPullQuotes({
  ...
  const prefer = preferTypes?.length ? preferTypes : ['journal-post', 'devotional', 'book'];
  ...
  for (const { d } of top) {
    const rawText = [d.summary, d.body].filter(Boolean).join('\n\n');
