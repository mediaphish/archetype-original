/**
 * Corpus pull quotes: candidate lines for quote cards, re-ranked by a model.
 *
 * The ranking decides which of Bart's lines get published on a card, so it runs
 * on Claude through textModel.js. It ran on gpt-4o-mini until 2026-09-23, which
 * meant his voice guardrails were judging lines chosen by a model that had
 * never seen the standard. Ranking falls back to input order as before.
 */
...
/**
 * Load the corpus for server-side use.
 *
 * Prefers data/knowledge-full.json, which carries every document. Since
 * 2026-08-21 public/knowledge.json is filtered to public document types before
 * it is published, so reading it here would silently drop the 55 Culture
 * Science documents from anything that ranks or embeds the corpus.
 *
 * Falls back to the public file so this still works in contexts where the full
 * corpus was not shipped alongside the function.
 *
 * @returns {Promise<object[]|null>} docs or null if neither file is readable
 */
export async function loadKnowledgeDocs() {
  for (const rel of [
    ['data', 'knowledge-full.json'],
    ['public', 'knowledge.json'],
  ]) {
    try {
      const raw = await readFile(join(process.cwd(), ...rel), 'utf-8');
...
export async function refinePullQuotesWithLlm(candidates, { queryText = '', limit = 5 } = {}) {
  ...
  if (!textModelConfigured() || pool.length <= limit) {
...
Return ONLY JSON like {"indices":[3,1,7,2,5]}: up to ${limit} entries, the candidate numbers ("n") in priority order (best first). Use only numbers that exist in the list.`;
  ...
  try {
    const parsed = await completeJson({ prompt, task: 'analysis', maxTokens: 1500, effort: 'low' });
