/**
 * AO Scout — crawl the frontier and judge what is worth sending to Analyst.
 *
 * The assessment step is a judgement call with reasoning, so it runs on Claude
 * through textModel.js at the analysis tier. It was a raw fetch to gpt-4o-mini,
 * and OpenAI is kept for image creation only.
 */
...
async function getEditorialContextForScout() {
  const ownerEmail = String(process.env.AO_OWNER_EMAIL || '').toLowerCase().trim();
  if (!ownerEmail) return { chase: [], recent: [] };
  try {
    ...
    const recentOut = await supabaseAdmin
      ...
      .limit(10);
    const recent = Array.isArray(recentOut.data) ? recentOut.data : [];
...
async function aiAssess({ url, title, excerpt, rawText }) {
  ...
  const editorial = await getEditorialContextForScout();
  const chaseLines = (Array.isArray(editorial?.chase) ? editorial.chase : [])
    .map((c) => `- ${safeText(c?.topic, 160)}${c?.why ? ` — ${safeText(c.why, 160)}` : ''}`)
    .filter(Boolean)
    .slice(0, 8)
    .join('\n');
  const recentLines = (Array.isArray(editorial?.recent) ? editorial.recent : [])
    .map((r) => `- ${safeText(r?.title, 140)}`)
    .filter(Boolean)
    .slice(0, 10)
    .join('\n');
  ...
  const prompt = `You are AO Scout — a conservative reporter on the leadership beat.\n\nHard rules:\n- If this is paywalled, irrelevant, or too thin to trust, return relevant=false.\n- No politics. No rage bait. No dunking.\n- Prefer practical leadership signals: leadership, teams, culture, accountability, trust, execution, decision-making.\n- No generic filler. If you cannot ground your response in the text, return relevant=false.\n\nContext (what to chase next):\n${chaseLines || '(no chase list configured yet)'}\n\nContext (what was posted recently — avoid repeats):\n${recentLines || '(no recent posted items recorded yet)'}\n\nGuidance:\n- If the story is relevant but feels like a repeat of something posted recently, keep relevant=true but add a risk flag like \"possible_repeat\" and steer summary_interpretation toward a fresh angle.\n\nInput JSON:\n${JSON.stringify(payload)}\n\nReturn ONLY JSON with exactly these keys:\n- relevant (boolean)\n- pull_quote (string, <= 25 words, or empty)\n- why_it_matters (string, 2-4 sentences that mention at least ONE concrete detail from the input text)\n- summary_interpretation (string, 4-8 sentences grounded in the input text)\n- risk_flags (string[])\n- content_kind (string: quote|research|framework|story|trend|other)\n- ao_lane (string)\n- topic_tags (string[])\n- recommended_next_stage (string: studio|publisher)\n`;

  // AI_TIMEOUT_MS bounds a single page assessment, and the helper cancels the
  ...
  try {
    ...
    const parsed = parseLooseJson(String(content || '').trim());
    ...
    for (const f of batch) {
      ...
      if (looksLikeListing && frontierAdds < MAX_TOTAL_FRONTIER_ADDS) {
        ...
        for (const u of seedLinks.slice(0, inlineMax)) {
          ...
          if (ownerEmail && (await isDiscarded({ email: ownerEmail, canonicalUrl: u }))) continue;
      ...
      if (relevant && !capReached) {
        ...
        if (ownerEmail && (await isDiscarded({ email: ownerEmail, canonicalUrl: frontierUrl }))) {
          await supabaseAdmin
            .from('ao_scout_frontier')
            .update({ status: 'done', reason: 'discarded_before', updated_at: new Date().toISOString() })
            .eq('id', f.id);
          continue;
        }
