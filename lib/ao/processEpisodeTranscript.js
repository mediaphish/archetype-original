...
import { mapEpisodeCorpusConnections } from './mapEpisodeCorpusConnections.js';
...
${transcript.slice(0, 120000)}`;
  ...
  let corpus_connections = [];
  let thematic_threads = [];
  try {
    const mapped = await mapEpisodeCorpusConnections({
      transcript,
      research_brief: input?.research_brief || '',
      episode_brief: input?.episode_brief || '',
      post_recording: input?.post_recording || null,
    });
    if (mapped.ok) {
      corpus_connections = mapped.corpus_connections || [];
      thematic_threads = mapped.thematic_threads || [];
    } else {
      console.warn('[processEpisodeTranscript] corpus mapping skipped:', mapped.error);
    }
  } catch (mapErr) {
    console.warn('[processEpisodeTranscript] corpus mapping failed:', mapErr?.message || mapErr);
  }
