 ...
 *
 * Reviewer sessions use the same cookie format with role: 'reviewer' and a
 * fixed internal identity (not a real mailbox). They never pass owner-only checks.
 *
...
// Fixed internal identity for reviewer sessions — not a real mailbox, never
// configured via env. Exists so the session payload has an email field.
...
export function readAoSession(req) {
  ...
  // Reviewer sessions use a fixed, non-owner identity. This is not a real
  // mailbox and never needs to be configured — it's an internal constant so
  // the session payload has something to put in the email field. Reviewer
  // sessions never pass the owner check below and can never access
  // owner-only routes, regardless of what this string is.
  if (role === 'reviewer') {
    if (email !== REVIEWER_IDENTITY) return { ok: false, error: 'invalid' };
