...
export async function loadSeriesImageReferenceBlocks(userMessageText, recentUserMessages = []) {
  try {
    ...
    const noteBlock = {
      type: 'text',
      text: `## REAL PRIOR HEADER IMAGES — ${match.label}\n\nThe image(s) above are the actual published header images for prior entries in this series (${resolvedTitles.join(', ')}). They were loaded automatically by the system just now because this conversation is about the series — Bart did not attach, submit, or upload them. Do not describe them as something Bart just sent you; he did not. If you refer to them, say plainly that the system pulled up the real prior header image(s) automatically for comparison. Look at them before writing or discussing the next header image prompt. Match the real rendering style, palette, and composition approach you can see, rather than guessing from memory or asking to be shown one.`,
    };
