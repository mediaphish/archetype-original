/**
 * Rapid Write hero images — OpenAI Images API (gpt-image-1), resize to 16:9 ~1024w, durable Supabase URL.
 *
 * Image generation stays on OpenAI, which is the one thing it is kept for. The
 * text step that turns the article into a scene description now runs on Claude
 * through textModel.js, because it is text and it was on gpt-4o-mini.
 */
...
async function refinePromptWithChat(fullPrompt) {
  ...
  if (!textModelConfigured()) return fullPrompt;
  const para = (
    await completeText({
      prompt: `Turn this into a single vivid scene description paragraph for an image generator (no text in the image):\n\n${fullPrompt.slice(0, 6000)}`,
      system:
...
/**
 * Resize raw image bytes → cover 1024×576 PNG.
 * @param {Buffer} buf
 */
function resizeToHeroPng(buf) {
  return sharp(buf)
...
async function callOpenAiImagesGenerations(prompt) {
  ...
  if (!apiKey) return { ok: false, error: 'AI key not configured for image generation.', buffer: null };

  // gpt-image-1, matching lib/ao/generateDesignImage.js and matching what
  // ChatGPT itself generates with. This path was on dall-e-3 at
  // quality:'standard' — an older model at its lower quality setting — which
  // is why hero images never looked like what the same prompt produces in
  // ChatGPT. That was a model gap, not a prompting gap.
  //
  // gpt-image-1 differences from dall-e-3: no response_format (it always
  // returns b64_json), quality is low|medium|high|auto rather than
  // standard|hd, and 1792x1024 is not a supported size — 1536x1024 is the
  // landscape option, and it is downscaled to the 1024x576 hero below anyway.
  ...
  if (!res.ok) {
    ...
    return { ok: false, error: err, buffer: null };
  }

  const b64 = json.data?.[0]?.b64_json || null;
  if (b64) return { ok: true, error: '', buffer: Buffer.from(b64, 'base64') };

  // Defensive: if a URL ever comes back instead, fetch it rather than fail.
  const url = json.data?.[0]?.url || null;
  if (url) {
    const img = await fetch(url);
    if (!img.ok) return { ok: false, error: 'Could not download generated image', buffer: null };
    return { ok: true, error: '', buffer: Buffer.from(await img.arrayBuffer()) };
  }

  return { ok: false, error: 'No image data in response', buffer: null };
}
...
export async function generateRapidWriteHeroForDraft(supabaseAdmin, draftId, email, opts = {}) {
  ...
  let pngBuffer;
  try {
    pngBuffer = await resizeToHeroPng(gen.buffer);
  } catch (e) {
