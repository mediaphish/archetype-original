...
export async function librarianAnnotate({ candidateRow, decision }) {
  ...
  const recentPosts = await findRecentPostedSimilar({ email: candidateRow?.created_by_email || process.env.AO_OWNER_EMAIL, queryText: query });
  ...
  const freshAngleSuggestion = recentPosts.length
    ? 'Fresh angle idea: keep the core point, but change the “frame” (new real-world example, a 3-step mini-framework, or a direct question that invites the right comments).'
    : null;
  ...
  return {
    ...
    similarity_notes: {
      ...
      fresh_angle_suggestion: freshAngleSuggestion,
