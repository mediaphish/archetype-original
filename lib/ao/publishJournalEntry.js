...
export async function publishJournalEntry(params = {}) {
  ...
  // A summary that stops mid-sentence stops the whole site from rebuilding.
  //
  // 2026-09-21: the post was committed with "About one in four professionals
  // stall when", verify-post-summaries refused it, and every deploy after that
  // failed. The post existed in the repo and never went live, while the captions
  // Bart had already posted by hand pointed at the dead URL. Repaired here
  // rather than refused: stranding the post is the worse failure, and a summary
  // cut mid-word is never what anyone intended.
  const summaryGuard = repairSummaryForPublish({ summary: summaryInput, content });
  const summary = summaryGuard.summary;
  if (summaryGuard.repaired) {
    console.warn(
      `[publishJournalEntry] repaired the summary before publishing ${slug}: ${summaryGuard.reason}`
    );
  }
  ...
  try {
    ...
    // Last gate before the public site.
    //
    // This path had zero voice checks. Anything that did not arrive through
    // save_draft, or that was edited after passing it, reached GitHub and the
    // live journal unexamined. That is the wrong place to have no backstop:
    // once it is committed it is published under Bart's name, and taking it
    // down is a second commit and a rebuild.
    //
    // Blocks rather than strips. A publish that stops is recoverable in a
    // minute; a silent rewrite of his prose on the way out the door is not.
    {
      const { detectVoiceViolationsInProse } = await import('./voiceGuardrails.js');
      const hits = detectVoiceViolationsInProse(String(content || '')).filter((v) => !v.soft);
      if (hits.length) {
        const labels = [...new Set(hits.map((h) => h.label))];
        return {
          ok: false,
          httpStatus: 400,
          gate: 'voice_violation',
          error:
            `Refusing to publish "${safeSlug}": the content contains ${hits.length} banned ` +
            `pattern(s): ${labels.join(', ')}. Fix the prose and publish again. ` +
            'Do not delete the punctuation and leave a broken sentence.',
        };
      }
    }
