 ...
 * @param {string[]} platforms
 * @returns {Promise<{ hour: number, minute: number, source: 'analytics' | 'benchmark' }>}
 */
export async function resolveSuggestedLocalClock(platforms) {
  ...
  if (hinted != null) return { hour: hinted, minute, source: 'analytics' };
  return { hour: benchmarkHourForPlatforms(platforms), minute, source: 'benchmark' };
}
