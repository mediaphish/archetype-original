...
/**
 * Minimum hashtags per caption.
 *
 * One is the floor. Zero is what Bart flagged as broken. Three is what a full
 * set usually carries and his run to seven, but his published work also
 * includes captions with a single #Leadership, so anything above zero passes
 * and anything under three is only a nudge.
 */
export const MIN_HASHTAGS = 1;
export const PREFERRED_HASHTAGS = 3;

export function countHashtags(text) {
  return (String(text || '').match(/(^|\s)#[A-Za-z][\w]{1,39}/g) || []).length;
}

/**
 * Does the caption give the reader a route back to the post?
 *
 * Instagram kills links in captions, so "link in bio" is the correct form there
 * and a bare URL is the wrong one. Everywhere else a real URL is expected.
 */
export function hasPostLink(text, channel = '') {
  const t = String(text || '');
  if (String(channel).startsWith('instagram')) {
    return /link in bio/i.test(t) || /https?:\/\/\S+/i.test(t);
  }
...
export function evaluateCaptionQualityGate(captionsByChannel) {
  ...
  for (const [channel, text] of present) {
    ...
    // Hashtags and a link. Both are structural in Bart's published captions and
    // neither was checked, so a caption set could pass every gate and still be
    // unusable. His real posts end with tags like "#ServantLeadership
    // #Leadership #ArchetypeOriginal", and every channel carries a route back to
    // the post: a full URL, or "link in bio" on Instagram where URLs are dead.
    // Hashtags. Zero is a real defect: Bart flagged a caption set with none.
    // But the floor has to be one, not three. His own published captions run
    // from a single #Leadership up to seven, and a three-tag minimum rejects
    // the real published set in the fixtures below. That is the guardrail
    // arguing with the owner's voice, which this file has already made once
    // with the CTA check. Below the preferred count is a nudge, not a block.
    if (reasons.length) failures.push({ channel, reasons });

    // Hashtags are advisory per channel. Bart's published X caption carries
    // none at all, deliberately: on a 280 character limit tags cost words. A
    // per-channel rule of any strength overrules that. The set-level check
    // below is the one that catches what he actually flagged.
    const tags = countHashtags(text);
    if (tags < PREFERRED_HASHTAGS) {
      warnings.push({
        channel,
        reason: tags === 0 ? 'no hashtags' : `only ${tags} hashtag(s); ${PREFERRED_HASHTAGS} or more is typical`,
      });
    }

    // A route back to the post. Advisory for the same reason: a real published
    // X caption carries neither a URL nor "link in bio", so blocking on this
    // would reject work Bart shipped deliberately.
    if (!hasPostLink(text, channel)) {
      warnings.push({
        channel,
        reason: String(channel).startsWith('instagram')
          ? 'no "link in bio" and no URL, so nothing routes to the post'
          : 'no link to the post; usually the full https URL belongs here',
      });
    }
  ...
  // The defect Bart actually reported: a whole caption set with no hashtags
  // anywhere. One bare caption is a choice; seven bare captions is an
  // unfinished job, and nothing was checking for it.
  // Only applies to a real multi-channel set. A single caption checked on its
  // own is not a set, and treating it as one contradicts the rule's own message.
  if (present.length > 1 && present.every(([, text]) => countHashtags(text) === 0)) {
    failures.push({
      channel: 'all',
      reasons: [
        'no hashtags anywhere in the set. End captions with tags from the house set, ' +
          'e.g. #ServantLeadership #Leadership #ArchetypeOriginal. A single bare caption is fine; a bare set is not.',
      ],
    });
  }
