...
/** Per-channel minimum caption length (usable space, not empty stubs). */
export const CAPTION_MIN_LENGTH_BY_CHANNEL = {
  linkedin_personal: 350,
  linkedin_business: 350,
  facebook_business: 300,
  facebook_personal: 300,
  instagram_business: 300,
  instagram_personal: 300,
  twitter: 200,
};
...
export function evaluateCaptionQualityGate(captionsByChannel) {
  ...
  for (const [channel, raw] of Object.entries(captions)) {
    const text = String(raw || '').trim();
    if (!text) continue;
    const reasons = [];
    const floor = CAPTION_MIN_LENGTH_BY_CHANNEL[channel] ?? 250;
    if (text.length < floor) {
      reasons.push(
        `too thin (${text.length} chars; need at least ${floor} for ${channel})`
      );
    }
    ...
    if (!hasQuestion && !hasCta) {
      reasons.push('no call to action (no question in the final third, no CTA verb near the end)');
    }
  ...
  const detail = failures
    .map((f) => `${f.channel}: ${f.reasons.join('; ')}`)
    .join(' | ');
  return {
    ok: false,
    failures,
