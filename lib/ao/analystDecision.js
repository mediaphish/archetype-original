...
3) Write why_it_matters (2-4 sentences) in Bart/AO voice.
4) Write summary_interpretation (6-10 sentences): what it says + what it means + one practical angle.
5) Generate risk_flags array (0-6). Include when relevant: Missing link, Hard paywall, Too generic, Off brand, Low-quality source, No quote-worthy line.
6) Decide objectives_by_channel as JSON with keys linkedin/facebook/instagram/x. Each has objective (reach|engagement|authority|leads) and rationale.
7) Provide alt_moves: 2 alternative moves with a short why.
8) Give publishability_score 0-100.
9) Decide quote_card_worthy boolean with a one-sentence quote_card_reason.

Return ONLY a single JSON object with exactly these keys:
auto_discarded (boolean), discard_reason (string|null), best_move (string), publishability_score (number), pull_quote (string), why_it_matters (string), summary_interpretation (string), risk_flags (string[]), objectives_by_channel (object), alt_moves (array), quote_card_worthy (boolean), quote_card_reason (string)
`;
  ...
  try {
    ...
    return {
      ...
      alt_moves: Array.isArray(parsed.alt_moves) ? parsed.alt_moves.slice(0, 2) : [],
