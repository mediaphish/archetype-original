...
export async function runExternalScan(opts = {}) {
  ...
  const sourcesOverride = Array.isArray(opts.sourcesOverride) ? opts.sourcesOverride.filter(Boolean) : null;
  const tag = String(opts.tag || '').trim().toLowerCase();
  const isCompetitorTag = tag === 'competitor';

  function candidateSourceName(src, url) {
    const base = String(src?.name || url || '').trim() || url;
    if (!isCompetitorTag) return base;
    const tier = String(src?.competitor_tier || '').trim().toLowerCase();
    const label = tier === 'competitor' ? 'Competitor' : tier === 'friendly' ? 'Friendly competitor' : 'Competitor watch';
    return `${label} — ${base}`.slice(0, 200);
  }
        ...
        try {
          ...
          for (const entry of entries) {
            ...
            candidates.push({
              ...
              source_url: link || url,
              source_name: candidateSourceName(src, url),
              source_type: isCompetitorTag ? 'competitor_rss' : 'external_rss',
          ...
          if (looksLikeListingPage(html, url)) {
            ...
            for (const link of listingLinks) {
              ...
              candidates.push({
                source_url: link,
                source_name: candidateSourceName(src, url),
                source_type: isCompetitorTag ? 'competitor_article' : 'external_article',
