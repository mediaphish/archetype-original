...
export async function rebuildEditorialMemory({ email }) {
  ...
  try {
    ...
    for (const part of chunk(rows, 200)) {
      const ins = await supabaseAdmin
        ...
        .upsert(part, { onConflict: 'created_by_email,kind,source_url_or_slug', ignoreDuplicates: false });
  ...
  try {
    ...
    for (const part of chunk(items, 200)) {
      const ins = await supabaseAdmin
        .from('ao_editorial_memory_items')
        .upsert(part, { onConflict: 'created_by_email,kind,source_scheduled_post_id', ignoreDuplicates: false });
      if (ins.error) throw ins.error;
