 ...
 * Quote-card captions.
 *
 * These captions publish under Bart's name, so they run on Claude through
 ...
 * standard. The plain fallback captions still apply when no text model is
 * configured.
 */
...
export async function generatePullQuoteCaptionsForQuotes(
  ...
  if (!textModelConfigured() || !selectedQuotes.length) {
    return empty();
  }
  ...
  try {
    const prompt = `You write captions for quote-card images. The IMAGE already contains the full pull quote. Your job is commentary: explain, interpret, deepen, or help the reader feel why the line matters.
...
${JSON.stringify(payload)}`;
    ...
    const parsed = await completeJson({ prompt, task: 'voice', maxTokens: 4500 });
