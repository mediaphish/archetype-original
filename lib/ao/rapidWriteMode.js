...
**Banned stock phrases (do not use in the body):** elephant in the room; hum of computers; modest office space where the hum of computers; bustling office on the edge of; palpable tension hanging in the air (as a repeated crutch—if tension matters, show it without this exact device every time).
...
- **Avoid repeated stems** across posts: do **not** default to **“How can you…”**, **“How would you bring peace…”**, or “your team” workbook prompts. If the instructions list **reflection questions already used in this batch**, do **not** mirror their shape.
...
/** First + last chunks of body text so batch prompts can forbid repeated phrasing (not only openings). */
export function rapidWriteBodySignatureSnippets(body, maxLen = 120) {
  const t = String(body || '')
    .replace(/\s+/g, ' ')
    .trim();
  if (!t) return [];
  if (t.length <= maxLen * 2) return [t];
  return [t.slice(0, maxLen), t.slice(-maxLen)];
}
...
export async function writeRapidWritePost(
  ...
  const snippets = Array.isArray(batchAntiRepeatSnippets)
    ? batchAntiRepeatSnippets.map((s) => safeText(s, 200)).filter(Boolean).slice(0, 24)
    : [];
  const phraseBlock =
    snippets.length > 0
...
${anchorHint}${diffBlock}${antiDup}${namesBlock}${reflBlock}${phraseBlock}
${trainingBlock}
