...
${anchorHint}${diffBlock}${titlesBlock}${costOfQuotaBlock}${closingBlock}${antiDup}${namesBlock}${reflBlock}${phraseBlock}
${trainingBlock}`;
...
Rules: AO Living Voice — human, direct, observational. Not marketer, therapist, or academic. No clichés. Do not moralize. Do not mention tags or system in body. No em dashes as a stylistic crutch. User examples in instructions are **intent only** — do not copy their wording.`;
  ...
  try {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
          ...
          messages: [
            {
              role: 'system',
              content:
                'You are the Writer for Archetype Original Rapid Write. JSON only. Treat duplicate titles, headline-quota violations, stock “known for” leader intros, and near-duplicate reflection questions as **errors to fix**, not suggestions. Obey the user message: reader bar, word band, adaptive AO voice, narrative-first prose, closing and reflection-question rules, and corpus fingerprint/passage context.',
            },
            { role: 'user', content: user },
          ],
        }),
      ...
      if ((dupTitle || costOfViolated || bannedIntro || reflSimilar) && attempt < 3) {
        const bits = [];
        if (dupTitle) {
          bits.push(
      ...
      if (dupTitle || costOfViolated || bannedIntro || reflSimilar) {
        return {
          ok: false,
          error:
            'Writer output still violated duplicate-title, headline-quota, banned-intro, or reflection-similarity rules after retries.',
        };
      }
...
Owner instruction:
${inst.slice(0, 6000)}
${trainingBlock}${overlapBlock}${diffBlock}${patternBlock}${sibBlock}${sibTitlesBlock}${costOfQuotaBlock}${sibCloseBlock}${sibReflBlock}${sibSnipBlock}`;
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      ...
      if (dupTitle || costOfViolated || bannedIntro || reflSimilar) {
        return {
          ok: false,
          error:
            'Revision still violated duplicate-title, headline-quota, banned-intro, or reflection-similarity rules after retries.',
        };
      }
