...
- **Title:** must **not** duplicate or lightly rephrase any title under “Titles already used in this batch.” Avoid repeating the same **headline formula** more than once in the batch (e.g. stacking “The Weight of…,” “The Cost of…,” “Navigating…,” “When X Meets Y” as interchangeable shells).
...
export async function writeRapidWritePost(
  ...
  const priorTitles = Array.isArray(batchPriorTitles)
    ? [...new Set(batchPriorTitles.map((s) => String(s || '').trim()).filter(Boolean))].slice(-20)
    : [];
  const titlesBlock =
    priorTitles.length > 0
      ? `\n**Titles already used in this batch (yours must be clearly different—not a light rephrase; avoid repeating the same headline formula):**\n${priorTitles.map((t) => `- ${safeText(t, 200)}`).join('\n')}\n`
      : '';
...
${anchorHint}${diffBlock}${titlesBlock}${closingBlock}${antiDup}${namesBlock}${reflBlock}${phraseBlock}
${trainingBlock}
