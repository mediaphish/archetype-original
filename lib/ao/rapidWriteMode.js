...
/** First line or leading phrase activates when followed by JSON seeds. */
...
/**
 * Parse JSON array from fenced ```json block or first [...] in message.
 */
export function parseRapidWriteSeeds(text) {
  ...
  try {
    ...
    for (let i = 0; i < parsed.length; i += 1) {
      ...
      const id = safeText(o.id, 80) || `rw-${i + 1}`;
      ...
      if (!core_idea || !leadership_category || !psychological_outcome) {
        return {
          ok: false,
          seeds: [],
          error: `Seed ${id}: core_idea, leadership_category, and psychological_outcome are required.`,
        };
      }
      seeds.push({
        id,
    ...
    return { ok: false, seeds: [], error: `Could not parse seeds JSON: ${e.message || e}` };
  }
}
