 ...
 *
 * The writer, revision, and polish passes produce prose that goes out under
 * Bart's name, so they run on Claude through textModel.js. They ran on
 * gpt-4o-mini until 2026-09-23, which meant his voice guardrails were checking
 * writing from a model that had never seen the standard. The seed extraction,
 * intent classification, and seed validation calls further up this file are
 * still on OpenAI and belong to the classify/extract conversion group.
 */
 ...
 */
/** Titles like "The Cost of …", "The Burden of …", "The Paradox of …" — scarce per batch. */
export const RAPID_WRITE_ABSTRACT_TITLE_SHELL_RE = /^\s*the\s+(cost|burden|weight|price|paradox)\s+of\b/i;
...
export function rapidWriteTitleCollidesWithBatch(title, priorTitles) {
  ...
  const k = normalizeRapidWriteTitleKey(title);
  const shellRe = /^the\s+(cost|burden|weight|price|paradox)\s+of\s+(.+)$/;
  const ma = k.match(shellRe);
  if (!ma) return false;
  const shell = ma[1];
  const tail = ma[2].trim();
  if (tail.length < 2) return false;
  const arr = Array.isArray(priorTitles) ? priorTitles : [];
  for (const p of arr) {
    const pk = normalizeRapidWriteTitleKey(p);
    const mb = pk.match(shellRe);
    if (!mb || mb[1] !== shell) continue;
    const ptail = mb[2].trim();
    if (!ptail) continue;
...
/** Count titles using "The Cost/Burden/Weight/Price/Paradox of …" (combined shell quota). */
export function countRapidWriteAbstractShellTitles(priorTitles) {
  const arr = Array.isArray(priorTitles) ? priorTitles : [];
  let n = 0;
  for (const t of arr) {
    if (RAPID_WRITE_ABSTRACT_TITLE_SHELL_RE.test(String(t || '').trim())) n += 1;
  }
  return n;
}

function normalizeOpenHead(s, maxLen) {
  return String(s || '')
    .slice(0, maxLen)
    .toLowerCase()
    .replace(/\s+/g, ' ')
...
/**
 * First ~400 chars of body read as the same opening "machine" as another post (readers feel déjà vu).
 */
export function rapidWriteOpeningTooSimilarToPrior(body, priorHeads) {
  const max = 420;
  const n = normalizeOpenHead(body, max);
  if (n.length < 95) return false;
  const heads = Array.isArray(priorHeads) ? priorHeads : [];
  for (const h of heads) {
    const p = normalizeOpenHead(h, max);
    if (p.length < 95) continue;
    const L = Math.min(n.length, p.length);
    let i = 0;
    while (i < L && n[i] === p[i]) i += 1;
    const ratio = i / L;
    if (ratio >= 0.78 && i >= 88) return true;
  }
  return false;
}

/** Conference-table / strategy-deck / metrics-screen spine in the opening (batch sameness). */
export function rapidWriteBodyOpensWithMeetingSpine(body) {
  const head = String(body || '').slice(0, 700).toLowerCase();
  if (
    /(conference room|boardroom|board room|strategy meeting|monthly strategy|quarterly review|executive table|around the table|polished table|metrics?\s+flashed|numbers?\s+on the screen|slide(s)?\s+detail|deck presented|performance metrics reflected)/i.test(
      head
    )
  ) {
    return true;
  }
  if (/(team|they)\s+gathered.*\bmeeting\b|\bgathered\s+for\s+their\s+monthly/i.test(head)) return true;
  return false;
}

export function countRapidWriteMeetingSpineOpenings(bodyHeads) {
  const arr = Array.isArray(bodyHeads) ? bodyHeads : [];
  let n = 0;
  for (const h of arr) {
    if (rapidWriteBodyOpensWithMeetingSpine(h)) n += 1;
  }
  return n;
}
...
/** Max titles per batch using "The Cost/Burden/Weight/Price/Paradox of …" before another must use a different headline architecture. */
export const RAPID_WRITE_MAX_ABSTRACT_SHELL_TITLES = 2;

/** @deprecated Use RAPID_WRITE_MAX_ABSTRACT_SHELL_TITLES — kept for scripts/tests. */
export const RAPID_WRITE_MAX_COST_OF_TITLES_PER_BATCH = RAPID_WRITE_MAX_ABSTRACT_SHELL_TITLES;

/** Max drafts in a batch whose **opening** may use the conference-table / strategy-metrics meeting spine. */
export const RAPID_WRITE_MAX_MEETING_SPINE_OPENINGS = 2;
...
- **Abstract headline shells (quota):** **“The Cost of…,” “The Burden of…,” “The Weight of…,” “The Price of…,” “The Paradox of…”** are **one family**—only a few may appear in a single batch. If the instructions say the quota is exhausted, you **must** use a different title shape (concrete image, stakes line, scene label, two-word hook—**not** swapping the last noun inside the same shell).
- **Batch spine / container (readers will notice sameness):** Do **not** run the same **story machine** as other posts: office + **strategy / quarterly / metrics** meeting as the **main** pressure stage, then the same middle beats (furtive glances → withheld truth → one brave line → partial thaw). If other posts already use that **meeting-deck spine**, you must choose a **different container** (artifact-led, field floor, handoff seam, dyad, semi-public sting, interior night, written trace—per your **assigned story pattern**).
...
**Hard failures (your JSON must not violate these):** duplicate or colliding titles vs other drafts in the batch, “The Cost of…” headline quota when listed, banned résumé-style leader intros (including “respected for / noted for / seasoned professional” molds), near-duplicate reflection questions. If a retry instruction appears, fix those issues completely.
...
- Maximum 100 seeds; if more ideas, take the clearest distinct items first.`;
  ...
  try {
    const parsed = await completeJson({
      prompt: user,
      system:
        'You extract structured Rapid Write seeds from unstructured paste. Archetype Original, servant leadership, diagnostic tone.',
      task: 'extract',
      maxTokens: 8000,
    });
    if (!parsed) return { ok: false, seeds: [], error: 'Could not interpret seeds (request failed).' };
...
export async function parseRapidWriteDraftOrDiscussIntent(userMessage, { seeds = [], drafts_by_seed_id = {} }) {
  ...
  if (!textModelConfigured()) return { intent: 'discuss', seed_ids: [], instruction: '' };
...
If the message clearly asks to change wording but intent is unclear, prefer "revise_draft" with all named seed ids.`;
  ...
  try {
    const parsed = await completeJson({
      prompt: user,
      system:
        'You classify Rapid Write owner messages. When Bart names multiple rw-* ids and wants tone or text changes (for example, remove "I"), use revise_draft and include ALL named ids that exist in Known seed ids.',
      task: 'classify',
      maxTokens: 1500,
    });
    if (!parsed) return { intent: 'discuss', seed_ids: [], instruction: '' };
...
export async function validateRapidWriteSeeds(seeds, _email) {
  const modelReady = textModelConfigured();
    ...
    if (modelReady && !skipPlainFact) {
      try {
        ...
        const parsed = await completeJson({
          ...
          task: 'classify',
          maxTokens: 1500,
          system: `You help review Rapid Write SEEDS for Archetype Original. The AUTHOR is the source: seeds are provided in good faith.
...
export async function writeRapidWritePost(
  ...
  if (!textModelConfigured()) return { ok: false, error: 'Text model not configured' };
  ...
  const priorBodyHeadsRaw = Array.isArray(batchPriorBodyHeads)
    ? batchPriorBodyHeads.map((s) => String(s || '').trim()).filter(Boolean).slice(-12)
    : [];
  const abstractShellPriorCount = countRapidWriteAbstractShellTitles(priorTitlesRaw);
  const abstractShellQuotaBlock =
    abstractShellPriorCount >= RAPID_WRITE_MAX_ABSTRACT_SHELL_TITLES
      ? `\n**MANDATORY (abstract headline quota):** This batch already has **${abstractShellPriorCount}** title(s) using **"The Cost / Burden / Weight / Price / Paradox of …"**. Your title **must NOT** use that shell family—use a different architecture (concrete image, stakes line, scene label, two-word hook).\n`
      : abstractShellPriorCount === 1
        ? `\n**Headline discipline:** Another post already uses an abstract **"The ___ of …"** title—prefer a **different** title shape for this piece.\n`
        : '';

  const meetingSpinePriorCount = countRapidWriteMeetingSpineOpenings(priorBodyHeadsRaw);
  const meetingSpineQuotaBlock =
    meetingSpinePriorCount >= RAPID_WRITE_MAX_MEETING_SPINE_OPENINGS
      ? `\n**MANDATORY (meeting-spine quota):** ${meetingSpinePriorCount} other post(s) already open on a **conference-table / quarterly metrics / strategy-deck** spine in the first section. Your **opening ~700 words** must **not** use that same container as the primary stage—follow your **assigned story pattern** with a different pressure surface.\n`
      : meetingSpinePriorCount === 1
        ? `\n**Spine discipline:** Another post already centers a formal metrics/strategy meeting early—strongly prefer a **different** container here.\n`
        : '';

  const headSnipBlock =
    priorBodyHeadsRaw.length > 0
      ? `\n**Opening stretches already drafted in this batch (do not echo the same scene grammar, setting stack, or first crisis move):**\n${priorBodyHeadsRaw.map((t) => `- ${safeText(t, 280)}`).join('\n')}\n`
      : '';
...
${anchorHint}${diffBlock}${titlesBlock}${abstractShellQuotaBlock}${meetingSpineQuotaBlock}${headSnipBlock}${closingBlock}${antiDup}${namesBlock}${reflBlock}${phraseBlock}
${trainingBlock}`;
  ...
  try {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      ...
      const parsed = await completeJson({
        prompt: user,
        system:
          'You are the Writer for Archetype Original Rapid Write. JSON only. Treat duplicate titles, abstract-shell quota, meeting-spine quota, near-duplicate openings vs other drafts, stock résumé leader intros, and near-duplicate reflection questions as **errors to fix**, not suggestions. Obey the user message: reader bar, word band, adaptive AO voice, narrative-first prose, assigned story pattern, closing and reflection-question rules, and corpus fingerprint/passage context.',
        task: 'draft',
        maxTokens: 4000,
      });
      if (!parsed) {
      ...
      const abstractShellViolated =
        abstractShellPriorCount >= RAPID_WRITE_MAX_ABSTRACT_SHELL_TITLES &&
        RAPID_WRITE_ABSTRACT_TITLE_SHELL_RE.test(String(title || '').trim());
      const meetingSpineViolated =
        meetingSpinePriorCount >= RAPID_WRITE_MAX_MEETING_SPINE_OPENINGS &&
        rapidWriteBodyOpensWithMeetingSpine(body);
      const openingTwin = rapidWriteOpeningTooSimilarToPrior(body, priorBodyHeadsRaw);
      ...
      if (
        (dupTitle ||
          abstractShellViolated ||
          meetingSpineViolated ||
          openingTwin ||
          bannedIntro ||
          reflSimilar) &&
        attempt < 3
        ...
        if (dupTitle) {
          bits.push(
            `The title **${title || '(empty)'}** collides with another title in this batch (exact duplicate or thin abstract-shell variant). Return a **distinct** title architecture.`
          );
        }
        if (abstractShellViolated) {
          bits.push(
            `The abstract **"The ___ of …"** headline quota is full (${abstractShellPriorCount} in batch). Your title must **not** use that shell family.`
          );
        }
        if (meetingSpineViolated) {
          bits.push(
            `The opening still uses a **conference-table / strategy-metrics meeting spine** while the batch quota for that container is exhausted. Rebuild the **first ~700 words** around a different stage (see assigned story pattern).`
          );
        }
        if (openingTwin) {
          bits.push(
            `The opening is **too structurally close** to another draft’s opening in this batch (same scene machine). Change setting, POV, and first crisis move.`
          );
        }
        if (bannedIntro) {
          bits.push(
      ...
      if (
        ...
        return {
          ok: false,
          error:
            'Writer output still violated duplicate-title, abstract-shell quota, meeting-spine quota, opening-similarity, banned-intro, or reflection-similarity rules after retries.',
        };
      }
...
export async function reviseRapidWriteDraft(
  ...
  const abstractShellQuotaBlock =
    abstractShellPriorCount >= RAPID_WRITE_MAX_ABSTRACT_SHELL_TITLES
      ? `\n**MANDATORY (abstract headline quota):** Other drafts already have **${abstractShellPriorCount}** title(s) using **"The Cost / Burden / Weight / Price / Paradox of …"**. Your revised title **must NOT** use that shell family.\n`
      : abstractShellPriorCount === 1
        ? `\n**Headline discipline:** Another draft already uses an abstract **"The ___ of …"** title—prefer a **different** title shape.\n`
        : '';

  const meetingSpinePriorCount = countRapidWriteMeetingSpineOpenings(priorBodyHeadsRaw);
  const meetingSpineQuotaBlock =
    meetingSpinePriorCount >= RAPID_WRITE_MAX_MEETING_SPINE_OPENINGS
      ? `\n**MANDATORY (meeting-spine quota):** ${meetingSpinePriorCount} other draft(s) already open on a **conference-table / quarterly metrics / strategy-deck** spine early. Your **opening ~700 words** must **not** repeat that container.\n`
      : meetingSpinePriorCount === 1
        ? `\n**Spine discipline:** Another draft already uses that meeting spine early—prefer a **different** container.\n`
        : '';

  const headSnipBlock =
    priorBodyHeadsRaw.length > 0
      ? `\n**Opening stretches on other drafts (do not echo the same scene machine):**\n${priorBodyHeadsRaw.map((t) => `- ${safeText(t, 280)}`).join('\n')}\n`
      : '';
...
Owner instruction:
${inst.slice(0, 6000)}
${trainingBlock}${overlapBlock}${diffBlock}${patternBlock}${sibBlock}${sibTitlesBlock}${abstractShellQuotaBlock}${meetingSpineQuotaBlock}${headSnipBlock}${sibCloseBlock}${sibReflBlock}${sibSnipBlock}`;
...
Rules: AO Living Voice. No emojis. No section labels in body. The Related (corpus) links will be re-appended by the system — do not include them in body. Treat owner examples as **intent**, not mandatory wording.`;
  ...
  try {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      ...
      const parsed = await completeJson({
        prompt: user,
        system: RAPID_WRITE_REVISE_SYSTEM,
        task: 'draft',
        maxTokens: 4200,
      });
      if (!parsed) {
      ...
      if (
        ...
        if (dupTitle) {
          bits.push(
            `The title **${title || '(empty)'}** collides with another draft in this batch. Return a **distinct** title (not a duplicate or thin abstract-shell variant).`
          );
        }
        if (abstractShellViolated) {
          bits.push(
            `The abstract **"The ___ of …"** headline quota is exhausted in this batch. Your title must **not** use that shell family.`
          );
        }
        if (meetingSpineViolated) {
          bits.push(
            `The opening still uses a **meeting-deck / metrics-table spine** while the batch quota is exhausted. Rebuild the **first ~700 words** around a different stage.`
          );
        }
        if (openingTwin) {
          bits.push(
            `The opening is **too close** to another draft’s opening. Change setting, POV, and first crisis move.`
          );
        }
        if (bannedIntro) {
          bits.push(
      ...
      if (
        ...
        return {
          ok: false,
          error:
            'Revision still violated duplicate-title, abstract-shell quota, meeting-spine quota, opening similarity, banned-intro, or reflection-similarity rules after retries.',
        };
      }
...
Output JSON only:
{
  ...
  try {
    const parsed = await completeJson({
      prompt: user,
      system: RAPID_WRITE_POLISH_SYSTEM,
      task: 'draft',
      maxTokens: 4200,
    });
    if (!parsed) return { ok: false, error: 'Polish request failed' };
    const seriesSlugPrefix = 'psychological-cost';
    const slugSuffix = safeText(parsed.slug_suffix, 80);
    const baseSlug = safeText(currentDraft.slug, 240) || `${seriesSlugPrefix}-draft`;
    const slug =
