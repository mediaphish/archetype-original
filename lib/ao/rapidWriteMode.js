...
- **Names:** Keep narrative-first scenes. **Rotate first names** across posts—do **not** lean on the same default names post after post. Within **one** post: use **at most one or two** named people unless the story truly needs more; do **not** pile several flaws or humiliations onto a single name in one piece. Names are **illustrative fiction**, not stand-ins for identifiable colleagues. A **continuing** character arc across multiple posts only when the owner is clearly running a labeled series; otherwise treat each post as **standalone** fiction with **fresh** names.
...
- **Prefer:** naming real tension the reader may already feel (e.g. where trust, freedom, or responsibility slipped)—without sounding like a workbook.`;
 ...
 * - **run_all:** overlap advisories do not block (owner asked to run the whole batch). Falsity flags still block until override.
 * - **next:** any validation flag blocks unless the seed id is in `overrides` or only falsity is cleared via do-it-anyway flow.
 */
export function rapidWriteSeedIsDraftable(seedId, validation, overrides, mode) {
  ...
  const hasFalsity = v.flags.some((f) => f.type === 'falsity');
  if (hasFalsity) return false;
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are the Writer for Archetype Original Rapid Write. JSON only. Obey the user message: reader bar, word band, adaptive AO voice, narrative-first prose, closing and reflection-question rules, and corpus fingerprint/passage context.',
          },
          { role: 'user', content: user },
        ],
      }),
