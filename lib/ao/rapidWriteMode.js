...
Rules: AO Living Voice — human, direct, observational. Not marketer, therapist, or academic. No clichés. Do not moralize. Do not mention tags or system in body. No em dashes as a stylistic crutch. User examples in instructions are **intent only** — do not copy their wording.`;
  ...
  try {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      ...
      const dupTitle = rapidWriteTitleDuplicatesPrior(title, priorTitlesRaw);
      ...
      if ((dupTitle || costOfViolated || bannedIntro || reflSimilar) && attempt < 3) {
        ...
        if (dupTitle) {
          bits.push(
            `The title **${title || '(empty)'}** duplicates another title in this batch (same normalized wording). You must return a **new** title that does not match any listed title.`
          );
        }
        if (costOfViolated) {
          bits.push(
...
export async function reviseRapidWriteDraft(
  ...
  const sibTitles = Array.isArray(batchPriorTitles)
    ? batchPriorTitles.map((s) => safeText(s, 200)).filter(Boolean).slice(-20)
    : [];
  const sibTitlesBlock =
    sibTitles.length > 0
      ? `\nOther drafts’ titles in this batch (do not retitle to match or lightly rephrase these):\n${sibTitles.map((t) => `- ${t}`).join('\n')}\n`
      : '';
...
Owner instruction:
${inst.slice(0, 6000)}
${trainingBlock}${overlapBlock}${diffBlock}${patternBlock}${sibBlock}${sibTitlesBlock}${sibCloseBlock}${sibReflBlock}${sibSnipBlock}
