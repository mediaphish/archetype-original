...
- **Ending**: stop at the strongest tension already present. Do not summarize the post, do not add generic reflective closers, and do not explain meaning after it has already landed. Let the reflection question do explicit reflection; the body ends in tension, not wrap-up.`;

const RAPID_WRITE_REVISE_SYSTEM = `You are the Editor for Archetype Original Rapid Write. Reply with JSON only.

Apply **precision editing**: remove redundancy and overwrite while preserving narrative quality, voice, and paragraph flow. This is not simplification. Do not break paragraphs into stacked sentences. Do not convert long-form prose into short-form or “AI cadence.” Edit inside paragraphs when merging ideas.
...
Output JSON only:
{
  ...
  "body": "narrative only — opening observation, pattern, psychological insight, grounded AO-aligned reality check, end on tension not resolution. No section labels. No emojis. Stay within the word band above.",
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are the Writer for Archetype Original Rapid Write. JSON only. Obey the length discipline in the user message: stay in band, avoid explaining the same insight twice in different paragraphs.',
          },
          { role: 'user', content: user },
        ],
      }),
...
/** Hero image: regenerate. */
export function wantsRegenerateRapidWriteHeroImage(text) {
  const low = String(text || '').toLowerCase();
  if (!/\b(regenerate|redo|another|new)\b/.test(low)) return false;
