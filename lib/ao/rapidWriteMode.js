 ...
 */
export const RAPID_WRITE_LENGTH_DISCIPLINE = `Length and structure (body only):
...
**Landing and stop (body — hard structure):**
- The **landing point** is when the core **psychological shift** is **clearly visible in behavior** (concrete, observable—not only claimed in the abstract).
- After that landing, the post **must end within the next one paragraph** (at most **one** paragraph after the behavioral turn, then **stop**). **More than one paragraph after the landing point is invalid**—cut until the rule is satisfied.
- After the landing: do **not** keep writing to reinforce, validate, re-explain, or expand the same idea.
- After the landing: do **not** introduce leadership instruction, how-to, lessons, or “what leaders should do.”
- After the landing: do **not** transition into teaching, advising, or summarizing. Do not add wrap-up closers or generic reflection in the body; the **reflection question** carries explicit reflection.
- The body should still end at **strong tension** where possible; the constraint above is about **where** stopping is allowed, not about adding a soft ending.`;

const RAPID_WRITE_REVISE_SYSTEM = `You are the Editor for Archetype Original Rapid Write. Reply with JSON only.

Apply **precision editing**: remove redundancy and overwrite while preserving narrative quality, voice, and paragraph flow. This is not simplification. Do not break paragraphs into stacked sentences. Do not convert long-form prose into short-form or “AI cadence.” Edit inside paragraphs when merging ideas. **Enforce the landing-and-stop rule** in the discipline below (behavioral shift visible → at most one following paragraph, then end).
...
export async function writeRapidWritePost(
  ...
  const user = `Write ONE journal post for Archetype Original. One coherent arc; depth without overwrite.
...
Output JSON only:
{
  ...
  "body": "narrative only — build to where the psychological shift is visible in behavior; after that, at most one more paragraph then stop. No reinforcement, teaching, advising, or summarizing after the landing. No section labels. No emojis. Stay within the word band and landing rule above.",
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are the Writer for Archetype Original Rapid Write. JSON only. Obey the user message: word band, no redundant paragraphs, and the landing rule — after the behavioral shift is shown, at most one more paragraph then end; no teaching or summarizing after the landing.',
          },
          { role: 'user', content: user },
        ],
      }),
...
**Structure:** If more than one paragraph appears **after** the core shift is already visible in behavior, delete or merge until **at most one** paragraph remains after that landing—no extra teaching, advice, or summary after the turn.
...
Output JSON only:
{
  ...
  "body": "revised narrative — landing rule: at most one paragraph after the behavioral shift; no teaching or summarizing after the landing",
  "reflection_question": "one specific question"
}
