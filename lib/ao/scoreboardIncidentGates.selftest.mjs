...
// --- Bug 1: fabricated completion ---
{
  ...
  // Slug/code spans must not trip fabrication patterns either
  assert.strictEqual(
    findFabricatedCompletions(
      'Cleanup: `do-we-have-any-examples-that-have-been-verified-of-the-hazing`',
      unchangedEvidence
    ).fabrications.length,
    0,
    'backticked slug with verified must not fire fabrication gate'
  );
  assert.strictEqual(
    findFabricatedCompletions(
      'Row: do-we-have-any-examples-that-have-been-verified-of-the-hazing-it-would-add-good',
      unchangedEvidence
    ).fabrications.length,
    0,
    'hyphen slug with verified must not fire fabrication gate'
  );
  assert.strictEqual(
    findFabricatedCompletions('I found new specific details in the rebuild.', unchangedEvidence)
      .fabrications.length,
    1,
    'real fabrication prose must still fire'
  );
  assert.strictEqual(
    findFabricatedCompletions('I verified the carbon monoxide detail and rebuilt the draft.', unchangedEvidence)
      .fabrications.length,
    1,
    'prose verified/rebuilt claims must still fire'
  );
}
