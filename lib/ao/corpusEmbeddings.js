...
export async function embedAndStoreDocument(doc) {
  ...
  try {
    ...
    // Keep the passage index (ao_corpus_chunks) in step with this table.
    //
    // Done here rather than at each publish site on purpose: publish-journal,
    // publish-devotional, edit-published and the backfill cron all already call
    // this function, so they inherit chunking without a second write path that
    // could drift. Dynamic import avoids a cycle — corpusChunks imports
    // generateEmbedding from this module.
    //
    // A chunking failure is logged and swallowed: the document embed succeeded
    // and the publish must not fail behind it. The nightly backfill re-runs.
    try {
      const { embedAndStoreChunks } = await import('./corpusChunks.js');
      const chunkResult = await embedAndStoreChunks(doc);
      if (!chunkResult.ok) {
        console.error(
          `[corpusEmbeddings] Chunk indexing incomplete for ${slug}:`,
          `${chunkResult.chunks} stored, ${chunkResult.failed} failed`
        );
      }
    } catch (chunkErr) {
      console.error(
        `[corpusEmbeddings] Chunk indexing threw for ${slug}:`,
...
export async function seedCorpusFromKnowledgeJson(knowledgeDocs, options = {}) {
  const { batchSize = 5, delayMs = 200, onProgress, skipExisting = true } = options;
  ...
  if (skipExisting) {
    try {
      const { data: existing } = await supabaseAdmin.from('ao_corpus_embeddings').select('slug');
      if (Array.isArray(existing)) {
        existingSlugs = new Set(existing.map((row) => String(row.slug || '').trim()).filter(Boolean));
      }

      // "Already embedded" has to mean passages too.
      //
      // Measured 2026-09-23: 194 of 609 documents had a row in
      // ao_corpus_embeddings and no rows in ao_corpus_chunks, among them 63
      // journal posts and 95 devotionals. Everything indexed before passage
      // search existed was skipped here on every run since, so Archy's search
      // and Auto's search_corpus could not see a third of the corpus. A
      // document without passages is not done, whatever the document table says.
      ...
      for (;;) {
        ...
        for (const row of rows || []) chunked.add(String(row.slug || '').trim());
        if (!rows || rows.length < page) break;
        from += page;
      }
      const missingPassages = [...existingSlugs].filter((slug) => !chunked.has(slug));
      for (const slug of missingPassages) existingSlugs.delete(slug);
      if (missingPassages.length) {
        console.log(`[seedCorpus] ${missingPassages.length} document(s) have no passages indexed; re-embedding those.`);
      }
    } catch (err) {
      console.warn('[seedCorpus] Could not load existing slugs — will re-embed all:', err?.message || err);
    }
  }
  ...
  for (let i = 0; i < knowledgeDocs.length; i += batchSize) {
    ...
    await Promise.all(
      batch.map(async (doc) => {
        processed++;
        const slug = String(doc.slug || '').trim();
        if (skipExisting && slug && existingSlugs.has(slug)) {
          skipped++;
          succeeded++;
          return;
        }
    ...
    if (onProgress) {
      onProgress({ processed, succeeded, failed, skipped, total: knowledgeDocs.length });
    }
