...
// painterly scenes and succeed; the cards, his face plus the brand type plus
// the logo, are the ones he ends up making by hand. Before this fallback
// existed a brand-new card matched nothing in the chain and generated with zero
...
//
// This case used to pass content_type 'journal_header' while its own comment
// said "quote card". That conflation IS the 2026-08-31 bug: a Vietnam
// battlefield header requested as a journal_header was handed an approved
// likeness card and put Bart in the frame. The fallback is opt-in by
// content_type now, so a card has to say it is a card.
{
  const urls = await resolveGenerateImageReferenceUrls(
    { intent: 'header_image_for_post', slug: 'a-brand-new-card', content_type: 'quote_card' },
    {
      email: 'bart@archetypeoriginal.com',
...
// --- a scene header must NOT inherit a likeness card ---
//
// The regression. Auto asked for "an American infantry officer in his early
// 40s, lean build, close-cropped dark hair" in the Ia Drang Valley and the
// server attached manual-upload-the-signal-i-was-actually-watching.jpg, which
// is Bart's face. Generating with no reference is the correct outcome here:
// every published series header is a painterly scene with no likeness in it.
{
  const urls = await resolveGenerateImageReferenceUrls(
    { intent: 'header_image_for_post', slug: 'the-account-under-fire', content_type: 'journal_header' },
    {
      email: 'bart@archetypeoriginal.com',
      _testGetBySlug: async () => ({ ok: true, data: { image_url: null } }),
      _testLoadPriorPartImageUrl: async () => null,
      _testLoadRecentApprovedHeaderUrl: async () => {
        throw new Error('scene header must not reach the approved-card fallback');
      },
    }
  );
  assertEqual(urls, [], 'scene header generates with no reference rather than inheriting a face');
}
