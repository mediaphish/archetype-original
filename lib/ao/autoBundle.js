/**
 * AO — package a finished post into a journal file, channel drafts, and pull-quote
 * companions.
 *
 * The quality alarm and the pull-quote extraction run on Claude through
 * textModel.js. Both were raw fetches to gpt-4o-mini, and OpenAI is kept for
 * image creation only.
 */
...
async function modelJson(prompt, { task = 'classify', maxTokens = 1500 } = {}) {
  if (!textModelConfigured()) return null;
  try {
    return await completeJson({ prompt, task, maxTokens, effort: 'low' });
  } catch {
    return null;
  }
}
...
export async function detectQualityAlarm({ text } = {}) {
  ...
  const parsed = await modelJson(`You are a strict quality alarm for a finished post.
...
Post:
${JSON.stringify(source)}`, { task: 'classify' });
...
export async function extractPullQuotes({ text, max = 4 } = {}) {
  ...
  const parsed = await modelJson(`Extract ${Math.max(2, Math.min(8, Number(max || 4)))} strong pull quotes from this finished post.
...
Post:
${JSON.stringify(source)}`, { task: 'extract', maxTokens: 1500 });
...
export function buildJournalMarkdown({ title, text, imageFilename = '' } = {}) {
  ...
  const data = {
    ...
    applications: [],
    related: [],
    // Draft until Bart explicitly approves Import publish or corpus/publish live token
    status: 'draft',
  };
...
export async function buildAutoBundle({
  ...
  return {
    ...
    schedule_suggestion: await buildScheduleSuggestionForChannels(
      Array.isArray(channels) && channels.length ? channels : ['linkedin', 'facebook', 'instagram', 'x']
    ),
