 ...
 * (no HTTP loopback).
 *
 * AO mark: public/images/ao-logo.svg at bottom center, 50% white opacity.
 */
...
async function loadAOMark() {
  ...
  const svgPath = join(process.cwd(), 'public', 'images', 'ao-logo.svg');
  const svgBuffer = readFileSync(svgPath);
  _aoMarkImage = await loadImage(svgBuffer);
  return _aoMarkImage;
}
...
async function drawAOMark(ctx, canvasWidth, canvasHeight) {
  try {
    ...
    // Target height for the mark
    const markHeight = 80;
    // Preserve aspect ratio: SVG viewBox is 440.3 x 480.05
    const aspectRatio = 440.3 / 480.05;
    const markWidth = markHeight * aspectRatio;
    // Center horizontally, position 60px from bottom
