...
// Fonts, wrapping and emphasis live in cardText.js so a renderer that only sets
// type does not have to import this file — and with it supabase-admin, which
// throws on a missing SUPABASE_URL before any glyph is measured.
import {
  DISPLAY_FONT,
  TEXT_FONT,
  ensureFont,
...
export { DISPLAY_FONT, TEXT_FONT, ensureFont, parseEmphasis, wrapSegments };
...
async function generateSplitCardBuffer(spec) {
  ...
  // Narrow. At 0.30 the fade reached a third of the way across the artwork and
  // greyed out the subject's near side. The artwork prompt already keeps him in
  // the outer 40%, so the blend only has to cover the seam.
