...
async function generateSplitCardBuffer(spec) {
  ...
  const blocks = spec.lines.map((line) => {
    ctx.font = `${line.weight} ${line.size}px Inter, Arial, sans-serif`;
    const text = spec.uppercase ? line.text.toUpperCase() : line.text;
    const wrapped = wrapText(ctx, text, textMaxW);
    return { line, wrapped, lineH: Math.round(line.size * 1.12) };
  });

  const markH = Math.round(H * 0.105);
  // Generous gap so the attribution line does not crowd the lockup.
  const markBlockH = markH + Math.round(H * 0.14);
  const totalTextH = blocks.reduce(
    (sum, b) => sum + b.wrapped.length * b.lineH + (b.line.gap_after || 0),
    0
  );

  // Type block sits above the lockup, both anchored to the panel's optical
  // centre rather than the frame's.
  let y = Math.max(padX + blocks[0].lineH, (H - markBlockH - totalTextH) / 2 + blocks[0].lineH);

  for (const b of blocks) {
    ctx.font = `${b.line.weight} ${b.line.size}px Inter, Arial, sans-serif`;
  ...
  if (spec.mark !== 'hidden') {
    try {
      ...
      if (spec.wordmark) {
        ...
        ctx.font = `bold ${wmSize}px Inter, Arial, sans-serif`;
        ctx.fillText('ARCHETYPE', textX, markY + markH + wmSize + 10);
        ctx.font = `normal ${Math.round(wmSize * 0.86)}px Inter, Arial, sans-serif`;
