...
// apart.
//
// Note for anyone extending this: there is NO wordmark asset. Everything on
// hand — the stored SVG, public/brand/*.svg, public/images/ao-logo* — is the
// symbol alone. An earlier version of this file set "ARCHETYPE ORIGINAL" as
// type in the card's display face, which invented brand furniture in the wrong
// typeface. Do not do that again. If the words are wanted, the lockup has to be
 ...
 * none.
 */
async function loadAOMark(filename, { color = '#ffffff', height = 128 } = {}) {
  const cacheKey = `${color}@${Math.round(height)}`;
  if (_aoMarkCache[cacheKey]) return _aoMarkCache[cacheKey];

  const { loadImage } = await import('@napi-rs/canvas');

  try {
    const { getDefaultLogoUrl } = await import('./brandLogos.js');
    const url = await getDefaultLogoUrl({ background: 'dark' });
    if (url && /\.svg(\?|$)/i.test(url)) {
      const res = await fetch(url);
      if (res.ok) {
        const svg = tintSvg(await res.text(), color);
    ...
    console.warn('[generateQuoteCardImage] brand SVG unavailable, using bundled mark:', err?.message || err);
  }
...
function parseCardTag(cardTagContent, openingAttrs) {
  ...
  return {
    ...
    // Off by default: there is no wordmark asset, and setting the words as type
    // put the brand in the wrong typeface.
    wordmark: String(attrs.wordmark ?? 'false') === 'true',
    lines,
  };
}
