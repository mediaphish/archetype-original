...
// Ranking lives apart so it can be tested without a database client.
import { keyPhrasesFrom, mergeHits, LEXICAL_SCORE } from './corpusHybridRank.js';

export { keyPhrasesFrom, mergeHits };

// 0.3 was permissive enough that a question about one essay pulled in a dozen
// service FAQs ("Can I hire Bart to speak to my team?") scoring 0.45–0.57.
// Measured across six real questions, nothing below ~0.44 was useful.
const SIMILARITY_THRESHOLD = 0.4;
const MAX_RESULTS = 12;
const MAX_PER_DOC = 3;
// No single document TYPE may dominate the result set — the same idea as
// MAX_PER_DOC, one level up. Raising the floor alone could not fix the FAQ
// flooding: those FAQs scored higher on one question (0.565) than genuinely
// relevant essays did on another (0.516), so any flat cutoff that removed the
// noise also removed real answers. Down-weighting FAQs by content would just
// recreate the hand-tuned +40 "canonical" bias this replaced, in reverse.
...
export async function searchCorpusChunks(queryText, options = {}) {
  ...
  const maxPerDoc = options.maxPerDoc ?? MAX_PER_DOC;
  const maxPerType = options.maxPerType ?? MAX_PER_TYPE;
  ...
  try {
    ...
    const semantic = data || [];
    if (options.lexical === false) return semantic;

    const lexical = await lexicalChunkHits(queryText, { exclude: semantic });
    if (!lexical.length) return semantic;
    return mergeHits(semantic, lexical, { maxResults, maxPerDoc, maxPerType });
  } catch (err) {
...
async function lexicalChunkHits(queryText, { exclude = [], limit = 6 } = {}) {
  const phrases = keyPhrasesFrom(queryText);
  if (!phrases.length) return [];

  const seen = new Set(exclude.map((h) => `${h.slug}:${h.chunk_index}`));
  const out = [];
  for (const phrase of phrases) {
    const { data, error } = await supabaseAdmin
      .from('ao_corpus_chunks')
      .select('slug, chunk_index, title, doc_type, categories, summary, content')
      .ilike('content', `%${phrase.replace(/[%_]/g, ' ')}%`)
      .limit(limit);
    if (error) {
      console.warn('[corpusChunks] lexical pass failed:', error.message);
      return out;
    }
    for (const row of data || []) {
      const key = `${row.slug}:${row.chunk_index}`;
      if (seen.has(key)) continue;
      seen.add(key);
      out.push({ ...row, similarity: LEXICAL_SCORE, matched_phrase: phrase });
    }
