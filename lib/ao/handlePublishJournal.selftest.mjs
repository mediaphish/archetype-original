...
async function insertDraft(status, { withCaptions = false } = {}) {
  const captions = withCaptions
    ? `\n\n[SOCIAL_CAPTIONS]
[CAPTION platform="linkedin_personal"]
LI caption
[/CAPTION]
[CAPTION platform="instagram_business"]
IG caption
[/CAPTION]
[CAPTION platform="instagram_personal"]
IG personal caption
[/CAPTION]
[CAPTION platform="facebook_business"]
FB caption
[/CAPTION]
...
[/SOCIAL_CAPTIONS]`
  ...
  const { data, error } = await contentDrafts()
    .insert({
      ...
      content: `# Selftest\n\nBody for publish gate test.${captions}`,
...
async function main() {
  ...
  try {
    ...
    await insertDraft('approved', { withCaptions: false });
    const noCaptions = await handlePublishJournal({ slug: TEST_SLUG }, { email: TEST_EMAIL });
    assert.strictEqual(noCaptions.ok, false, 'approved without captions must refuse');
    assert.strictEqual(noCaptions.gate, 'captions_required');
    assert.strictEqual(publishCalls.length, 0);
    await cleanup();

    await insertDraft('approved', { withCaptions: true });
    const published = await handlePublishJournal({ slug: TEST_SLUG }, { email: TEST_EMAIL });
    assert.strictEqual(published.ok, true, 'approved draft with captions must publish');
