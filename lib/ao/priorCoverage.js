...
export function priorCoverageGateResult(matches) {
  ...
  return {
    ...
    error:
      ...
      `${lines.join('\n')}\n\n` +
      'Bart does not want pitches that repeat or overlap published work. Do not show him this one. ' +
      'Build a genuinely different angle whose central claim is not the argument of any post above, and present that instead. ' +
      'Do not describe this pitch or its draft as something he already has. ' +
      'Only if Bart himself asks for this exact angle despite the overlap, call the tool again with prior_coverage_cleared=true.',
  };
}
