...
export function priorCoverageGateResult(matches) {
  ...
  return {
    ...
    error:
      ...
      `${lines.join('\n')}\n\n` +
      'Before pitching anything, fetch_full_text the closest post and compare its argument to this one. ' +
      'Then tell Bart plainly, at the top of your reply: name the post, its date, the argument it already made, ' +
      'and what, if anything, this pitch adds. If it only restates that argument with new data, say so and ' +
      'bring a genuinely different angle instead. Do not present the draft as pre-existing or as something he already has. ' +
      'Only if Bart then says to go ahead with this angle, call the tool again with prior_coverage_cleared=true.',
  };
}
