...
export async function buildScheduleSuggestionForChannels(channels = CHANNEL_ORDER) {
  ...
  const { hour: prefH, minute: prefM } = await resolveSuggestedLocalClock(hintPlatforms);
  const minute = Number.isFinite(prefM) ? prefM : benchmarkMinuteDefault();
  const tz = getOwnerTimeZone();

  let slot = nextUtcAtOwnerLocalClock(new Date(anchorMs), prefH, minute, tz);
  if (slot.getTime() <= now.getTime()) {
    slot = nextUtcAtOwnerLocalClock(new Date(now.getTime() + MIN_GAP_MS), prefH, minute, tz);
  }
