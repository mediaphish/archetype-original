...
async function main() {
...

  {
    const form = buildImagesEditsFormData({
      prompt: 'test',
      size: '1536x1024',
      images: [{ buffer: TINY_PNG, mime: 'image/png', filename: 'ref.png' }],
    });
    let inputFidelity = null;
    for (const [key, val] of form.entries()) {
      if (key === 'input_fidelity') inputFidelity = val;
    }
    assert.strictEqual(inputFidelity, 'high', 'edits must default input_fidelity to high');
  }
  ...
  assert.strictEqual(inputFidelity, 'high', 'live edits call must send input_fidelity high');
