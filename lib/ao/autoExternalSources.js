...
const MAX_REBUILD_TOTAL_MS = 22_000;
...
export async function buildExternalSourcesFromPrompt({
  ...
  const cleanTarget = Math.max(5, Math.min(40, Number(targetCount || 20)));
  const trimmed = candidates.slice(0, Math.max(10, Math.min(120, Number(maxCandidates || 50))));
