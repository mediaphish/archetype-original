/**
 * AO Scout source rebuild — propose and verify a feed allowlist.
 *
 * The proposal step is a judgement call with reasoning, so it runs on Claude
...
async function sourcesJson(prompt) {
  if (!textModelConfigured()) return null;
  // The rebuild runs inside a short-lived request, so the same time budget
  // applies, and the helper cancels the request when it expires.
  let content = '';
  try {
...
export async function buildExternalSourcesFromPrompt({
  ...
  const parsed = await sourcesJson(prompt);
