...
{
  "candidates": [
    { "name": "string", "feed_url": "https://...rss-or-atom...", "homepage_url": "https://..." }
  ]
}

Notes:
- Return raw JSON only (no markdown, no code fences like ```json).
- Prefer including a direct feed_url for each source (RSS or Atom).
...
export async function buildExternalSourcesFastFromPrompt({
  ...
  return {
    ok: true,
    verified: out,
    note: 'Fast rebuild: saved feed links without live verification. The next scan will confirm which ones work.',
  };
}
