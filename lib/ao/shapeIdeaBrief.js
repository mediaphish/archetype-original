/**
 * AO Ideas — shape a raw idea into a decision-ready brief.
 *
 * The brief is written in Bart's voice and feeds what he publishes, so it runs
 ...
 * seen the standard. The plain fallback brief still applies when no text model
 * is configured.
 */
...
async function ideaBriefJson(prompt) {
  if (!textModelConfigured()) return null;
  // A brief that seeds a post, not the post itself. Sonnet is enough.
  return completeJson({ prompt, task: 'analysis', maxTokens: 1600 });
}
...
export async function shapeIdeaBrief({ rawInput, sourceUrl } = {}) {
  ...
  try {
    const parsed = await ideaBriefJson(prompt);
