...
## MARKDOWN FORMATTING — CLEAN BEFORE DELIVERY

Check the full text for these formatting problems and fix every one before returning.

### CITATION INTEGRATION
Web research citations often arrive with leading spaces or as floating indented lines. These render as code blocks or indented text in markdown. Find any line that:
- Begins with one or more spaces before the text starts
- Appears to be a citation or research reference floating on its own line between paragraphs
- Has inconsistent spacing around it compared to surrounding paragraphs

Fix them by integrating the citation text as a plain prose sentence inside the paragraph it belongs to. Remove all leading spaces. The sentence should read as part of the paragraph, not as a separate indented block.

### BLANK LINES BETWEEN PARAGRAPHS
Every paragraph must be separated by exactly one blank line. No more. Fix:
- Two or more consecutive blank lines — reduce to one
- Paragraphs that run together with no blank line — add one

### NO LEADING SPACES ON ANY LINE
No line of prose should begin with spaces or tabs. If a line begins with whitespace, remove it. The only exception is intentional code blocks formatted with triple backticks, which must be left exactly as written.

### SECTION HEADERS
Section headers (## and ###) must have exactly one blank line before them and one blank line after them. Fix any header that is missing its surrounding blank lines.

These formatting fixes apply to the prose content only. Do not reformat content inside signal tags.
