...
// Voice review fires on conversational replies and short content.
// For long-form posts (journal entries, series parts), the cost of running a full
// second Anthropic generation is too high — it pushes the chat route past its
// timeout budget. Long content gets the self-review pass that Auto runs internally;
// the server-side rewrite pass is reserved for shorter output where it is safe.
const MIN_LENGTH_FOR_REVIEW = 800;
const MAX_LENGTH_FOR_REVIEW = 4000;
...
export async function reviewAndCleanVoice(reply) {
  // Short conversational replies do not need review.
  // Long-form content (journal entries, series posts) skips the server-side rewrite
  // pass to stay within the Vercel function timeout budget. Auto's internal self-review
  // pass (in the system prompt) handles these; the server-side pass is for shorter output.
  if (!reply || reply.length < MIN_LENGTH_FOR_REVIEW) {
    return reply;
  }
  if (reply.length > MAX_LENGTH_FOR_REVIEW) {
    console.log('[Voice Review] Long-form content — skipping rewrite pass, relying on Auto self-review');
    return reply;
  }
