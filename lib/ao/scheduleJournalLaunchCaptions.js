...
import { assertInstagramHasImage } from './instagramImageGate.js';
...
export async function scheduleJournalLaunchCaptions({
  ...
  // draftImageUrl is read from the draft as it stands right now, so this fires
  // whenever captions are scheduled before the header image is attached.
  const igGate = assertInstagramHasImage({
    channels: providedCaptions,
    imageUrl: draftImageUrl,
    slug,
  });
  if (!igGate.ok) return igGate;
