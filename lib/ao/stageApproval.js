...
export async function recordStageApproval({ email, slug, stage, captions = null }) {
  ...
  if (gate === 'captions') {
    ...
    // Bart's approval stands whatever is missing, but the gap is named. The two
    // he posts by hand were dropped from every caption list in one session
    // (2026-09-21): "At what point do you begin understanding that I need
    // LinkedIn Business and Facebook Personal captions so I can post them?"
    missingChannels = buildCaptionsPanel({})
      .map((c) => c.key)
      .filter((key) => !approvedCaptions[key]);
  ...
  return {
    ...
    ...(missingChannels.length ? { missing_channels: missingChannels } : {}),
  };
}
