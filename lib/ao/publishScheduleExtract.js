 ...
 *
 * That model call runs on Claude through textModel.js. It was a raw fetch to
 * gpt-4o-mini, and OpenAI is kept for image creation only.
 */
...
- user_facing_note: optional one short sentence for the publish plan (e.g. "Using 2 days between posts; posts around 10:00 AM your time.")`;
  ...
  try {
    const parsed = await completeJson({
      prompt: user,
      system: 'You extract structured scheduling preferences. Reply with JSON only. No markdown.',
      task: 'extract',
      maxTokens: 1500,
      effort: 'low',
    });
