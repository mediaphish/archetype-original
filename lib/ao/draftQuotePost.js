 ...
 *
 * This writes in Bart's voice, so it runs on Claude through textModel.js. It ran
 * on gpt-4o-mini until 2026-09-23, which meant his voice guardrails were
 * inspecting prose from a model that had never seen the standard. Returns plain
 * fallbacks when no text model is configured.
 */
...
export async function draftQuotePost(quoteRow) {
  ...
  if (!textModelConfigured()) return defaultDrafts(quote);

  const source = safeText(quoteRow?.source_slug_or_url, 200) || 'External';
  const prompt = `You write social posts in Bart's voice: clear, grounded, leadership-forward, not hypey, not salesy.\n\nCreate drafts for the SAME underlying idea using this quote:\n\"${quote}\"\nSource: ${source}\n\nReturn ONLY one JSON object (no markdown). Keys:\n- drafts_by_channel: { linkedin: string, facebook: string, instagram: string, x: string }\n- hashtags_by_channel: { linkedin: string[], facebook: string[], instagram: string[], x: string[] }\n- first_comment_suggestions: { linkedin: string|null, facebook: string|null, instagram: string|null, x: string|null }\n\nRules:\n- LinkedIn: 600 chars max, up to two short paragraphs when the idea needs room. Interpret or contextualize—do not paste a fragment of the quote as a substitute for commentary.\n- Facebook: 500 chars max, conversational; same standard as LinkedIn (clarity over compression).\n- Instagram: Often several sentences (up to ~7 when needed); the quote may appear on an image. The caption must interpret, explain, or enhance—not repeat the quote, and not use a partial quote as the caption. Not generic praise.\n- X: ~200 characters is a typical target when it serves clarity; hard max 260. Same interpretive standard—not a shortened quote and not “compression for its own sake.”\n- Hashtags: 0-6 per channel, no duplicates across that channel.\n- First comment: optional; if used, it should add one extra insight or question.\n- Do NOT include links unless you truly need one (assume no link).\n- Avoid emojis.\n`;

  try {
    const parsed = await completeJson({ prompt, task: 'voice', maxTokens: 1500 });
    if (!parsed) return defaultDrafts(quote);
