...
/**
 * Recover corpus_pull_quotes from the most recent assistant message that stored them
 * (e.g. thread state incomplete after navigation).
 * @param {Array<{ role?: string, meta?: object }>} messages Chronological order (oldest first), as in Auto chat load.
 * @returns {Array<object>|null}
 */
export function restoreCorpusPullQuotesFromMessages(messages) {
  if (!Array.isArray(messages)) return null;
  for (let i = messages.length - 1; i >= 0; i -= 1) {
    const row = messages[i];
    if (row.role !== 'assistant') continue;
    const meta = row.meta && typeof row.meta === 'object' ? row.meta : null;
    const cq = meta?.corpus_pull_quotes;
    if (Array.isArray(cq) && cq.length) return cq;
  }
  return null;
}
...
export function buildThreadStateSnapshot(state) {
  ...
  if (state.rapid_write?.active) {
    ...
    if (draftsBy && typeof draftsBy === 'object') {
      ...
      snap.rapid_write_drafts = keys.map((kid) => {
        ...
        const hi = d?.hero_image && typeof d.hero_image === 'object' ? d.hero_image : null;
        return {
          ...
          hero_image_status: hi?.status ? safeText(hi.status, 40) : null,
          hero_image_url: hi?.url && String(hi.url).startsWith('http') ? safeText(hi.url, 500) : null,
        };
      });
    }
  }
  ...
  if (state.path_state && typeof state.path_state === 'object') {
    if (state.path_state.confirmed_path) snap.confirmed_path = safeText(state.path_state.confirmed_path, 60);
    if (state.path_state.pending_path) snap.pending_path = safeText(state.path_state.pending_path, 60);
  }
  if (state.memory_profile && typeof state.memory_profile === 'object') {
    const prefs = state.memory_profile.preferences && typeof state.memory_profile.preferences === 'object'
      ? state.memory_profile.preferences
      : {};
    const clar = state.memory_profile.clarified_answers && typeof state.memory_profile.clarified_answers === 'object'
      ? state.memory_profile.clarified_answers
      : {};
    snap.memory_profile = {
      default_response_depth: safeText(prefs.default_response_depth, 40),
      execution_style: safeText(prefs.execution_style, 60),
      failure_recovery: safeText(prefs.failure_recovery, 80),
      voice_from_corpus: !!prefs.voice_from_corpus,
      last_active_path: safeText(state.memory_profile.last_active_path, 60),
    };
    const vte = state.memory_profile.voice_training_extraction && typeof state.memory_profile.voice_training_extraction === 'object'
      ? state.memory_profile.voice_training_extraction
      : null;
    if (vte?.digest_one_line) {
      snap.voice_training_digest = safeText(vte.digest_one_line, 400);
    }
    if (safeText(clar.voice_cadence, 20)) snap.voice_cadence_hint = safeText(clar.voice_cadence, 280);
    if (safeText(clar.voice_avoid, 20)) snap.voice_avoid_hint = safeText(clar.voice_avoid, 280);
  }
  if (state.series_focus && typeof state.series_focus === 'object' && state.series_focus.label) {
    snap.series_focus_label = safeText(state.series_focus.label, 80);
  }
  if (Array.isArray(state.quote_card_rejected_indices) && state.quote_card_rejected_indices.length) {
    snap.quote_card_rejected_indices = state.quote_card_rejected_indices.slice(0, 40);
  }
  return snap;
}
...
export function workflowHintFromState(state) {
  if (state?.rapid_write?.active) {
    ...
    return (
      ...
      `System-handled phrases when used clearly: Run all seeds, Next seed, do it anyway, Exit Rapid Write, Agent Training:, generate/create hero images (Rapid Write or all drafts), regenerate hero image [seed], approve hero image [seed], editor/polish/register pass or second pass (Rapid Write or seed id) for a manual register polish.`
    );
  }
