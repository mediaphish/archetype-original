...
export function workflowHintFromState(state) {
  if (state?.rapid_write?.active) {
    ...
    return (
      ...
      `System-handled phrases when used clearly: Run all seeds, Next seed, do it anyway, Exit Rapid Write, Agent Training:, generate/create hero images (Rapid Write or all drafts), regenerate hero image [seed], approve hero image [seed].`
    );
  }
