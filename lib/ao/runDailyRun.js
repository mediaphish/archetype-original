...
export async function runDailyRun() {
  ...
  try {
    ...
    const defaultLogoUrl = await getDefaultLogoUrl({ background: 'dark' });
    let drafted = 0;
    for (const id of keep) {
      ...
      if (decision.best_move === 'pull_quote_card' && decision.quote_card_worthy && decision.pull_quote) {
        const rendered = renderQuoteCardSvg({ quote: decision.pull_quote, sourceName: row.source_name || row.source_title || '', logoUrl: defaultLogoUrl });
        if (rendered.ok) {
          quoteCard = {
    ...
    try {
      ...
      for (const r of internalRows || []) {
        ...
        let quoteCard = null;
        if (decision.best_move === 'pull_quote_card' && decision.quote_card_worthy && decision.pull_quote) {
          const rendered = renderQuoteCardSvg({ quote: decision.pull_quote, sourceName: r.source_name || r.source_title || 'Archetype Original', logoUrl: defaultLogoUrl });
          if (rendered.ok) {
            quoteCard = {
              quote_card_template: rendered.template,
              quote_card_svg: rendered.svg,
