...
export async function continueAfterClientToolUse({
  ...
  while (loops < maxLoops) {
    ...
    for (const use of clientUses) {
      ...
      // Timed, because a stalled turn used to be undiagnosable. On 2026-08-25 a
      // revision turn logged nothing at all between the draft injection and the
      // user's retry almost three minutes later: the tool loop is silent, so
      // there was no way to tell which call was slow, or whether anything was
      // running at all. Elapsed milliseconds per tool makes the next occurrence
      // self-diagnosing.
      const toolStartedAt = Date.now();
      let result;
      try {
        result = await executeAutoTool(use.name, use.input || {}, toolContext);
      } finally {
        const ms = Date.now() - toolStartedAt;
        // Warn on anything approaching the client's 90 second stall threshold,
        // since that is the one worth finding in a log.
        const log = ms >= 30_000 ? console.warn : console.log;
        log(`[Auto V2 tool] ${use.name} ${ms}ms`);
      }
