...
{
  // Full block schedules every required channel (explicit times avoid
  // scheduler/DB import). instagram_personal was added to the required set
  // after this fixture was written, so the block was silently one channel short
  // and this case failed on `incomplete` against a correct parser.
...
[CAPTION platform="instagram_personal" scheduled_time="${t}"]IG personal copy[/CAPTION]
...
`;
  ...
  assert(parsed.rows.length === 5, `expected 5 rows, got ${parsed.rows.length}`);
