 ...
 * @returns {Promise<{ alignment_score: number, clarity_score: number, shareability_score: number, brand_fit_score: number, depth_score: number, composite_score: number, classification: string, caption_suggestions: string[] }>}
 */
export async function evaluateCandidate(candidate) {
  ...
  const prompt = `You are evaluating a short quote for social sharing and editorial fit.

Quote: "${text.slice(0, 500)}"
Source: ${source}

Respond with ONLY a single JSON object (no markdown, no code block) with these exact keys and integer values 0-100: alignment_score, clarity_score, shareability_score, brand_fit_score, depth_score, composite_score. Also include: "classification" (string, e.g. "quote", "insight", "story") and "caption_suggestions" (array of 1-3 short caption ideas for social posts).`;
