/**
 * AO — Evaluate a quote candidate (scores, classification, caption suggestions).
 *
...
function heuristicLeadershipRelevant(text) {
  const s = String(text || '').toLowerCase();
  if (!s) return { score: 0, isRelevant: false };

  const strong = ['leadership', 'leader', 'management', 'manager', 'servant leadership', 'executive'];
  const support = [
    'culture',
    'team',
    'accountability',
  ...
  const score = (hasStrong ? 55 : 0) + Math.min(45, supportHits * 9);

  // Conservative: require explicit leadership OR a dense set of adjacent signals.
  const isRelevant = hasStrong || supportHits >= 4;
...
export async function evaluateCandidate(candidate) {
  ...
  const defaults = {
    ...
    leadership_relevance_score: Math.min(100, Math.max(0, Number(h.score) || 0)),
    is_leadership_related: !!h.isRelevant,
  };
