...
function extractCardsFromPlainTextPairs(blob) {
  ...
  while ((m = flexRe.exec(s)) !== null) {
    cards.push({ card_spec: m[1].trim(), line1: '', line2: '' });
  }
...
function userRequestedImageGeneration(userMessage) {
  ...
  const patterns = [
    ...
    /^\s*go\s*$/i,
    /^\s*(yes|yep|yeah|do it|fire|let's go|let's do it)\s*$/i,
  ];
...
export async function appendQuoteCardImagesToReplyIfNeeded({
  ...
  // Do not fire the quote card renderer if the CURRENT reply or user message
  // is a DALL-E design image request. Only block on current context, not thread history.
  // Thread history may contain DALL-E images from earlier work but that does not
  // mean the current request is for a design image.
  ...
  const imageResults = await generateAllPngs(cardSpecs, threadId, { forceNew: forceRegen, theme: cardTheme });
