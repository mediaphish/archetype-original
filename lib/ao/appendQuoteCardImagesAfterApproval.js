...
// A negation word within ~20 characters before "image(s)"/"png(s)" flips the meaning --
// "not talking about an image yet" is a denial, not a request. Confirmed live: Bart wrote
// exactly this sentence and it was misread as a request for image generation.
const NEGATED_IMAGE_REQUEST_PATTERN =
  /\b(not|isn'?t|don'?t|doesn'?t|didn'?t|wasn'?t|weren'?t|never|no|aren'?t)\b[\s\S]{0,20}\b(images?|pngs?)\b/i;

function userRequestedImageGeneration(userMessage) {
  const u = String(userMessage || '').toLowerCase();
  if (!u.trim()) return false;
  if (NEGATED_IMAGE_REQUEST_PATTERN.test(u)) return false;
  return IMAGE_REQUEST_PATTERNS.some((re) => re.test(u));
}
...
export async function appendQuoteCardImagesToReplyIfNeeded({
  ...
  if (cardSpecs.length === 0) {
    if (wantImages) {
      const note =
        "\n\nI don't see quote card text (Power says / Servant leadership says pairs) earlier in this conversation to render into images. Confirm the card list or captions are still above, then ask again.\n";
