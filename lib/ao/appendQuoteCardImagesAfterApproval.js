...
async function generateAllPngs(cardSpecs, threadId) {
  ...
  // This is the reliable guard — it checks the actual saved content
  // rather than relying on in-memory state timing.
  if (threadId) {
    try {
...
export async function appendQuoteCardImagesToReplyIfNeeded({
  ...
  const wantImages = userRequestedImageGeneration(userMessage);
  ...
  const imageResults = await generateAllPngs(cardSpecs, threadId);
