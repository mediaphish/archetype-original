...
async function generateAllPngs(cardSpecs) {
  return (
    await Promise.all(
...
export async function appendQuoteCardImagesToReplyIfNeeded({ userMessage, priorMessages, reply }) {
  ...
  if (cardSpecs.length === 0) return r;

  const imageResults = await generateAllPngs(cardSpecs);
  if (imageResults.length === 0) return r;
