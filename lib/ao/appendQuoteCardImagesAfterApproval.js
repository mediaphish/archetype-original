...
export async function appendQuoteCardImagesToReplyIfNeeded({
  ...
  // Do not fire the quote card renderer if this is a journal/design image thread.
  // DALL-E handles images in those threads. The quote card renderer only fires
  // when Power says / Servant leadership pairs are present in the thread.
  const isDesignThread =
    /\[DALLE_GENERATE/i.test(String(reply || '')) ||
    /\[IMAGE_GENERATED/i.test(
      Array.isArray(priorMessages)
        ? priorMessages.map((m) => String(m.content || '')).join('\n')
        : ''
    );
  if (isDesignThread) return r;
