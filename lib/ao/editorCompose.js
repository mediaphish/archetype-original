/**
 * AO Editor — channel drafts, hashtags, and optional first comments.
 *
 * This writes prose that goes out under Bart's name, so it runs on Claude
 * through textModel.js. It ran on gpt-4o-mini until 2026-09-23, which meant his
 * voice guardrails were checking writing from a model that had never seen the
 * standard. Falls back to plain drafts when no text model is configured.
 */
...
async function editorJson(prompt) {
...
- Instagram: <=400 chars. The quote image already shows the pull quote—do not repeat the full quote. Use the caption to sharpen why it lands: name the tension, risk, or “I’ve seen this” recognition. Answer what a reader might silently ask, or name the consequence if leaders ignore it. Optional: one short honest question at the end. Not generic praise or filler.
...
`;
  ...
  try {
    const parsed = await editorJson(prompt);
