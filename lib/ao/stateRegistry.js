...
export const RESOURCES = {
  drafts: {
    ...
    order: 'updated_at',
    max: 50,
    // Lifecycle only. Content is deliberately not writable here — rewriting a
    // post is save_draft's job, where drafting rules and versioning apply.
    mutable: ['status', 'scheduled_publish_at', 'published_at', 'approved_at', 'image_url', 'summary'],
    describe:
  ...
  scheduled_posts: {
    ...
    order: 'scheduled_at',
    max: 100,
    mutable: ['status', 'scheduled_at', 'caption', 'image_url'],
    describe:
  ...
  podcast_guests: {
    ...
    order: 'submitted_at',
    max: 50,
    mutable: ['research_brief', 'producer_brief', 'suggested_questions', 'bio_md'],
    describe:
  ...
  episodes: {
    ...
    max: 50,
    mutable: [
      'title', 'status', 'episode_type', 'summary', 'show_notes', 'key_takeaways',
      'guest', 'guest_id', 'guest_ids', 'recorded_date', 'youtube_id',
      'spotify_embed_url', 'duration', 'target_path', 'slug',
    ],
    describe:
...
/**
 * Values a field is allowed to take.
 *
 * A free-text status is how a table ends up with 'Published', 'publish' and
 * 'published' meaning the same thing and nothing matching. The model gets told
 * the vocabulary rather than inventing one.
 */
export const FIELD_VALUES = {
  'drafts.status': ['draft', 'approved', 'published', 'abandoned'],
  'scheduled_posts.status': ['scheduled', 'posted', 'failed', 'cancelled'],
  'episodes.status': ['draft', 'ready', 'published', 'abandoned'],
};

/**
 * Validate a mutation against the registry.
 *
 * Writes must name a single row by id. There is no filter-based update, so a
 * wrong filter cannot rewrite a table — the blast radius of any mistake is one
 * row, by construction.
 */
export function validateUpdate({ resource, id, patch }) {
  const def = RESOURCES[String(resource || '')];
  if (!def) {
    return { ok: false, error: `Unknown resource "${resource}". Available: ${Object.keys(RESOURCES).join(', ')}.` };
  }
  if (!def.mutable || def.mutable.length === 0) {
    return { ok: false, error: `${resource} is read-only.` };
  }
  if (!id || typeof id !== 'string') {
    return {
      ok: false,
      error: `An id is required to update ${resource}. Find the row with query_state first, then update it by id.`,
    };
  }
  if (!patch || typeof patch !== 'object' || Array.isArray(patch) || Object.keys(patch).length === 0) {
    return { ok: false, error: 'patch must be an object of field/value pairs.' };
  }

  const bad = Object.keys(patch).find((k) => !def.mutable.includes(k));
  if (bad) {
    return {
      ok: false,
      error: `Cannot write "${bad}" on ${resource}. Writable: ${def.mutable.join(', ')}.`,
    };
  }

  for (const [k, v] of Object.entries(patch)) {
    const allowed = FIELD_VALUES[`${resource}.${k}`];
    if (allowed && v !== null && !allowed.includes(String(v))) {
      return {
        ok: false,
        error: `"${v}" is not a valid ${resource}.${k}. Allowed: ${allowed.join(', ')}.`,
      };
    }
  }
