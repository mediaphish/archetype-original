 ...
 *
 * The briefing is written in AO voice and feeds what Bart publishes, so it runs
...
End with one line: "**Note:** This is a briefing, not exhaustive research."`;
  ...
  // An internal briefing Bart reads, not prose published under his name, so
  // Sonnet is the right size for it.
  const text = (await completeText({ prompt, task: 'analysis', maxTokens: 3600 })).trim();
  if (!text) {
...
Do not write the article. Keep under 350 words.`;
  ...
  const text = (await completeText({ prompt, task: 'analysis', maxTokens: 1600 })).trim();
  if (!text) {
