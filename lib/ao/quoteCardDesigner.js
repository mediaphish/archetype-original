 ...
 */
function renderTemplateMinimal({ quote, sourceName, logoUrl, minimalVariant = 'dark' }) {
  const dark = String(minimalVariant || 'dark').toLowerCase() !== 'light';
  const bg = dark ? '#000000' : '#FFFFFF';
  const fg = dark ? '#f5f5f5' : '#1a1a1a';
  const muted = dark ? 'rgba(245,245,245,0.65)' : 'rgba(26,26,26,0.55)';
  const lines = wrapText(quote, 30, 8);
  const startY = 200;
  const lineGap = 52;
  const svgLines = lines
    .map((l, i) => `<text x="540" y="${startY + i * lineGap}" text-anchor="middle" font-size="38" font-weight="700" fill="${fg}">${escapeXml(l)}</text>`)
    .join('');
  const src = sourceName
    ? `<text x="540" y="720" text-anchor="middle" font-size="20" font-weight="600" fill="${muted}">${escapeXml(sourceName)}</text>`
    : '';
  const logoW = 200;
  const logoH = 88;
  ...
  const logoY = 920;
  const brand = logoUrl
    ? `<image href="${escapeXml(logoUrl)}" x="${logoX}" y="${logoY}" width="${logoW}" height="${logoH}" preserveAspectRatio="xMidYMid meet" />`
    : `<text x="540" y="${logoY + 55}" text-anchor="middle" font-size="42" font-weight="900" fill="${fg}">AO</text>`;
  return {
    template: 'minimal',
 ...
 */
export function renderQuoteCardSvg({ quote, sourceName, logoUrl, style = 'default', minimalVariant = 'dark' } = {}) {
  const q = String(quote || '').trim();
  if (!q) return { ok: false, error: 'quote required' };
  if (String(style).toLowerCase() === 'minimal') {
    const out = renderTemplateMinimal({ quote: q, sourceName, logoUrl, minimalVariant });
