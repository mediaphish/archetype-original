...
function renderTemplateCenter({ quote, sourceName }) {
...
function renderTemplateLeft({ quote, sourceName }) {
  const { bg, fg, accent } = palette();
...
function renderTemplateSplit({ quote, sourceName }) {
  const { bg, fg, accent } = palette();
...
export function renderQuoteCardSvg({ quote, sourceName } = {}) {
  const q = String(quote || '').trim();
  if (!q) return { ok: false, error: 'quote required' };
  const templatePick = Math.random();
  const out =
    templatePick < 0.34
      ? renderTemplateCenter({ quote: q, sourceName })
      : templatePick < 0.67
        ? renderTemplateLeft({ quote: q, sourceName })
        : renderTemplateSplit({ quote: q, sourceName });
