...
function rowToDraft(row) {
  ...
  return {
    ...
    tags: row.tags || [],
    related: row.related || [],
    corpus_connections: row.corpus_connections || [],
    thematic_threads: row.thematic_threads || [],
    guest: row.guest,
    guest_id: row.guest_id || null,
    guests: row.guests || null,
    guest_ids: row.guest_ids || null,
    slug: row.slug,
...
export async function insertEpisodeDraft(email, payload) {
  const { data, error } = await supabaseAdmin
    ...
    .insert({
      ...
      corpus_connections: payload.corpus_connections || [],
      thematic_threads: payload.thematic_threads || [],
      guest: payload.guest || null,
      guest_id: payload.guest_id || null,
      guests: payload.guests || null,
      guest_ids: payload.guest_ids || null,
      meta: payload.meta || {},
    })
    .select('*')
    .single();
...
export async function listAllEpisodeDrafts() {
  const { data, error } = await supabaseAdmin
    .from('ao_episode_drafts')
    .select(
      'id, title, episode_type, status, guest, guest_id, guests, guest_ids, recorded_date, updated_at, slug, meta, corpus_connections, thematic_threads'
    )
