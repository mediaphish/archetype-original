...
export async function getNextQuoteCardScheduleAnchor() {
  ...
  const { hour: prefHourUtc } = await resolveSuggestedHourUtc(['linkedin', 'instagram', 'facebook', 'twitter']);

  let base = new Date(minStart);
  base.setUTCMinutes(0, 0, 0);
  base.setUTCHours(prefHourUtc, 0, 0, 0);
  if (base.getTime() <= minStart.getTime()) {
