...
export function buildImagesEditsFormData({
  ...
  for (const img of images) {
    ...
    // The field name must be image[] once there is more than one, otherwise the
    // API rejects the request outright:
    //   "Duplicate parameter: 'image' ... use the array syntax instead"
    //
    // The comment here previously claimed repeated "image" fields were accepted.
    // They are not, which means multi-reference edits have never worked despite
    // MAX_REFERENCE_IMAGES advertising support for several. A single reference
    // keeps the plain name, since that path is proven.
    form.append(images.length > 1 ? 'image[]' : 'image', blob, img.filename || 'reference.png');
  }
