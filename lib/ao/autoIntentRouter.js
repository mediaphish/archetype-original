/**
 * Auto intent router — a model classifies natural-language turns so quote-card
 * flows do not depend on brittle user-facing regex phrases. Heuristics in chat.js
 * remain as fast-paths; this layer interprets meaning.
 *
 * This runs on Claude through textModel.js. Both calls here were raw fetches to
 * gpt-4o-mini, and OpenAI is for image creation only, so they moved across with
 * the rest of the text call sites. The continuation call writes lines in Bart's
 * voice, so it asks for the voice tier.
 *
 * Env: AO_AUTO_INTENT_ROUTER_DISABLED=1 to skip the router.
 */
...
import {
  ...
  shouldSkipAsQuoteCardBody,
} from './userSuppliedQuoteCards.js';
...
${raw}`;
  ...
  try {
    ...
    const extracted_quotes = Array.isArray(parsed.extracted_quotes)
      ? parsed.extracted_quotes
          .map((x) => safeText(x, 2000))
          .filter(Boolean)
          .filter((x) => !shouldSkipAsQuoteCardBody(x))
          .slice(0, 24)
      : [];
    return {
      intent,
