...
**Journal entries:** Draft approved → image approved → state in chat what you are publishing → call the publish_journal tool (approved drafts publish immediately; no Ready-to-publish button) → report the live URL or the real error. Prefer publish_journal over the legacy [PUBLISH_JOURNAL] text tag. Approving the draft is the only gate.
...
**Instagram Personal:** Personal energy. 1-2 sentences. 5-10 hashtags. Always includes link-in-bio language, per LINK POLICY above.
...
- LinkedIn metrics may be missing when the connected account lacks social-actions permissions; Instagram is the most reliable signal when others are empty.
...
export async function loadApprovedDraftsContext(email, userMessageText, recentUserMessages = []) {
  ...
  try {
    const { data: drafts, error } = await contentDrafts()
      .select('kind, series_slug, part_number, title, slug, status, image_url, summary, approved_at, content')
      .eq('created_by_email', email.toLowerCase().trim())
    ...
    const lines = regularDrafts.map((d) => {
      ...
      return `- [${d.kind}] ${title} | slug: ${realSlug} | Part ${d.part_number} | Status: ${d.status} | ${imageStatus} | Approved: ${date}`;
    });
...
Each line below shows the real, exact slug for that draft after "slug:". That is the only correct value to use in [DRAFT_FETCH_FULL_TEXT slug="..."]. Never guess a slug from the title, never invent one, and never fire DRAFT_FETCH_FULL_TEXT without a slug attribute. If the draft you need is not in this list, say so plainly to Bart instead of guessing.
...
export const AUTO_TOOL_SCHEMAS = [
  ...
  },
  {
    ...
    description:
      'Persist a short outline/brief for a brand-new journal before writing the full draft. Call this first for new posts, show Bart the plan, and wait for approval (or an explicit skip). Does not replace save_draft for the full piece.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description:
      'Publish an already-approved journal draft to the live site immediately (GitHub journal file, header image, caption scheduling, subscriber email). Requires status=approved, a header image, and captions for all required channels (linkedin_personal, instagram_business, facebook_business, twitter) in the draft or already scheduled. Approving the draft is the editorial gate — there is no second UI confirmation button. Before calling, state plainly what you will publish, then call the tool, then report the real live URL or the real error.',
    input_schema: {
      type: 'object',
      properties: {
        slug: {
  ...
  },
  {
    ...
    description:
      'Schedule social captions for a journal slug. Captions object keys must be channel keys: linkedin_personal, instagram_business, facebook_business, twitter. Duplicate-schedule and missing-channel guards apply server-side.',
    input_schema: {
      type: 'object',
      properties: {
        slug: {
        ...
        captions: {
          type: 'object',
          description:
            'Map of channel key → caption text. Keys: linkedin_personal, instagram_business, facebook_business, twitter.',
          properties: {
  ...
  },
  {
    ...
    description:
      'Return the best posting times grounded in real engagement metrics when available. Call this whenever Bart asks for optimal/peak/best schedule or posting times — never recite static Peak times from memory. Each channel includes source: engagement_data (with sample_count) or fallback_insufficient_data. Tell Bart which case applies in plain language.',
    input_schema: {
      type: 'object',
      properties: {
        platform: {
