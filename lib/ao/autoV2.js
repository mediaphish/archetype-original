...
export function buildReferencedDraftContentBlock(
  ...
  // Editorial-pass language with only one pending journal draft — scan the window.
  if (toInject.length === 0) {
    const editorialInWindow = texts.some((text) =>
      /\b(editorial pass|word count|read (it |this )?back|pull (it |the draft )?up|revise this|edit this|polish this|did that come through|access to the draft|one more check)\b/i.test(
        text
      )
    );
...
export async function runAutoChatStream(
  ...
  try {
    ...
    return {
      ...
      systemPrompt: scheduleContext ? `${systemPrompt}\n\n${scheduleContext}` : systemPrompt,
      messages,
    };
  } catch (err) {
