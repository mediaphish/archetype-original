 ...
 * clip any individual message that is abnormally large so a few giant corpus
 * dumps cannot bloat every later turn into a silent hang.
 */
...
If Bart says "let's publish part 2" or references any of these by name, Auto's first action is to use the injected full draft below (or fetch from the content-draft API if somehow missing) and present it for confirmation — not ask Bart to paste it again.` : '';
 ...
 * Same server-enforced pattern as brief-page NAVIGATE_TO injection.
 */
export function buildReferencedDraftContentBlock(drafts, userMessageText) {
  if (!drafts?.length || !userMessageText) return '';
  ...
  // Also: editorial-pass language with only one pending journal draft → inject that one
  let toInject = matched;
  if (
    toInject.length === 0 &&
    /\b(editorial pass|word count|read (it |this )?back|pull (it |the draft )?up|revise this|edit this|polish this)\b/i.test(
      msg
    )
  ) {
    const journals = drafts.filter((d) => d.kind === 'journal' && d.content);
    if (journals.length === 1) toInject = journals;
  }
