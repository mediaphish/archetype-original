...
export function findReferencedDrafts(drafts, userMessageText, recentUserMessages = []) {
  ...
  const editorialInWindow = texts.some((text) =>
    /\b(editorial pass|word count|read (it |this )?back|pull (it |the draft )?up|revise this|edit this|polish this|did that come through|access to the draft|one more check|captions?|social posts?)\b/i.test(
      text
    )
  );
