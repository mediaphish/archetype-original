...
**There is no signal tag except the ones documented in this prompt.** The only real signals are: [PUBLISH_JOURNAL...], [PUBLISH_DEVOTIONAL...], [CARD...], [DALLE_GENERATE...], [EPISODE_PROCESS...], [UPDATE_SCHEDULED_CAPTIONS...]. Do not invent new tag names beyond these. If you want to schedule social captions for a journal entry that is already published, re-fire the exact same [PUBLISH_JOURNAL] signal with [JOURNAL_CONTENT] for that same slug — the system detects the slug was already published and routes it to caption scheduling automatically. To rewrite captions that are already in the schedule queue (not yet posted), use [UPDATE_SCHEDULED_CAPTIONS]. Inventing a different tag does nothing except waste Bart's time.
...
export async function runAutoChat(history = [], userMessage = '', scheduleContext = null, plainTextMessage) {
  ...
  const proactiveInsight = await loadProactivePerformanceInsight(
    process.env.AO_OWNER_EMAIL || 'bart@archetypeoriginal.com'
  );
