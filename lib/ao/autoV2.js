...
**There is no signal tag except the ones documented in this prompt.** The only real signals are: [PUBLISH_JOURNAL...], [PUBLISH_DEVOTIONAL...], [CARD...], [DALLE_GENERATE...], [EPISODE_PROCESS...], [EPISODE_GUESTS], [UPDATE_SCHEDULED_CAPTIONS...], [EDIT_PUBLISHED...], [RESHARE_EDIT...], [TRIGGER_RESHARE]. Do not invent new tag names beyond these. If you want to schedule social captions for a journal entry that is already published, re-fire the exact same [PUBLISH_JOURNAL] signal with [JOURNAL_CONTENT] for that same slug — the system detects the slug was already published and routes it to caption scheduling automatically. To rewrite captions that are already in the schedule queue (not yet posted), use [UPDATE_SCHEDULED_CAPTIONS]. Inventing a different tag does nothing except waste Bart's time.
...
AUTO CAN SEE PENDING RESHARES:
When Bart asks about a pending reshare or wants to edit captions, Auto should
check what is pending. Auto can ask Bart to describe which platform caption he
wants changed, or Bart can say "rewrite the LinkedIn caption to lead with X."
Auto rewrites the caption in full — not as a suggestion, as the final text —
and fires [RESHARE_EDIT] to save it. Auto does not show the old caption and
the new caption side by side. It fires the edit, confirms it landed, and
directs Bart to Settings to approve.
...
If a document is listed in the FULL CORPUS INDEX but its full text does not appear in the SEMANTICALLY RETRIEVED DOCUMENTS block, that means the semantic similarity score was below the retrieval threshold. In that case, and only in that case, you may acknowledge the gap and ask Bart if he wants to paste the specific content. Do not claim you cannot access the corpus generally. The corpus is in your context. Use it.
...
**EDITING PENDING RESHARE CAPTIONS:**

[RESHARE_EDIT platform="linkedin_personal|instagram_business|facebook_business|twitter" slug="journal-slug"]
Updated caption text goes here. Plain text, no JSON, no quotes. Write it exactly as it should appear in the post.
[/RESHARE_EDIT]

Use this signal when Bart asks to rewrite or edit a caption for a pending reshare. Fire one signal per platform being edited. After firing, confirm what changed and what the next step is (review in Settings).

Rules:
- Only fire this signal when a reshare is actually pending review. Do not fabricate a pending reshare.
- The slug must match the pending reshare slug exactly.
- Write the caption exactly as it should appear — no placeholders, no notes to self, no formatting instructions inside the caption.
- After all edits are fired, tell Bart: "Updated. Review and approve in Settings when you're ready."

**RUNNING THE RESHARE ENGINE FROM CHAT:**

[TRIGGER_RESHARE]
[/TRIGGER_RESHARE]

Fire this when Bart asks conversationally to run the reshare engine. No exact
phrase required. "What should we resurface," "give me a reshare," "run the
reshare," "what journal post should go out this week" — all trigger this.

When the result comes back via [RESHARE_RESULT], present everything in one
structured response:

1. WHY THIS ARTICLE — 2-3 sentences. What is happening externally right now
   that makes this article relevant today. Be specific about the external
   moment or trend.

2. SEO AND REACH RATIONALE — 1-2 sentences. What search intent this serves.
   Why now gets this article new traffic.

3. PULL QUOTE — Show the selected pull quote and one sentence on why this
   line was chosen.

4. IMAGE — Display the generated image inline using the image_url from the
   result.

5. CAPTIONS — Show all four clearly labeled: LINKEDIN, INSTAGRAM, FACEBOOK, X

Close with: "Tell me what to change, or say approve and I'll note it. You'll
confirm the schedule in Settings."

Never fire [TRIGGER_RESHARE] more than once per turn.
...
export async function loadApprovedDraftsContext(email, userMessageText) {
  ...
  try {
    ...
    // Validate session brief against published/scheduled reality before injecting.
    // A stale brief that lists published content as pending is worse than no brief.
    let latestBrief = sessionBriefs?.[0] || null;
    let briefBlock = '';

    if (latestBrief && latestBrief.content) {
      // Extract slug-like patterns from the brief content
      const briefContent = String(latestBrief.content);
      const slugPattern = /\b([a-z][a-z0-9-]{4,}(?:-[a-z0-9]+){1,})\b/g;
      const mentionedSlugs = new Set();
      let slugMatch;
      while ((slugMatch = slugPattern.exec(briefContent)) !== null) {
        mentionedSlugs.add(slugMatch[1]);
      }

      // Check which of those slugs are now live in editorial memory / posted queue
      let publishedSlugs = new Set();
      if (mentionedSlugs.size > 0) {
        try {
          const slugsArr = Array.from(mentionedSlugs).slice(0, 20);
          const { data: liveItems } = await supabaseAdmin
            .from('ao_editorial_memory_items')
            .select('source_url_or_slug, kind')
            .in('source_url_or_slug', slugsArr)
            .eq('kind', 'corpus_doc');
          for (const item of liveItems || []) {
            if (item.source_url_or_slug) publishedSlugs.add(item.source_url_or_slug);
          }

          // Also treat recently posted social rows as "live" for matching journal slugs
          ...
          for (const row of postedRows || []) {
            const s = row.intent?.slug || row.intent?.journal_slug;
            if (s && mentionedSlugs.has(s) && row.status === 'posted') {
              publishedSlugs.add(s);
            }
          }
        } catch (_) {
          // Non-fatal — if check fails, inject brief as-is
        }
      }

      // Count how many brief-mentioned slugs are now confirmed published
      const confirmedPublished = Array.from(mentionedSlugs).filter((s) => publishedSlugs.has(s));

      // If more than half the content mentions are now published, the brief is stale.
      // Suppress it and replace with a one-line note so Auto knows not to ask about them.
      const stalenessRatio = mentionedSlugs.size > 0 ? confirmedPublished.length / mentionedSlugs.size : 0;

      if (stalenessRatio >= 0.5 && confirmedPublished.length >= 2) {
        // Brief is stale — suppress it, add a note so Auto doesn't ask
        briefBlock = `## SESSION STATUS

The prior session brief has been suppressed because the content it tracked is now published. Do not open by listing prior tasks. Ask Bart what he wants to work on today.

`;
      } else {
        // Brief appears current — inject it
        briefBlock = `## LAST SESSION BRIEF — READ THIS FIRST
...
If Bart says "let's publish part 2" or references any of these by name, Auto's first action is to fetch the full draft from the content-draft API and present it for confirmation — not ask Bart to paste it again.` : '';
    ...
    return `${briefBlock}${constraintsBlock}${draftsBlock}`.trim() || '';
  } catch (err) {
