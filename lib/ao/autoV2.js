...
export const AUTO_TOOL_SCHEMAS = [
  {
    name: 'save_draft',
    description:
      'Save or update a journal, devotional, or captions draft in ao_content_drafts. Use when producing draft content that should persist. Slug is canonicalized server-side.',
    input_schema: {
      type: 'object',
      properties: {
        kind: {
  ...
  },
  {
    ...
    description:
      'Publish an already-approved journal draft to the live site immediately (GitHub journal file, header image, caption scheduling, subscriber email). Approving the draft is the gate — there is no second UI confirmation button. Refuse if the draft is not status=approved. Before calling this tool, state plainly in chat what you are about to publish (title, live site, image, captions), then call the tool, then report the real live URL or the real error.',
    input_schema: {
      type: 'object',
      properties: {
        slug: {
  ...
  },
  {
    name: 'generate_image',
    description:
      'Generate or attach an image. intent=attach_for_reference means show/use for discussion only; intent=header_image_for_post saves as the draft header (requires slug).',
    input_schema: {
      type: 'object',
      properties: {
        intent: {
        ...
        size: {
          type: 'string',
          description: 'Optional size string, e.g. 1536x1024 (live DALLE_GENERATE attr).',
        },
        content_type: {
          type: 'string',
          description: 'Optional content type, e.g. journal_header (live DALLE_GENERATE attr).',
        },
        series_slug: {
          type: 'string',
