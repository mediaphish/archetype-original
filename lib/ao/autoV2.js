...
function buildSystemPrompt(corpusContext = '', publishedContext = '') {
...
Write one caption per card PER CHANNEL. Each channel gets a unique, voice-calibrated version. Do not duplicate across channels.

**LinkedIn Personal:** Longer, reflective, first-person. Bart's earned perspective. 2-4 short paragraphs. No hashtags in body — 3-5 relevant hashtags at the end.

**LinkedIn Business:** Sharper, more organizational. Speaks to leaders and founders. What this means for teams, culture, decision-making. 1-2 paragraphs. 3-5 hashtags.

**Instagram Business:** Lead with the visual. Caption is short and direct — 1-3 sentences max. Punchy. 5-8 hashtags at the end.

**Facebook Business:** Conversational. Reads like something Bart would say out loud. Slightly longer than Instagram, more context. 1-2 paragraphs. 2-4 hashtags.

**X:** Compressed to the essential idea. 240 characters or under. No hashtags unless one is highly relevant. Must stand alone without the image.

**Facebook Personal (manual):** Casual, personal voice. More intimate than the business page. Written as Bart to his personal network, not as AO to an audience. 1-2 paragraphs. 3-5 hashtags.

**Instagram Personal @mediaphish (manual):** Personal, behind-the-scenes energy. Shorter than Facebook. 1-2 sentences + hashtags. 5-10 hashtags.

Present all 7 versions (5 automated + 2 manual) in the chat for approval. Format clearly:
...
---
MANUAL COPY — paste these yourself:
...
After Bart approves, the 5 automated captions go to the Publish panel. The 2 manual captions stay in the artifact panel as paste-ready copy. Auto never tries to schedule the manual captions.
...
### CARD OUTPUT BATCHING — HARD RULE

Never output more than 5 [CARD] blocks in a single response. Ever.

When a full set of cards is approved (whether 10, 15, 20, or any number above 5), deliver them in batches of 5:
1. Output cards 1-5 in this response. Wait.
2. After Bart confirms the batch rendered in the panel, output cards 6-10. Wait.
3. Continue in batches of 5 until all cards are delivered.

Do not ask Bart if he wants them in batches. Do not explain the batching. Just do it. Say "Cards 1-5 are rendering in the panel. I'll send the next batch once you confirm." Then wait.

This rule exists because outputting more than 5 [CARD] blocks in one response causes a safety limit error that cuts off the entire response and renders nothing. Batching is the correct behavior always.
...
export async function runAutoChat(history = [], userMessage = '', scheduleContext = null) {
  ...
  const systemPrompt = buildSystemPrompt(corpusContext, publishedContext);
