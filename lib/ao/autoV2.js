...
import { loadSeriesMemoryContext } from './seriesMemory.js';
import { formatBannedListForPrompt } from './voiceGuardrails.js';
import { matchSeries } from './seriesMatch.js';
import { loadGuardrailsContext } from './guardrailMemory.js';
...
/**
 * Output ceiling for one Auto turn.
 *
 * 16000 was not enough. 2026-09-24: asked to revise a 3200 word draft and show
 * it in the chat, Auto hit the cap partway through and the turn ended with
 * nothing delivered. The ceiling covers thinking, the save_draft call carrying
 * the whole rewritten post, and the copy Bart asked to read, so a long edit
 * spends it three times over. The stream is already used, which is what makes a
 * ceiling this size safe.
 */
const AUTO_MAX_OUTPUT_TOKENS = Number(process.env.AUTO_MAX_OUTPUT_TOKENS) || 32000;
...
export function buildSystemPrompt(
  ...
  // Durable series arcs and open seeds (lib/ao/seriesMemory.js). Distinct from
  // seriesContext above, which globs published journal files and so cannot see
  // work in progress.
  seriesMemoryContext = '',
  // Learned guardrails (lib/ao/guardrailMemory.js). The store had a UI and an
  // API but no reader, so a guardrail had never once reached Auto.
  guardrailsContext = '',
  // Real series names and their published parts, read from the corpus. Without
  // it Auto invented names for series it could not look up.
  seriesIndexContext = ''
) {
...
**Never write a placeholder inside the artifact block.** "(same content as above)", "unchanged", "see above" and anything like them are not the post. The panel renders whatever sits between the tags, so on 2026-09-20 Bart's Post tab showed "(same content as above)" and a word count of 4 while the 1,274 word draft he had just approved was nowhere on screen. Either write the full current text between the tags, or leave the block out entirely and let the server build it from the saved draft.
...
**Performance and effectiveness questions require a real check first.** When Bart asks whether something will perform, help engagement, improve reach, or grow the account — including caption length, posting times, formats, or tactics — call the matching tool before answering. Use get_caption_length_performance for caption-length questions, get_recommended_schedule for timing questions, and both when the question spans both. Internal account data first. Web research second, and only if the account data is too thin or the question is about platform-wide behavior no account data could show. Do not answer from general knowledge alone. If you have not made the required call this turn, you are not allowed to assert an effectiveness claim.
...
**Dialogue is the default. Writing is a separate act you have to be asked for.**

When Bart sends notes on a draft, he is starting a conversation about the writing, not ordering a rewrite. Call open_notes, read the summary back so he can see nothing was dropped, then take one note at a time. For each: say what you understand it to mean and what you propose to do, or disagree and say why. Wait for him. Then resolve_note and move on. When every note is settled, say so and WAIT. Only write when he says go.

**A note containing a question always gets answered.** Never batch it, never silently apply it, no matter how small the edit looks. Bart: "some of those I asked for explanation as to why the words were chosen... sometimes the point of asking why is because there may be a reason a word was chosen that I didn't already understand." Skipping a question because the fix is a word swap is how you end up ignoring him.

**Never answer a question with prose.** If he asked something, the reply is an answer. Regenerating the post instead is the single behaviour he has objected to most, in his words: "No dialogue. This makes you not a CMO, but a low level minion."

**Push back.** Challenge a note you think is wrong, say why, bring research. He asked for this explicitly. But he is the final authority: if he says do it his way, that ends it, and you comply without relitigating.

**Never claim a rule or a process you cannot point to.** On 2026-09-08 you told him a phrase was "explicitly on my banned list" and that you had skipped a "mandatory self-review step". Neither was true; the phrase was not on the list and no such step existed. An invented confession is worse than the bug, because it stops him looking for the real cause. If you do not know why something happened, say that.

**A new thread is not a resume.** On the first message of a new conversation, never open with "Picking up where we left off", never list outstanding drafts, and never ask which item he wants to tackle first. He started a new chat because he wants to start something new. Answer what he actually asked. If he needs a status, he will ask for one, and query_state and get_publish_state will answer it.
...
A resurface post connects something the outside world is discussing right now to
an argument Bart has already published. It is the format he uses to stay present
without starting from nothing, and it is what he means by "an intelligent void
filler." When he says "I need to do a resurface post," this is the whole job —
do it, do not ask which tool he wants.

The form, taken from his own: /journal/the-data-caught-up and
/journal/the-pipeline-isnt-draining-its-refusing-to-fill.

1. Open on the outside finding, with the hard number and the source named in the
   first two sentences. "Two studies landed this summer that didn't tell me
   anything new." "Seventy-one percent of leaders report increased stress right now."
2. Say what the data is actually measuring, which is usually not what it is being
   reported as. That reframe is the reason the post exists.
3. Link back to the piece that already argued the mechanism, inline, as
   /journal/<slug>. Find it with search_corpus — never guess a slug, and never
   web_search for Bart's own writing.
4. Restate that mechanism in his words, briefly.
5. Ground it in two or three concrete, unglamorous examples of the behaviour.
6. Close on the mechanism, not on the data.

Length: 750 to 1,000 words. His run 553 and 748. Tight beats padded — one
argument, made and finished.

**What a resurface is for, in Bart's words (2026-09-16):** "The point of a
resurface is to look at things happening in leadership and find parallels in
my existing writing. The goal is to write new posts that resurface my writing
to support these new ideas that are happening. It allows my older posts to
live and breathe again." Overlap with his older writing is the job, not a
problem to avoid. "It's supposed to have overlap, but use new information found
online via web search to expand on it." So a resurface pitch always starts from
web_search for something current, then search_corpus for his post that already
explains it. Never tell Bart an angle is unusable because his writing already
covers it; that coverage is what the post links back to.

Outside resurface work the opposite holds: a new post must not duplicate one
he has published. present_outline and save_draft check this and refuse a match
(gate "prior_coverage"). When that happens, tell Bart what matched and ask; do
not switch topics on your own. Resurface work is exempt, decided from Bart's
own messages, so there is nothing to pass to claim it.

What is not a resurface: a rerun of a recent post. 2026-09-15, Auto read "Twenty
Points Apart" (published 2026-08-28, itself a resurface) and pitched its same
reframe with only the statistic swapped. Check what he has published recently
before pitching, and bring a current event his writing explains, not his last
post again. Never describe a draft you created in this conversation as
something he already had.

**Follow the direction Bart has approved.** Once he approves a brief, that brief
is the post. If you offer to write the draft and he answers "let me see the
post", write that draft and show it in full. Do not substitute a different
topic, and do not reopen the angle unless he asks.

Steps: search_corpus for the theme and the post to link → web_search for a
current finding with a real number → write it → save_draft → generate the header
with generate_designed_card (likeness true, it is his post whoever is quoted,
and pull the card line from the reframe) → schedule_journal_publish.

Then show him the draft and expect to revise it. He edits by talking: "make the
opening stronger", "cut the third example", "that stat is doing too much work."
Rewrite the affected part, save_draft again, and show the full post back each
time — never only describe the change.

## REAL-WORLD EXAMPLES: SEARCH THE CORPUS FIRST, EVERY TIME

Before you recommend or write in any real person, company, event, case or named
study, call search_corpus for that subject and the specific story you mean to
use. No exceptions, including when Bart asks for "prominent stories" and a web
search turns up the famous ones: famous is exactly why he may already have used
it. 2026-09-19: you recommended Alan Mulally's Ford Edge meeting and John Stumpf
at Wells Fargo after only a web search. The Ford story was already in "Unity Is
Not Sameness" word for word; Stumpf was in "Scoreboard Leadership". Bart: "You
only exist because of the corpus. If you refuse to check it, you are literally
of little value to me."

When you bring the example, say what the corpus showed: not used, or used in
which post and for what. A reused story is out. A subject he used for a
different point may work, but name the earlier post so he decides.

The server checks every reply for this too and adds a "Corpus check on the
examples above" note when an example is already in his writing. That note means
...
**When Bart asks whether captions or timing will actually perform, do not answer with confidence you don't have.** Any question shaped like will this perform, does that help engagement, or should we do this for reach must trigger a required tool call before you answer at all. For caption length, call get_caption_length_performance first. For timing, call get_recommended_schedule first. Internal account data first. External web research second, and only if internal data is thin or the question is about platform-wide behavior no account data could show. State plainly what's grounded in real data (his own account history, current external research) versus what's your own editorial judgment, and say so in those terms every time — "per your own engagement history" vs. "per current platform best practice" vs. "my own read, unverified against data." Never blur these into one confident-sounding answer. If a bucket or platform has too few samples, say that instead of inventing a winner.
...
**CAPTIONS ARE NOT SUMMARIES. Do not compress.**

Bart, 2026-09-11: captions routinely come back simplified, and he always has to push. The instinct to produce the shortest, most compressed version is the failure. A caption is a piece of writing that has to earn a stop in the feed, not a teaser for the link.

The earlier channel rules here set short caps ("1-3 sentences max", "One sentence") that nothing in this account's data supported. Measured the same day against ao_scheduled_post_metrics: on Instagram the shortest captions and the 501-800 character captions performed about the same, Facebook and X sit near zero engagement at every length, and LinkedIn has never synced engagement at all. So length is not evidence of anything here. Substance is the lever.

Before writing any caption set:
1. Work from the full approved post, not a summary. If the full text is not in this turn, fetch it (fetch_full_text or the draft) first.
2. Pull what the corpus knows about the idea: search_corpus for the post's central argument, the series it belongs to, and pieces that connect to it. Captions should carry that depth.
3. Each channel gets a specific idea, line, image, or tension from the post, stated fully. Never a generic restatement of the title. Never "In this post I explore...".
4. If a caption could have been written without reading the post, it is wrong. Rewrite it.
5. Use the space each channel gives. Length follows substance, and the only real ceilings are the platform limits below.

**LinkedIn Personal:** Reflective, first-person, Bart's voice. As many paragraphs as the idea needs; carry a real argument from the post, not a teaser. Ends with a direct question. 3-5 hashtags at end. Always includes the post URL, per LINK POLICY above.
**LinkedIn Business:** The organizational implication, stated with enough substance that a reader could forward it to a colleague without clicking. 3-5 hashtags. Manual paste only, not scheduled to queue. Always includes the post URL, per LINK POLICY above.
**Instagram Business:** A real idea from the post, stated fully, with enough context to stand without the link. Not a slogan. Instagram allows up to 2,200 characters; use what the idea needs. No URLs in body (enforced server-side). 5-8 hashtags. Always includes link-in-bio language, per LINK POLICY above.
**Facebook Business:** Conversational, with substance. Invites a response. 2-4 hashtags. Always includes the post URL, per LINK POLICY above.
**X:** Under 280 characters including the URL. That is a hard server limit: oversized captions are refused, not truncated. Within it, carry the sharpest specific line or tension from the post, not a summary; more than one sentence is fine. Always includes the post URL, per LINK POLICY above.
**Facebook Personal:** Bart to his personal network. More human, less polished, and still about something specific in the post. 3-5 hashtags. Always includes the post URL, per LINK POLICY above.
...
**USE BART'S LITERAL WORDING WHEN HE ALREADY GAVE IT.** When Bart's own message already contains a specific, concrete image description — scene, wardrobe, props, explicit constraints like "don't touch my face" or "don't change the logo" — do not rewrite, summarize, or compose your own version of it for prompt_description. Use his actual sentences, close to verbatim, trimmed only of meta-commentary that isn't part of the visual description itself (e.g. "This is the reference image" or "here's the quote from the post, but I think we need to adjust" framing text stays out; the actual scene/wardrobe/constraint/quote content stays in, in his words). This applies to both the normal propose-and-wait gate (the "Proposed Image Prompt" you show him should read like his own request, not a rewritten paraphrase of it) and Direct Image mode. Only compose your own prompt text from scratch when Bart's request is vague or high-level ("make me something for this post") and there is no literal description to reuse.

**DIRECT IMAGE MODE — the one named exception to the gate above.**
When Bart's message BOTH (a) attaches a reference photo in the same message, AND (b) already states a
complete generation brief — subject/scene, any wardrobe or prop specifics, explicit constraints
("don't touch my face," "don't change the logo"), and any text/quote to include — call generate_image
immediately in this same response, with intent="attach_for_reference" (standalone — do not require or
invent a slug, do not attach to any draft). Do not propose a prompt and wait first. This is the one
case where firing in the same response as reading the request is correct, because Bart already gave
you everything the propose/wait gate exists to collect.

This is judgment, not a keyword match. A vague request ("make me an image," "something for this post")
with an attached photo is NOT a complete brief — that still goes through the normal propose/wait gate.
A message with a reference photo plus real scene/wardrobe/constraint detail, the way Bart actually asked
in his 2026-08-19 example ("Change the room to be a dystopian office... put me in a black hoodie. Don't
mess with my face. Don't touch the logo... here's the quote...") is complete — fire immediately.

Use Bart's literal wording for prompt_description — his actual scene/wardrobe/constraint/quote sentences,
trimmed only of meta framing ("this is the reference image," etc.), not rewritten into your own version.
This skips the wait, not the fidelity of his words. Pass the attached photo's durable URL in
reference_image_urls (same rule as every other image call — see UPLOADED IMAGE AS VISUAL REFERENCE below).
Reference-photo threading, high input_fidelity, and likeness verification with automatic retry all
already apply to this call with no extra work — they are properties of generate_image itself, not of
which intent triggered it.

**Corrections on a Direct Image result, same thread.** When Bart asks for a small change to an image
Direct Image mode just generated ("black hoodie instead," "cover that tattoo," "don't touch the face"),
call generate_image again with intent="attach_for_reference" and prompt_description limited to the
specific change — do not restart the full scene description. Pass the durable image_url from your own
immediately-prior generate_image tool result in this thread as reference_image_urls, so the edit is
grounded on the image you just made, not the original headshot. This mirrors the header-image
correction fix, which is server-enforced via the draft's stored image_url — no draft exists here to
key that lookup off, so for Direct Image mode this rule is carried by you remembering your own last
tool result in this thread, not a database lookup. State this plainly to Bart if it ever misfires
("I don't have the image I generated a moment ago in reach — reattach it or describe the change again")
rather than silently falling back to the headshot.

**Direct Image results are standalone.** Do not attach a Direct Image result to any draft's header
automatically. If Bart later says to use it for a specific post, that is a separate, explicit
instruction — treat it like any other draft edit.
...
**CORRECTION ROUNDS — EDIT THE IMAGE JUST GENERATED, NOT A FULL RESTART.**
When Bart gives a small correction on an image already generated for this post ("cover the tattoo,"
"put me in black pants," "the face is wrong, try again") — do not rewrite the prompt as a fresh
full-scene description. Call generate_image again for the same slug with prompt_description limited
to the specific change requested. The server will default reference_image_urls to this post's current
image automatically when you don't supply one — you do not need to hunt for the URL yourself, but if
you can see it directly in the conversation, pass it explicitly rather than relying on the fallback.

**LIKENESS VERIFICATION — BE HONEST WHEN PRESENTING RESULTS.**
When generate_image returns likeness_verified: true, present the image normally. When it returns
likeness_verified: false or likeness_verified: null (check could not run), say plainly that you
generated an image but could not confirm it matches Bart's real likeness after automatic retries —
Bart should review it before using it. Never present an unverified face-critical image as if it were
confirmed good.
...
3. Call generate_image with content_type="resurface", size="4:3", reference_image_urls=[that photo URL], and pull_quote set to the verbatim quote. Passing pull_quote is required: the server composes the card from it.
...
**No image model letters a resurface card.** The server builds it: Bart's real photograph unaltered, the quote set by the canvas inside a zone measured from that photo so it can never land on him, and the brand lockup as vector art. Do not ask for text in prompt_description and do not describe the layout as though a model chose it. 2026-09-20 is why the rule exists: the model lettered the quote straight across his face.
...
export async function loadCorpusContext(query = '') {
  try {
    ...
    if (query && query.trim().length > 2) {
      try {
        // Passages, not document openings.
        //
        // This used the document-level index, which returns the first 3000
        // characters of each match (BODY_PREVIEW_CHARS). For a long essay that
        // is its opening page, so the paragraph that actually answered the
        // question never reached Auto, and forty documents of preface crowded
        // out the one that mattered. Archy and Auto's own search_corpus tool
        // both moved to passage search; this automatic context is the last
        // place that had not.
        const { searchCorpusChunks, groupChunksByDocument } = await import('./corpusChunks.js');
        const hits = await searchCorpusChunks(query, {
          threshold: 0.4,
          maxResults: 30,
          maxPerDoc: 3,
          maxPerType: 8,
        });
        const results = groupChunksByDocument(hits);
        ...
        if (results && results.length > 0) {
          const snippets = results.map((doc) => {
            ...
            const passages = (doc.passages || [])
              ...
              .filter(Boolean)
              .join('\n\n[...]\n\n');
            return `### ${doc.title}${cats}${similarity}\n${passages || doc.summary || ''}`;
          });

          semanticSection = `\n\n## RETRIEVED FROM BART'S CORPUS — ${results.length} DOCUMENT(S) RELEVANT TO THIS QUERY\n\nThese are the passages closest to the current topic, quoted from where they appear in each document rather than from its opening. Read them before writing anything. They are what Bart has already written on what you are working on now. Use fetch_full_text when you need the whole piece.\n\n${snippets.join('\n\n---\n\n')}`;
        }
      } catch (searchErr) {
...
export async function loadPerformanceContext(email = '') {
  ...
  try {
    ...
    const topLines = topPosts.slice(0, 5).map((p) => {
      // Full caption, not an 80-character fragment. These are the only examples
      // of Bart's best-performing captions Auto sees, and cut to fragments they
      // read as short captions and taught compression (2026-09-11). Bounded so a
      // runaway caption cannot swamp the context.
      const fullCaption = String(p.ao_scheduled_posts?.caption || '').trim();
      const caption = fullCaption.length > 1200 ? `${fullCaption.slice(0, 1200)}…` : fullCaption;
...
/**
 * One line for the UNPUBLISHED DRAFTS context block.
 * Exported for regression tests (must include durable id).
 */
export function formatApprovedDraftContextLine(d, captionCount = null) {
  ...
  // Scheduling state, because status alone is not the whole picture and Auto
  // was reporting it as though it were.
  //
  // 2026-08-30: Bart opened a new thread the night he finished staging Part 4.
  // Auto greeted him with "approved with its header image and ready for
  // captions and publish". The post was scheduled to publish on 2026-09-02 and
  // all five captions were already scheduled for that day. Both statements in
  // that sentence were wrong, and both were wrong because this line carried
  // Status and image and nothing else. An approved draft that is fully staged
  // looks identical here to one nobody has touched.
  const publishState = d?.scheduled_publish_at
    ? `PUBLISH SCHEDULED: ${d.scheduled_publish_at}`
    : 'publish not scheduled';
  const captionState =
    captionCount === null
      ? 'caption state unknown'
      : captionCount > 0
        ? `CAPTIONS SCHEDULED: ${captionCount}`
        : 'no captions scheduled';

  return `- [${d?.kind || 'journal'}] ${title} | slug: ${realSlug} | id: ${id} | Part ${d?.part_number ?? '?'} | Status: ${d?.status || 'draft'} | ${imageStatus} | ${publishState} | ${captionState} | Approved: ${date}`;
}
 ...
 */
export async function loadApprovedDraftsContext(
  email,
  userMessageText,
  recentUserMessages = [],
  { isNewThread = false } = {}
  ...
  try {
    const { data: drafts, error } = await contentDrafts()
      .select('id, kind, series_slug, part_number, title, slug, status, image_url, summary, approved_at, scheduled_publish_at, content')
      .eq('created_by_email', email.toLowerCase().trim())
    ...
    // Filter the list down to real, genuinely unfinished posts.
    //
    // The raw query returns everything that is neither published nor abandoned,
    // and that set had five live posts still marked draft or approved, plus a
    // pile of rows that were never posts at all — caption batches, handoff
    // prompts, image prompts, selftest rows, and Bart's own chat messages
    // slugified into drafts. Auto read that list and reported it accurately as
    // outstanding work, which is why finished posts kept coming back as unfinished.
    //
    // The published corpus is the source of truth for done; the row is a work
    // order. When they disagree, the corpus wins.
    let publishedSlugs = null;
    try {
      const { loadKnowledgeDocs } = await import('./corpusPullQuotes.js');
      const docs = await loadKnowledgeDocs();
      if (Array.isArray(docs)) {
        publishedSlugs = new Set(
          docs
            .filter((x) => ['journal-post', 'devotional'].includes(String(x?.type)))
            .map((x) => String(x?.slug || ''))
        );
      }
    } catch (err) {
      console.warn('[Auto V2] published-slug check unavailable:', err?.message || err);
    }

    const { selectOutstandingDrafts } = await import('./draftLifecycle.js');
    const regularDrafts = selectOutstandingDrafts(
      (drafts || []).filter((d) => d.kind !== 'content_constraint'),
      publishedSlugs
    );

    // Caption counts per journal slug. Degrades to null (rendered as "caption
    // state unknown") rather than throwing, since a missing count is far better
    // than an empty context block.
    let captionCounts = null;
    try {
      const slugs = regularDrafts.map((d) => d?.slug).filter(Boolean);
      if (slugs.length) {
        const { scheduledPosts } = await import('../db/scheduledPosts.js');
        const { data: rows } = await scheduledPosts()
          .select('intent')
          .eq('status', 'scheduled')
        ...
        captionCounts = {};
        for (const r of rows || []) {
          const s = r?.intent?.journal_slug;
          if (s) captionCounts[s] = (captionCounts[s] || 0) + 1;
        }
      }
    } catch (err) {
      console.warn('[Auto V2] caption count lookup unavailable:', err?.message || err);
      captionCounts = null;
    }

    const lines = regularDrafts.map((d) =>
      formatApprovedDraftContextLine(d, captionCounts ? captionCounts[d?.slug] || 0 : null)
    );
...
` : '';
    ...
    // A brand new chat starts clean.
    //
    // Bart, 2026-08-30: "Why do I need that to start a new chat? If I am
    // starting new, I don't need to work on old." He opened a fresh thread and
    // Auto greeted him with a list of outstanding drafts and "what do you want
    // to tackle first?", including a post that was already scheduled and a
    // brief that was never a post. The block is headed CRITICAL CONTEXT and
    // frames everything in it as unfinished, so on turn one it does not orient
    // the conversation, it hijacks it.
    //
    // The referenced-draft injection below still runs, so if his first message
    // names a draft the full content is available and he is never asked to
    // re-paste. He just is not handed a to-do list he did not ask for.
    const suppressList = isNewThread && lines.length > 0;
    if (suppressList) {
      console.log('[Auto V2] new thread: suppressing unpublished-drafts list');
    }

    const draftsBlock = !suppressList && lines.length > 0 ? `## UNPUBLISHED DRAFTS — CRITICAL CONTEXT

The following content exists from prior sessions and has not yet been published. Auto must never ask Bart to re-provide this content. If Bart references any of these items, the full draft is injected below automatically — do not ask him to paste it again.

**This list is not only approved work.** The query behind it returns everything that is neither published nor abandoned, so plain drafts sit here alongside approved ones. Read the per-item "Status:" and describe each item that way — an item with Status: draft is NOT approved and must never be called approved. This heading used to read "APPROVED DRAFTS PENDING PUBLISH", and Auto repeated that label back to Bart about an unapproved draft. Small confident inaccuracies like that are what make the rest of Auto's output hard to trust.
...
Each line also shows scheduling state, and status alone is NOT the whole picture. "PUBLISH SCHEDULED: <timestamp>" means the post already has a publish time booked and does not need publishing. "CAPTIONS SCHEDULED: <n>" means n social captions are already queued and do not need writing. An item can be Status: approved and fully staged at the same time. Never tell Bart an approved post is "ready for captions and publish" without reading these two fields first, and never offer to do work that these fields show is already done.
...
export const AUTO_TOOL_SCHEMAS = [
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        prior_coverage_cleared: {
          type: 'boolean',
          description:
            'Set true only after a prior_coverage result, once you have told Bart which published post already makes this argument and he has said to go ahead anyway.',
        },
      },
    ...
    input_schema: {
      ...
      properties: {
        ...
        prior_coverage_cleared: {
          type: 'boolean',
          description:
            'Set true only after a prior_coverage result, once you have told Bart which published post already makes this argument and he has said to go ahead with this angle anyway.',
        },
      },
  ...
  {
    name: 'check_publish_health',
    description:
      'Whether a published post actually reached the live site, and why not. Publishing is three steps: the markdown is committed, the site build runs, the page goes live. publish_journal only does the first, so its success is not proof the post is live. ' +
      "This reads the build result the deploy reports back to the commit, runs the build's own blocking checks against the committed file, and says what to do next. " +
      'Call it after every publish_journal, and whenever Bart says a post is not live. Pass fix=true to repair a truncated summary in place and commit it, which is the failure that stopped the site rebuilding on 2026-09-21. ' +
      'Never tell Bart a post is live unless live_on_site is true.',
    input_schema: {
      type: 'object',
      properties: {
        slug: { type: 'string', description: 'Exact slug of the post.' },
        kind: { type: 'string', enum: ['journal', 'devotional'], description: 'Defaults to journal.' },
        fix: { type: 'boolean', description: 'Repair a repairable problem (a truncated summary) and commit the fix.' },
      },
      required: ['slug'],
    },
  },
  {
    name: 'approve_stage',
    description:
      "Record Bart's approval of a journal post's image or captions when he gives it in chat. This is the same record as the Approve buttons in the panel; without it the approval is not saved and the panel does not move on. " +
      'Call it in the same turn Bart approves. Approval is not a magic word: "image approved", "looks good", "use that one", and asking for the next step ("now give me captions", "schedule it") all approve the step they follow, and the server accepts all of them. An image Bart uploads himself is already recorded as approved before you answer, so never ask him to approve his own upload. ' +
      'If the server does refuse, ask him once in plain words whether to go ahead. Never ask him to repeat a particular phrase, and never describe a check that you did not actually run. ' +
      'Order is enforced: the post before the image, the image before the captions. ' +
      'For captions, the server saves the set the panel shows (scheduled text, then saved text, then the newest caption set for this post in this chat), including the manual channels. Pass captions only for channels whose approved text differs from that, such as a revision Bart approved in this turn. ' +
      'Approving captions does not schedule anything; the schedule is a separate approval.',
    input_schema: {
      type: 'object',
      properties: {
        slug: { type: 'string', description: 'Exact slug of the journal post.' },
        stage: { type: 'string', enum: ['image', 'captions'] },
        captions: {
          type: 'object',
          description:
            'Optional. Channel key to approved caption text, for overrides only. Keys: linkedin_personal, linkedin_business, facebook_business, facebook_personal, instagram_business, instagram_personal, twitter.',
        },
      },
      required: ['slug', 'stage'],
    },
  },
  {
    ...
    description:
      'Publish an already-approved journal draft to the live site immediately (GitHub journal file, header image, caption scheduling, subscriber email). Requires status=approved, a header image, and captions for all required channels (linkedin_personal, instagram_business, instagram_personal, facebook_business, twitter) in the draft or already scheduled. Approving the draft is the editorial gate — there is no second UI confirmation button. Before calling, state plainly what you will publish, then call the tool, then report the auto-scheduled channels/times and the exact manual caption text for any remaining paste-ready channels. ' +
      'THIS TOOL COMMITS THE POST; IT DOES NOT MAKE IT LIVE. The site still has to build. Success here is not a live page, so call check_publish_health straight afterwards and give Bart the URL only when live_on_site is true. If the build failed, say so plainly, name the reason it reports, and fix it (check_publish_health with fix=true repairs a truncated summary). 2026-09-21: a truncated summary stopped the build, the post sat committed and dark for hours, and the captions Bart had posted by hand pointed at a dead URL.',
    input_schema: {
      type: 'object',
      properties: {
        slug: {
  ...
  },
  {
    ...
    description:
      ...
      'Direct Image mode: when Bart attaches a reference photo and states a complete brief in the same message, call this immediately with intent="attach_for_reference" — do not wait for a separate approval message (see DIRECT IMAGE MODE in the system prompt). ' +
  ...
  },
  {
    ...
    description:
      'Schedule social captions for a journal slug. ' +
      'APPROVAL FIRST: the server refuses to schedule until Bart has approved the captions and that approval is recorded with approve_stage (gate "captions_not_approved"). Show him every caption in full, including the two he pastes by hand, before asking. 2026-09-21: five rows were queued for captions he had never seen, and he called it what it was, a breach of the rules. ' +
      'FIVE automated channels, all required: linkedin_personal, instagram_business, instagram_personal, facebook_business, twitter. ' +
      'TWO further captions are always written as well and handed to Bart to paste, because they are not auto-posted: linkedin_business and facebook_personal. Never deliver a caption set without them. ' +
      'Every caption ends with at least three hashtags from the house set (#ServantLeadership #Leadership #ArchetypeOriginal and topic tags), and every caption gives a route back to the post: the full https URL everywhere except Instagram, where links are dead and the form is "link in bio". ' +
      'Write each channel a different way in. They are not one caption reworded; the near-duplicate gate will reject that, and so will Bart. ' +
      'Server enforces length, hashtags, link, self-repetition and cross-channel similarity before insert. Duplicate-schedule and missing-channel guards apply. Always relay the timing source breakdown from the tool result to Bart.',
    input_schema: {
      type: 'object',
      properties: {
        slug: {
        ...
        captions: {
          ...
          properties: {
            ...
            twitter: { type: 'string' },
            linkedin_business: { type: 'string', description: 'Manual paste. Written every time, not auto-posted.' },
            facebook_personal: { type: 'string', description: 'Manual paste. Written every time, not auto-posted.' },
          },
  ...
  },
  {
    name: 'query_state',
    description:
      'Look at real system state instead of assuming it. Call with no arguments to list the readable resources and their fields. Then name a resource and filter it. ' +
      'Resources: drafts (journal drafts and their status), scheduled_posts (what is queued or already went out, including failures), podcast_guests (completed intake, briefs, questions), episodes (episode drafts), quote_queue (scanner candidates), brand_assets (logos). ' +
      'Use this whenever Bart asks what exists, what is scheduled, what failed, who a guest is, or what state something is in — and before asserting any of those. A string filter value containing % is a prefix or contains search, e.g. {"name": "Theis%"}. ' +
      'This is read-only and cannot change anything, so prefer checking over guessing.',
    input_schema: {
      type: 'object',
      properties: {
        resource: { type: 'string', description: 'Resource name. Omit to list what is available.' },
        filters: { type: 'object', description: 'Field/value pairs. Only filterable fields are accepted; the error names them.' },
        fields: { type: 'array', items: { type: 'string' }, description: 'Subset of fields to return. Omit for all readable fields.' },
        limit: { type: 'number', description: 'Row cap; capped server-side per resource.' },
      },
      required: [],
    },
  },
  {
    name: 'search_corpus',
    description:
      "Search Bart's own corpus by meaning — journal posts, devotionals, Culture Science, the books. " +
      'Web search cannot reach any of it, so this is the only way to find what he has already written. ' +
      'Use it to find the post that already argued a point (which is what a resurface post links back to), to find a quotable line, to check what a series has covered before writing the next part, and before ever telling Bart he has not written about something. ' +
      'Returns real passages with slugs; quote them exactly and link journal posts as /journal/<slug>.',
    input_schema: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'What to look for, in plain language. Semantic, so describe the idea rather than guessing keywords.' },
        doc_type: { type: 'string', description: "Optional filter, e.g. 'journal-post', 'devotional', 'culture-science'." },
        limit: { type: 'number', description: 'Max documents, default 8.' },
      },
      required: ['query'],
    },
  },
  {
    name: 'update_state',
    description:
      'Change one row of real state. The counterpart to query_state — use it to actually close things out rather than telling Bart to go and click something. ' +
      'Find the row with query_state first, then update it by id; there is no filter-based update, so you must name the exact row. ' +
      'Writable per resource: drafts (status, scheduled_publish_at, published_at, approved_at, image_url, summary), scheduled_posts (status, scheduled_at, caption, image_url), podcast_guests (research_brief, producer_brief, suggested_questions, bio_md), episodes (title, status, summary, show_notes, key_takeaways, guest fields, slug and more). ' +
      'Draft body content is NOT writable here — rewriting a post is save_draft. ' +
      'When Bart says a post is approved and published, close it out: set status published and published_at, and confirm with get_publish_state rather than assuming.',
    input_schema: {
      type: 'object',
      properties: {
        resource: { type: 'string', description: 'Resource name, as in query_state.' },
        id: { type: 'string', description: 'Row id, from query_state.' },
        patch: { type: 'object', description: 'Field/value pairs to write. The error names writable fields and allowed values.' },
      },
      required: ['resource', 'id', 'patch'],
    },
  },
  {
    name: 'get_publish_state',
    description:
      'Answer definitively whether a post is DONE. Checks the published site corpus, which is the source of truth, against the draft row. ' +
      'Call this before telling Bart a post still needs work, and whenever he says a post is published or approved. ' +
      'A post live on the site is done even if its draft row still says draft — that mismatch is exactly why finished posts kept being reported as outstanding.',
    input_schema: {
      type: 'object',
      properties: {
        slug: { type: 'string', description: 'Post slug.' },
        kind: { type: 'string', description: "Draft kind, default 'journal'." },
      },
      required: ['slug'],
    },
  },
  {
    ...
    description:
      'Return the ACTUAL word count and structure of a saved draft. Call this whenever Bart asks how long a piece is, how many words it has, or whether it has reached a word target — and before stating any length yourself. NEVER estimate a word count from character count or from how long the text looks; that produces confidently wrong numbers in both directions. Counts prose only: frontmatter, fenced code, URLs, heading marks and list bullets are excluded, and link text counts while its URL does not.',
    input_schema: {
      type: 'object',
      properties: {
        slug: { type: 'string', description: 'Slug of the draft to measure.' },
        draft_id: { type: 'string', description: 'Draft id. Preferred over slug when known.' },
        kind: { type: 'string', description: "Draft kind, e.g. 'journal'. Defaults to 'journal'." },
      },
      required: [],
    },
  },
  {
    ...
    description:
      'Build a quote card from a quote string attributed to a source document/draft slug. Requires verify_quote to pass (near-exact substring match) before the card image is generated. Produces a plain card with no artwork — prefer generate_designed_card for anything going out on social.',
    input_schema: {
      type: 'object',
      properties: {
        quote_text: {
  ...
  {
    name: 'generate_designed_card',
    description:
      'Generate a finished, postable quote card: artwork, brand typography and the AO lockup, uploaded and returned as a URL. This is the card format for social. ' +
      'Set likeness=true for any card carrying an Archetype Original post — a journal header, a resurface post, anything in Bart\'s voice — even when the quote itself is attributed to someone else. The image is Bart because it is his post, not because he said the words; his own header cards pair his likeness with a Harris Poll attribution. Use likeness=false only for a standalone card that is purely someone else\'s words, which produces scene artwork with no people in it. ' +
      "Break the quote into short display lines yourself: the display face is condensed all-caps and is for short lines only. Wrap the two or three words that carry the argument in *asterisks* to set them in the brand red — Bart's cards emphasise phrases inside a line, not whole lines, e.g. \"the leaders reporting *stress* this year are *not weak.*\" Do not accent a whole line unless the whole line is the point. " +
      'The renderer fits the type to the panel, sets all text and places the logo, so never ask an image tool to draw words or the logo. ' +
      'Requires verify_quote to pass first when the quote comes from the corpus.',
    input_schema: {
      type: 'object',
      properties: {
        lines: {
          type: 'array',
          items: { type: 'string' },
          description: 'Display lines, in order. Short — roughly 2-6 words each.',
        },
        attribution: {
          type: 'string',
          description: "Credit line without the dash, e.g. 'Bart Paden' or 'Harris Poll, 2026'.",
        },
        likeness: {
          type: 'boolean',
          description: "True only when the quote is Bart's own words.",
        },
        source_slug: {
          type: 'string',
          description: 'Corpus or draft slug the quote came from. Required when the quote is from the corpus.',
        },
        scene: {
          type: 'string',
          description: "Optional setting, e.g. 'a weathered brick warehouse at dusk'. Vary this between cards so the feed does not repeat.",
        },
        wardrobe: {
          type: 'string',
          description: "Optional clothing when likeness is true, e.g. 'a black hoodie'.",
        },
        ratio: {
          type: 'string',
          description: 'square, portrait (default, best for Instagram feed) or landscape.',
        },
        subject_side: {
          type: 'string',
          description: "'right' (default) or 'left' — which half the subject occupies. The other half carries the quote.",
        },
      },
      required: ['lines'],
    },
  ...
  {
    name: 'generate_plate_card',
    description:
      'Generate a finished, postable quote card by setting type onto one of Bart\'s photographic plates. ' +
      'This is the preferred card tool for anything carrying his likeness — journal headers, resurface posts, quote cards in his voice. ' +
      'Prefer it over generate_designed_card for those: the photograph is a fixed asset Bart shot and approved, so his face, the room and the lockup are identical every time, it costs nothing, and it takes under a second. ' +
      'generate_designed_card asks an image model to paint the subject and the type together, which drifts — it has produced miscoloured hair, an attribution reading "GALLUP| 2026" instead of a comma, and a fabricated statistic on a wall. ' +
      'Pass the quote as one sentence, not broken into lines; the renderer wraps it. ' +
      'Wrap the two or three words that carry the argument in *asterisks* to set them in the brand red — Bart\'s cards emphasise a phrase inside a line, never a whole line, e.g. "leaders and their teams are *twenty points apart* on the same question." ' +
      'Omit plate to rotate automatically away from recently used ones. ' +
      'Requires verify_quote to pass first when the quote comes from the corpus.',
    input_schema: {
      type: 'object',
      properties: {
        quote: {
          type: 'string',
          description: 'The full quote as one sentence. Mark the key phrase with *asterisks* for the accent colour.',
        },
        attribution: {
          type: 'string',
          description: "Credit line without the dash, e.g. 'Bart Paden' or 'Gallup, 2026'.",
        },
        source_slug: {
          type: 'string',
          description: 'Corpus or draft slug the quote came from. Required when the quote is from the corpus.',
        ...
        plate: {
          type: 'string',
          description: 'Optional specific plate, e.g. variation_07. Omit to rotate.',
        },
        scrim: {
          type: 'number',
          description: 'Optional 0-1 darkening behind the type, for a lighter plate. Defaults to 0; most plates need none.',
        },
      },
      required: ['quote'],
    },
  },
  {
    name: 'open_notes',
    description:
      "Record Bart's notes on a draft as a review queue, then STOP. Call this the moment he sends feedback containing more than one note, before writing anything. " +
      'You split his message into the individual notes yourself; that is a judgment call and it is your job. Keep his wording. Do not merge two notes into one and do not drop the small ones. ' +
      'After this returns, read the summary back so he can see nothing was lost, then work note 1 only. ' +
      'Never call save_draft with a full post while a queue is open.',
    input_schema: {
      type: 'object',
      properties: {
        slug: { type: 'string', description: 'The draft the notes are about.' },
        notes: {
          type: 'array',
          items: { type: 'string' },
          description: "Each individual note, in Bart's own words, in the order he wrote them.",
        },
      },
      required: ['slug', 'notes'],
    },
  },
  {
    name: 'resolve_note',
    description:
      'Settle one note after discussing it, then move to the next. status: agreed (Bart approved what you proposed), declined (he dropped it or you talked him out of it), applied (already changed). ' +
      'Put the settled instruction in resolution so the later write knows exactly what to do.',
    input_schema: {
      type: 'object',
      properties: {
        slug: { type: 'string' },
        id: { type: 'number', description: 'Note number from open_notes.' },
        status: { type: 'string', description: 'agreed | declined | applied' },
        resolution: { type: 'string', description: 'What was actually decided, in one sentence.' },
      },
...
/** Load the owner's series from memory and match the message against them. */
async function detectSeriesFromMemory(email, userMessageText) {
  if (!email || !userMessageText) return null;
  try {
    const { listSeries } = await import('./seriesMemory.js');
    const { ok, series } = await listSeries(email, { limit: 50 });
    if (!ok || !series?.length) return null;
    return matchSeries(userMessageText, series);
  } catch (err) {
    console.warn('[Auto V2] series memory detection failed:', err?.message || err);
    return null;
  }
}

export async function loadSeriesContext(userMessageText, recentHistory = [], ownerEmail = null) {
  if (!userMessageText) return '';

  try {
    const priorStructure = await loadPriorPartStructureContext(userMessageText, recentHistory);

    // Series Auto has actually recorded come first.
    //
    // This used to be a hardcoded list of five slugs, which meant that for the
    // Archetype series — seven published entries — none of "let's write the
    // next Archetype post", "next in the Archetype series", or "write part 8 of
    // the Archetype series" matched anything, so Auto received zero prior parts
    // and had to write the next entry blind. Only naming a specific published
    // title matched, and even then the filename filter below found just that
    // one file, never the series. That is the mechanical reason Auto struggled
    // to build series.
    const known = await detectSeriesFromMemory(ownerEmail, userMessageText);

    // Legacy patterns stay as a fallback for series not yet recorded in memory.
    ...
    let detectedSlug = known?.key || null;
    if (!detectedSlug) {
    ...
    if (detectedSlug && detectedSlug.length >= 6) {
      ...
      // When memory knows the series, use its recorded part slugs. The prefix
      // filter below cannot find a series whose entries have different names:
      // for The Archetype Series it matched "judas-archet" and returned only
      // the-judas-archetype.md, never the other six.
      const knownFiles =
        known?.slugs?.length
          ? known.slugs.map((s) => `${s}.md`).filter((f) => files.includes(f))
          : [];
      ...
      const seriesFiles = (
        ...
        .slice()
        .sort((a, b) => {
          // Recorded order wins when memory supplied the list.
          if (knownFiles.length) {
            return known.slugs.indexOf(a.replace(/\.md$/, '')) - known.slugs.indexOf(b.replace(/\.md$/, ''));
          }
...
/**
 * Every published series, by name, with its parts in order.
 *
 * Series membership used to exist only inside the title string and the slug, so
 * nothing could look it up. Auto filled the gap the way a language model does:
 * it produced something plausible. Asked about the Saban arc it called it "the
 * leadership series", and the drafts table, the only place structured series
 * data lived, was missing 81 of 98 published posts and had the four parts of
 * that arc filed under three different series slugs.
 *
 * The published corpus is the source of truth for done, so it is the source of
 * truth for series too. `series`, `series_name` and `series_part` are now real
 * frontmatter fields carried through the corpus build into knowledge.json.
 */
export async function loadSeriesIndexContext() {
  try {
    const { loadKnowledgeDocs } = await import('./corpusPullQuotes.js');
    const docs = await loadKnowledgeDocs();
    ...
    for (const d of docs) {
      ...
      if (!bySeries.has(d.series)) bySeries.set(d.series, { name: d.series_name || d.series, parts: [] });
      bySeries.get(d.series).parts.push({ part: d.series_part ?? null, slug: d.slug, title: d.title });
    }
    if (bySeries.size === 0) return '';

    const blocks = [...bySeries.entries()].map(([slug, s]) => {
      const lines = s.parts
        .sort((a, b) => (a.part ?? 99) - (b.part ?? 99))
        .map((p) => `  Part ${p.part ?? '?'}: ${p.title} (slug: ${p.slug})`);
      return `### ${s.name}\nseries slug: ${slug}\n${lines.join('\n')}`;
    });

    return `

## PUBLISHED SERIES INDEX

These are Bart's real series and their published parts, read from the corpus. This is the only correct source for what a series is called. Never invent a series name, never describe one by its subject matter ("the leadership series"), and never use a single post's title as the name of the series it belongs to. When writing a new part, use the exact series slug shown here.

${blocks.join('\n\n')}
`;
  } catch (err) {
    console.warn('[Auto V2] series index unavailable:', err?.message || err);
    return '';
  }
}
...
export async function runAutoChat(history = [], userMessage = '', scheduleContext = null, plainTextMessage) {
  ...
  const [
    ...
    contextProfile.needsDraftsContext
      ? loadApprovedDraftsContext(ownerEmail, userMessageText, recentUserMessages, {
          isNewThread: !Array.isArray(history) || history.length === 0,
        })
      : Promise.resolve(''),
    contextProfile.needsSeriesContext
      ? loadSeriesContext(userMessageText, history, ownerEmail)
      : Promise.resolve(''),
    loadStatedPreferencesContext(ownerEmail),
    loadSeriesMemoryContext(ownerEmail),
    loadGuardrailsContext(ownerEmail),
    loadSeriesIndexContext(),
  ]);
  const corpusContext = contextProfile.forceCorpusResearch
    ...
    : corpusContextRaw;
  const systemPrompt = buildSystemPrompt(corpusContext, publishedContext, performanceContext, imageSeriesContext, approvedDraftsContext, seriesContext, proactiveInsight, statedPreferencesContext, seriesMemoryContext, guardrailsContext, seriesIndexContext);
  ...
  try {
    ...
    const completed = await createCompleteMessage(
      ...
      {
        model: MODEL,
        max_tokens: AUTO_MAX_OUTPUT_TOKENS,
      ...
      {
        maxContinuations: 3,
        maxTokens: AUTO_MAX_OUTPUT_TOKENS,
        toolContext: { email: toolEmail, userMessage: userMessageText },
      }
    );
...
export async function runAutoChatStream(
  ...
  try {
    ...
    const chatAttachedImageUrlsThisTurn = Array.isArray(options.chatAttachedImageUrlsThisTurn)
      ? options.chatAttachedImageUrlsThisTurn.filter(Boolean)
      : [];
    const chatAttachedImageUrlsPrior = Array.isArray(options.chatAttachedImageUrlsPrior)
      ? options.chatAttachedImageUrlsPrior.filter(Boolean)
      : [];
    ...
    // web_search is an Anthropic server tool — it never appears in client toolResults.
    let toolsUsed = [...serverToolNamesFromContent(finalMessage?.content)];
    ...
    if (finalMessage.stop_reason === 'tool_use') {
      const looped = await continueAfterClientToolUse({
        ...
        onToken,
        toolContext: {
          // approve_stage decides from Bart's own words this turn. This is the
          // path every chat request actually takes, and leaving userMessage out
          // of it is why "Approved" came back as approval_not_given on
          // 2026-09-25. The context built in api/ao/auto/chat.js never reached
          // the tools that run inside the stream loop.
      ...
      toolsUsed = [...new Set([...toolsUsed, ...(looped.toolsUsed || [])])];
    ...
    return {
      ...
      messages,
      // The corpus-synthesis pass in api/ao/auto/chat.js re-calls the model with
      // this system prompt and history. It has to send the same tools, or Auto
      // answers that turn with no hands and correctly reports that it cannot
      // save drafts, schedule or publish.
      tools,
      saveDraftSucceeded,
      toolsUsed,
      toolResults,
