...
export async function loadPublishedContentContext(email = '', query = '') {
  ...
  try {
    ...
    // Format social posts into a readable block
    const socialLines = socialPosts.map((p) => {
      const date = p.published_at ? new Date(p.published_at).toISOString().split('T')[0] : 'unknown date';
      const platform = p.external_platform || 'unknown';
      const title = String(p.title || '').trim();
      return `- [${date}] [${platform}] ${title}`;
    });
...
### Recently Published Social Posts (${socialPosts.length} most recent)
${socialLines.join('\n')}
