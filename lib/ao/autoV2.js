...
export function buildSystemPrompt(
  ...
  // Durable series arcs and open candidate ideas (lib/ao/seriesMemory.js).
  // Distinct from seriesContext above, which globs published journal files
  // behind a hardcoded slug list and so cannot see work in progress.
  seriesMemoryContext = ''
) {
...
/**
 * Match a message against series Auto actually knows about.
 *
 * Checks the series title, its key, and every recorded part title, so "the next
 * Archetype post" resolves the same as "The Judas Archetype".
 * @returns {Promise<{ key: string, title: string, slugs: string[] } | null>}
 */
async function detectSeriesFromMemory(email, userMessageText) {
  ...
  try {
    ...
    for (const s of series) {
      const names = [words(s.title), words(s.series_key)].filter(Boolean);
      // A part title is a strong signal: naming any entry means this series.
      const partNames = (s.parts || []).map((p) => words(p.title)).filter(Boolean);

      let score = 0;
      for (const n of names) if (n && text.includes(n)) score = Math.max(score, n.length + 10);
      for (const n of partNames) if (n && text.includes(n)) score = Math.max(score, n.length);
      // "archetype" alone should resolve The Archetype Series — match the
      // distinctive token of the key, not just the whole phrase.
      for (const token of words(s.series_key).split(' ')) {
        if (token.length >= 6 && text.includes(token)) score = Math.max(score, token.length);
      }
...
export async function runAutoChat(history = [], userMessage = '', scheduleContext = null, plainTextMessage) {
  ...
  const corpusContext = contextProfile.forceCorpusResearch
    ...
    : corpusContextRaw;
  const systemPrompt = buildSystemPrompt(corpusContext, publishedContext, performanceContext, imageSeriesContext, approvedDraftsContext, seriesContext, proactiveInsight, statedPreferencesContext, seriesMemoryContext);
