...
2. Bart enters slug, video source URL (or uploads video file to storage), Spotify embed URL, and duration
3. Leave YouTube video ID blank to auto-upload to YouTube from the video source URL on publish (unlisted)
4. Bart clicks Approve (mints a one-time token via POST /api/ao/journal/publish-approval with kind: episode)
5. Bart clicks Publish episode (POST /api/ao/auto/episode-publish)
6. If video source URL is set and YouTube ID is blank, publish uploads to YouTube first, then commits ao-knowledge-hq-kit/journal/podcast/[slug].md
7. Vercel deploys. Live URL: archetypeoriginal.com/podcast/[slug]
8. Journal feed cross-post is created automatically by build-knowledge on deploy (no second file)
9. If YouTube upload fails, the episode still publishes without a video embed; Bart sees the error in the panel
