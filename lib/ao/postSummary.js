...
function toPlain(markdown) {
  return String(markdown || '')
    ...
    .replace(/\[([^\]]*)\]\([^)]*\)/g, '$1')
    .replace(/^#{1,6}\s+.*$/gm, ' ')
    // A line that is only italics or bold, with no sentence-ending punctuation,
    // is a label, not prose: "*The Archetype Series, Entry Ten*". Left in, the
    // whitespace collapse below ran it straight into the first sentence, and
    // The Cain Archetype published 2026-09-14 with the summary "The Archetype
    // Series, Entry Ten Cain did not fail...". A fully italic sentence that ends
    // in punctuation is real prose and is kept.
    .replace(/^[ \t]*([*_]{1,3})(?=\S)([^*_\n]*[^*_\n.!?…"'’”])\1[ \t]*$/gm, ' ')
    .replace(/[*_`>]+/g, '')
    .replace(/\s+/g, ' ')
    .trim();
