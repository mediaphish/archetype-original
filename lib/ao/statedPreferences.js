...
export async function classifyPreferenceWithModel(userMessage, { anthropicClient = null } = {}) {
  ...
  try {
    ...
    // Was pinned to claude-sonnet-4-20250514 while every other Auto path moved
    // on — this is the classifier that decides what Auto remembers across
    // threads, so it is exactly the wrong place to run the oldest model.
