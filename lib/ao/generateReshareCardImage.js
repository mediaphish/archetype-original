...
/**
 * Normalize unknown binary input into a Node Buffer.
 * OpenAI / fetch paths sometimes hand us Uint8Array or ArrayBuffer.
 */
...
async function loadAOMark(filename) {
  ...
  try {
    ...
    // Path-based load is sometimes more reliable than buffer load in napi-rs
    try {
...
export async function generateReshareCardImage({ photoBuffer, pullQuote, mood }) {
  ...
  try {
    ...
    console.log(
      '[generateReshareCardImage] photoBuffer type:',
      typeof photoBuffer,
      'isBuffer:',
    ...
    if (!isDecodableRasterBuffer(normalized)) {
      console.error(
        '[generateReshareCardImage] Failed to load photo buffer: not PNG/JPEG/WEBP — buffer length:',
        normalized.length,
        'magic:',
        describeImageMagic(normalized),
        'first8:',
        normalized.subarray(0, 8)
      );
      return {
        ok: false,
    ...
    let photoImg;
    try {
      photoImg = await loadImage(normalized);
    } catch (err) {
      console.error(
        '[generateReshareCardImage] Failed to load photo buffer:',
        err?.message,
        '— buffer length:',
        normalized.length,
        'magic:',
        describeImageMagic(normalized),
        'first8:',
        Array.from(normalized.subarray(0, 8))
      );
