 ...
 *
 * The optional refinement step runs on Claude through textModel.js. It used to be
 * a raw fetch to gpt-4o-mini, and OpenAI is kept for image creation only.
 */
...
async function maybeRefineChannelsWithModel(items, baseline) {
  if (!textModelConfigured() || !items?.length) return null;
  try {
    const snippet = items
    ...
    const parsed = await completeJson({
      prompt: `Given these interpretive captions for leadership quote cards, reply JSON ONLY: {"note":"one short sentence audience/channel tip","discourage_x":boolean}
